...
/**
 * Default Share Mapper Class <p>Maps disk and print share lookup requests to
 * the list of shares defined in the server configuration.
 * 
 * @author gkspencer
 */
public class HomeShareMapper implements ShareMapper, ConfigurationListener {
	...
	public void initializeMapper(
		ServerConfiguration config, ConfigElement params)
		...
		m_securityConfig =
			(SecurityConfigSection) m_config.getConfigSection(
				SecurityConfigSection.SectionName);

		// Filesystem configuration will usually be initialized after the
		// security configuration so we need to plug in
	...
	public SharedDevice findShare(
		...
		if ((typ == ShareType.DISK || typ == ShareType.UNKNOWN) &&
			name.compareTo(HOME_DIRECTORY_SHARE) == 0 &&
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"Map share " + name + ", type=" +
					ShareType.TypeAsString(typ) + ", client=" + client);

			// Find the user account details and check if a home directory has
			// been configured

			UserAccount acc = m_securityConfig.getUserAccounts().findUser(
				client.getUserName());
			if (acc != null && acc.hasHomeDirectory()) {
				...
				if (sess.hasDynamicShares()) {
					...
					// Check if the required share exists in the sessions
					// dynamic share list

					share = sess.getDynamicShareList().findShare(
						name, typ, false);
					...
					if (Debug.EnableInfo && hasDebug() && share != null)
						Debug.println(
							"  Reusing existing dynamic share for " + name);
				}

				// Check if we found a share, if not then create a new dynamic
				// share for the home directory
				...
				if (share == null && create == true) {
					...
					DiskDeviceContext diskCtx =
						new DiskDeviceContext(acc.getHomeDirectory());

					// Create a temporary shared device for the users home
					// directory

					DiskSharedDevice diskShare = new DiskSharedDevice(
						HOME_DIRECTORY_SHARE, diskDrv, diskCtx,
						SharedDevice.Temporary);
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  Mapped share " + name + " to " +
							acc.getHomeDirectory());
				}
			}
			else
			...
			// first, if that fails use a case
			// insensitive search.
		...
		if (share != null && share.getContext() != null &&
