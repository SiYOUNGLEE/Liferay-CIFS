 ...
 * @author gkspencer
 */
public class FTPServer extends NetworkFileServer
	...
	public FTPServer(ServerConfiguration config) {
		...
		// Add the FTP server as a configuration change listener of the server
		// configuration
		...
		m_configSection = (FTPConfigSection) config.getConfigSection(
		...
		if (m_configSection != null) {
			...
			if (getFTPConfiguration().hasFTPRootPath()) {
				...
				try {
					...
					m_rootPath = new FTPPath(
						getFTPConfiguration().getFTPRootPath());
				}
		...
		// Create the UTF-8 string normalizer, if the normalizer cannot be
		// initialized then swicth off UTF-8 support
	 ...
	 */
	protected final FTPDataSession allocateDataSession(
		FTPSrvSession sess, InetAddress remAddr, int remPort)
		throws IOException {

		// Check if there is a data port range configured, if not then just
		...
		if (Debug.EnableInfo && sess.hasDebug(FTPSrvSession.DBG_DATAPORT))
			Debug.println(
				"[FTP] Allocated data port " + dataPort +
				" to session " + sess.getSessionId());
	 ...
	 * @exception IOException
	 */
	protected final FTPDataSession allocatePassiveDataSession(
		FTPSrvSession sess, InetAddress localAddr)
		throws IOException {
		...
		if (Debug.EnableInfo && sess.hasDebug(FTPSrvSession.DBG_DATAPORT))
			Debug.println(
				"[FTP] Allocated passive data port " + dataPort +
				" to session " + sess.getSessionId());
	...
	protected final void releaseDataSession(FTPDataSession dataSess) {
		...
		// Check if there is a data port range configured, if not then do
		// nothing
		...
		if (Debug.EnableInfo &&
			dataSess.getCommandSession().hasDebug(FTPSrvSession.DBG_DATAPORT))
			Debug.println(
				"[FTP] Released data port " +
				dataSess.getAllocatedPort() + " for session " +
				dataSess.getCommandSession().getSessionId());
	}
	...
	private final int getNextFreeDataSessionPort() {
		...
		synchronized (m_dataSessions) {
			...
			while (m_dataSessions.findSession(dataPort) != null) {
				...
				// Check if the port number has reached the upper limit, if so
				// then wrap around
	...
	public final SharedDeviceList getShareList() {
		...
		if (m_shares == null)
			m_shares = getShareMapper().getShareList(
	...
	public void run() {
		...
		try {
			...
			// Create the server socket to listen for incoming FTP session
			// requests

			if (hasBindAddress())
				m_srvSock =
					new ServerSocket(
						getPort(), LISTEN_BACKLOG, getBindAddress());
			else {

				// If IPv6 is enabled we need to bind to the global IPv6 address
				// in order to get an IPv6 socket

				if (getFTPConfiguration().isIPv6Enabled())
					m_srvSock = new ServerSocket(
						getPort(), LISTEN_BACKLOG, InetAddress.getByName("::"));
				else
			...
			if (Debug.EnableInfo && hasDebug() &&
				getFTPConfiguration().hasFTPDataPortRange())
				Debug.println(
					"[FTP] Data ports restricted to range " +
					getFTPConfiguration().getFTPDataPortLow() + " - " +
			...
			while (hasShutdown() == false) {
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"[FTP] FTP session request received from " +
				...
				srvThread.setName(
					"Sess_FTP" + srvSess.getSessionId() + "_" +
					sessSock.getInetAddress().getHostAddress());
				srvThread.start();
			...
			if (hasShutdown() == false) {
				Debug.println(
					"[FTP] FTP Socket error : " + ex.toString(), Debug.Error);
				Debug.println(ex);
			...
			if (hasShutdown() == false) {
				Debug.println(
					"[FTP] FTP Server error : " + ex.toString(), Debug.Error);
				Debug.println(ex);
			}
		...
		while (enm.hasMoreElements()) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"[FTP] FTP Close session, id = " + sess.getSessionId());
	...
	public void shutdownServer(boolean immediate) {
		...
		// Close the FTP server listening socket to wakeup the main FTP server
		// thread
	...
	public int configurationChanged(
		...
		try {
			...
			throw new InvalidConfigurationException(
				"FTP Server configuration error", ex);
		}
