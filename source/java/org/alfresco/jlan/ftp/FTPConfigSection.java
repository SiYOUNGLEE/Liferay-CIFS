...
public class FTPConfigSection extends ConfigSection {
	...
	// Bind address and FTP server port. A port of -1 indicates do not start FTP
	// server.
	...
	// FTP root path, if not specified defaults to listing all shares as the
	// root
	...
	public FTPConfigSection(ServerConfiguration config) {
		...
		try {
			m_ftpAuthenticator.initialize(
				config, new GenericConfigElement("ftpAuthenticator"));
		}
	...
	/**
	 * Return the FTP server bind address, may be null to indicate bind to all
	 * available addresses
	 * 
	 * @return InetAddress
	 */
	...
	/**
	 * Set the FTP server bind address, may be null to indicate bind to all
	 * available addresses
	 * 
	...
	/**
	 * Set the FTP server port to use for incoming connections, -1 indicates
	...
	public final int setFTPDataPortLow(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.FTPDataPortLow, new Integer(port));
	...
	public final int setFTPDataPortHigh(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.FTPDataPortHigh, new Integer(port));
	...
	public final int setAllowAnonymousFTP(boolean ena)
		...
		if (m_ftpAllowAnonymous != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.FTPAllowAnon, new Boolean(ena));
	...
	public final int setFTPDebug(int dbg)
		...
		int sts =
			fireConfigurationChange(ConfigId.FTPDebugFlags, new Integer(dbg));
		m_ftpDebug = dbg;
	...
	public final int setFTPSiteInterface(FTPSiteInterface siteInterface)
		...
		int sts = fireConfigurationChange(
			ConfigId.FTPSiteInterface, siteInterface);
	...
	public final int setAuthenticator(String authClass, ConfigElement params)
		...
		try {
			...
			throw new InvalidConfigurationException(
				"Authenticator class " + authClass + " not found");
		}
		catch (Exception ex) {
			throw new InvalidConfigurationException(
	...
	public final int setIPv6Enabled(boolean ipv6ena)
		...
		int sts = fireConfigurationChange(
				ConfigId.FTPIPv6Enable, new Boolean(ipv6ena));
