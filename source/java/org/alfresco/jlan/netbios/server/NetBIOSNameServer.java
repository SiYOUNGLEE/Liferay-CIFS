 ...
 * @author gkspencer
 */
public class NetBIOSNameServer extends NetworkServer
	...
	private static final int AddNameWINSInterval = 250; // ms between requests
														// when using WINS
	...
	// Default time to live value for names registered by this server, in
	// seconds
	...
	class NetBIOSRequestHandler extends Thread {
		...
		public void run() {
			...
			while (m_hshutdown == false) {
				...
				try {
					...
					synchronized (m_reqList) {
						...
						if (m_reqList.size() == 0) {
							...
							if (Debug.EnableInfo && hasDebug())
								Debug.println(
									"NetBIOS handler waiting for request ...");
					...
					// Get the request retry count, for WINS only send one
					// request
					...
					while (req.hasErrorStatus() == false && 
						retry++ < reqRetry) {
						...
						case NetBIOSRequest.AddName:
							...
							if (hasPrimaryWINSServer())
								txsts = sendAddName(
									req, getPrimaryWINSServer(), false);
							else
								txsts = sendAddName(
									req, getBroadcastAddress(), true);
							break;
					...
					if (req.hasErrorStatus() == false) {
						...
						case NetBIOSRequest.AddName:
							...
							nbName.setExpiryTime(System.currentTimeMillis() +
								(nbName.getTimeToLive() * 1000L));

							// Inform listeners that the request was successful

							fireAddNameEvent(
								nbName, NetBIOSNameEvent.ADD_SUCCESS);
							break;
		 ...
		 * @return boolean
		 */
		private final boolean sendAddName(
			NetBIOSRequest req, InetAddress dest, boolean bcast) {
			...
			try {
				...
				for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); 
					i++) {

					// Build an add name request for the current IP address

					int len = addPkt.buildAddNameRequest(
						req.getNetBIOSName(), i, req.getTransactionId());
					if (bcast == false)
						addPkt.setFlags(0);
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  Add name " + (bcast ? "broadcast" : "WINS") + 
							", " + req);
				}
			}
			catch (IOException ex) {
				fireAddNameEvent(
					req.getNetBIOSName(), NetBIOSNameEvent.ADD_IOERROR);
		 ...
		 * @return boolean
		 */
		private final boolean sendRefreshName(
			NetBIOSRequest req, InetAddress dest, boolean bcast) {
			...
			try {
				...
				for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); 
					...
					int len = refreshPkt.buildRefreshNameRequest(
						req.getNetBIOSName(), i, req.getTransactionId());
					if (bcast == false)
						refreshPkt.setFlags(0);

					// Allocate the datagram packet, using the refresh name
					// buffer
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  Refresh name " + (bcast ? "broadcast" : "WINS") +
							", " + req);
				}
			}
			catch (IOException ex) {
		 ...
		 * @return boolean
		 */
		private final boolean sendDeleteName(
			NetBIOSRequest req, InetAddress dest, boolean bcast) {
			...
			try {
				...
				for (int i = 0; i < req.getNetBIOSName().numberOfAddresses(); 
					...
					int len = delPkt.buildDeleteNameRequest(
							req.getNetBIOSName(), i, req.getTransactionId());
					if (bcast == false)
						delPkt.setFlags(0);
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  Delete name " + (bcast ? "broadcast" : "WINS") + 
							", " + req);
				}
			}
			catch (IOException ex) {
		...
		public void run() {
			...
			while (m_hshutdown == false) {
				...
				try {
					...
					// Check if there are any registered names that will expire
					...
					synchronized (m_localNames) {
						...
						for (int i = 0; i < m_localNames.size(); i++) {
							...
							NetBIOSName nbName =
								(NetBIOSName) m_localNames.elementAt(i);

							// Check if the name has expired, or will expire
							// before the next wakeup event
							...
							if (nbName.getExpiryTime() < expireTime) {
								...
								if (Debug.EnableInfo && hasDebug())
									Debug.println(
										"Queuing name refresh for " + nbName);

								// Queue a refresh request for the NetBIOS name

								NetBIOSRequest nbReq = new NetBIOSRequest(
									NetBIOSRequest.RefreshName, nbName,
	...
	private final void commonConstructor()
		...
		// Add the NetBIOS name server as a configuration change listener of the
		...
		m_cifsConfig = (CIFSConfigSection) getConfiguration().getConfigSection(
		...
		if (m_cifsConfig != null) {
			...
			setSecondaryWINSServer(getCIFSConfiguration()
			...
			if (hasPrimaryWINSServer() == false) {
				...
				try {
					m_bcastAddr = InetAddress.getByName(getCIFSConfiguration()
							.getBroadcastMask());
				}
				catch (Exception ex) {
				}
	...
	/**
	 * Return the local address the server binds to, or null if all local
	 * addresses are used.
	 * 
	...
	/**
	 * Determine if the server binds to a particulat local address, or all
	 * addresses
	 * 
	 * @return boolean
	 */
	...
	public final synchronized void AddName(NetBIOSName name)
		...
		NetBIOSRequest nbReq = new NetBIOSRequest(
			NetBIOSRequest.AddName, name, getNextTransactionId());
	...
	public final synchronized void DeleteName(NetBIOSName name)
		...
		NetBIOSRequest nbReq = new NetBIOSRequest(
			NetBIOSRequest.DeleteName, name, getNextTransactionId(),
			DeleteNameRetries);
	 ...
	 * @param sts int
	 */
	protected final synchronized void fireAddNameEvent(
		NetBIOSName name, int sts) {
	 ...
	 */
	protected final synchronized void fireQueryNameEvent(
		NetBIOSName name, InetAddress addr) {
		...
		NetBIOSNameEvent evt =
			new NetBIOSNameEvent(name, NetBIOSNameEvent.QUERY_NAME);
	 ...
	 * @param addr InetAddress
	 */
	protected final synchronized void fireNameRegisterEvent(
		NetBIOSName name, InetAddress addr) {
		...
		NetBIOSNameEvent evt =
			new NetBIOSNameEvent(name, NetBIOSNameEvent.REGISTER_NAME);
	 ...
	 * @param addr InetAddress
	 */
	protected final synchronized void fireNameReleaseEvent(
		NetBIOSName name, InetAddress addr) {
	 ...
	 * @param fromPort int
	 */
	protected final void processNameQuery(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"%% Query name=" + searchName + ", type=" +
				NetBIOSName.TypeAsString(nameType) + ", len=" + len);
		...
		while (enm.hasMoreElements() && foundName == false) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"NetBIOS Name - " + nbName.getName() + ", len=" +
					nbName.getName().length() + ",type=" +
					NetBIOSName.TypeAsString(nbName.getType()));

			// Check if the name matches the query name

			if (nbName.getType() == nameType &&
				nbName.getName().compareTo(searchName) == 0)
				foundName = true;
		}
		...
		if (foundName == true) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					...
					" in local name table : " + nbName.toString());
			...
			if (Debug.EnableInfo && hasDebug()) {
				Debug.println(
					"%% NetBIOS Reply to " + fromAddr.getHostAddress() + " :-");
	 ...
	 * @param fromPort int
	 */
	protected final void processNameRegister(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"%% Added remote name " + nbName.toString() +
				" to remote names table");
	}
	 ...
	 * @param fromPort int
	 */
	protected final void processNameRelease(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"%% Released remote name " + nbName.toString() +
				" from remote names table");
	}
	 ...
	 * @param fromPort int
	 */
	protected final void processQueryResponse(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
	}
	 ...
	 * @param fromPort int
	 */
	protected final void processRegisterResponse(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
		...
		if (errCode != 0) {
			...
			byte[] hostIP = fromAddr.getAddress();
			NetBIOSName nbName =
				new NetBIOSName(regName, nameType, false, hostIP);
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println("%% Name Registration Successful name=" +
					req.getNetBIOSName().getName());
	 ...
	 * @param fromPort int
	 */
	protected final void processReleaseResponse(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
	}
	 ...
	 * @param fromPort int
	 */
	protected final void processWack(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
	}
	 ...
	 * @param fromPort int
	 */
	protected final void processAdapterStatus(
		NetBIOSPacket pkt, InetAddress fromAddr, int fromPort) {
		...
		int pktLen =
			pkt.buildAdapterStatusResponse(nameList, hasPrimaryWINSServer()
				? NetBIOSPacket.NAME_TYPE_PNODE : 
					NetBIOSPacket.NAME_TYPE_BNODE);
	 ...
	 * @param l QueryNameListener
	 */
	public final synchronized void removeQueryNameListener(
		QueryNameListener l) {
	...
	public void run() {
		...
		try {
			...
			if (hasBindAddress()) {
				...
				for (int i = 0; i < addrs.length; i++) {
					...
					// Check for a valid address, filter out '127.0.0.1' and
					// '0.0.0.0' addresses
				...
				// If no valid addresses were found use the network interface
				// list to find the local server address(es)
				...
				if (ipList.size() == 0) {
					...
					if (niEnum != null) {
						...
						while (niEnum.hasMoreElements()) {
							...
							NetworkInterface ni =
								(NetworkInterface) niEnum.nextElement();
							...
							if (niAddrs != null) {
								...
								while (niAddrs.hasMoreElements()) {
									InetAddress curAddr =
										(InetAddress) niAddrs.nextElement();

									if (curAddr.getHostAddress().equals(
										"127.0.0.1") == false &&
										curAddr.getHostAddress().equals(
											"0.0.0.0") == false)
						...
						if (Debug.EnableInfo && ipList.size() > 0 && hasDebug())
							Debug.println(
								"Found " + ipList.size() +
								" addresses using interface list");
					}
				}
			}
			...
			// Allocate the NetBIOS request queue, and add the server name/alias
			// name requests
			...
			AddName(new NetBIOSName(
				m_cifsConfig.getServerName(), NetBIOSName.FileServer, false,
				ipList, DefaultTTL));
			AddName(new NetBIOSName(
				m_cifsConfig.getServerName(), NetBIOSName.WorkStation, false,
				ipList, DefaultTTL));

			if (getCIFSConfiguration().getDomainName() != null)
				AddName(new NetBIOSName(
					m_cifsConfig.getDomainName(), NetBIOSName.Domain, true,
					ipList, DefaultTTL));

			// Check if the server has alias names configured, if so then also
			// add those names
			...
			if (getCIFSConfiguration().hasAliasNames()) {
				...
				for (int i = 0; i < names.numberOfStrings(); i++) {
					...
					AddName(new NetBIOSName(
						alias, NetBIOSName.FileServer, false, ipList,
						DefaultTTL));
					AddName(new NetBIOSName(
						alias, NetBIOSName.WorkStation, false, ipList,
						DefaultTTL));
				}
			}
		...
		// If there are any pending requests in the queue then wakeup the
			...
			while (m_shutdown == false) {
				...
				try {
					...
					if (Debug.EnableInfo && hasDebug()) {
						...
						Debug.println(
							"%% NetBIOS Name Server Rx Datagram from " +
						...
						HexDump.Dump(
							buf, pkt.getLength(), 0, Debug.getDebugInterface());
					}
					...
					// Store the error and inform listeners of the server error.
					// If the server is shutting down we expect a
					// socket error as the socket is closed by the shutdown
					// thread and the pending read request generates an
					// exception.
	...
	protected final void sendPacket(NetBIOSPacket nbpkt, int len)
		...
		DatagramPacket pkt =
			new DatagramPacket(
				nbpkt.getBuffer(), len, NetworkSettings.getBroadcastAddress(),
				getPort());
	...
	protected final void sendPacket(
		NetBIOSPacket nbpkt, int len, InetAddress replyAddr, int replyPort)
		throws java.io.IOException {

		// Allocate the datagram packet, using the add name buffer

		DatagramPacket pkt =
			new DatagramPacket(nbpkt.getBuffer(), len, replyAddr, replyPort);
	...
	private final NetBIOSRequest findRequest(int id) {
		...
		synchronized (m_reqList) {
			...
			while (req == null && idx < m_reqList.size()) {
				...
				// Get the current request and check if it is the required
				// request

				NetBIOSRequest curReq =
					(NetBIOSRequest) m_reqList.elementAt(idx++);
	...
	public void shutdownServer(boolean immediate) {
		...
		// If the shutdown is not immediate then release all of the names
		...
		if (isActive() && immediate == false) {
			...
			// Wait for the request handler thread to process the delete name
			// requests
		...
		try {
			...
			if (m_socket != null) {
				...
				// Send a dummy packet to release the receive on the datagram
				// socket, then close the socket

				// m_socket.send(new DatagramPacket(new byte[0], 0,
				// m_socket.getLocalAddress(), getPort()));
				try {
					m_socket.close();
				}
				catch (Exception ex) {
	...
	/**
	 * Validate configuration changes that are relevant to the NetBIOS name
	 * server
	 * 
	 * @param id int
	 * @param config ServerConfiguration
	...
	public int configurationChanged(
		...
		try {
			...
			throw new InvalidConfigurationException(
				"NetBIOS Server configuration error", ex);
		}
