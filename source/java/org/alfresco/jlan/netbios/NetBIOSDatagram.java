...
/**
 * NetBIOS datagram class <p>Used for sending/receiving NetBIOS broadcast
 * datagrams that are used for name lookups and registration.
 * 
 * @author gkspencer
 */
public class NetBIOSDatagram {
	...
	public final NetBIOSName getDestinationName() {
		...
		if (name != null) {
			...
			NetBIOSName nbName = new NetBIOSName(
				name.substring(0, 14), name.charAt(15), false);
	...
	public final NetBIOSName getSourceName() {
		...
		if (name != null) {
			...
			NetBIOSName nbName =
				new NetBIOSName(name.substring(0, 14), name.charAt(15), false);
			return nbName;
		}
		return null;
	}
	 ...
	 * @exception UnknownHostException Failed to generate the broadcast mask for
	 *                the network
	 */
	public final void SendDatagram(
		int dgramTyp, String fromName, char fromNameType, String toName,
		char toNameType, byte[] userData, int userLen, int userOff,
	 ...
	 */
	public final void SendDatagram(
		int dgramTyp, String fromName, String toName, byte[] userData,
		int userLen)
		throws IOException, UnknownHostException {
		...
		SendDatagram(
			dgramTyp, fromName, NetBIOSName.FileServer, toName,
	 ...
	 */
	public final void SendDatagram(
		int dgramTyp, DatagramSocket sock, String fromName, char fromNameType,
		String toName, char toNameType, byte[] userData, int userLen,
		int userOff)
		throws IOException {
		...
		InetAddress destAddr = InetAddress.getByName(
			NetworkSettings.GenerateBroadcastMask(null));
		DatagramPacket dgram = new DatagramPacket(
			m_buf, userLen + NB_USERDATA, destAddr, 
			RFCNetBIOSProtocol.DATAGRAM);
	 ...
	 */
	public final void SendDatagram(
		int dgramTyp, DatagramSocket sock, String fromName, String toName,
		byte[] userData, int userLen)
		throws IOException {

		// Send the datagram from the standard port

		SendDatagram(
			dgramTyp, sock, fromName, NetBIOSName.FileServer, toName,
			NetBIOSName.FileServer, userData, userLen, 0);
	}
	...
	public final void setSourceName(String name) {
		...
		NetBIOSSession.EncodeName(
			name, NetBIOSName.FileServer, m_buf, NB_FROMNAME);
	}
