...
/**
 * NetBIOS Datagram Socket Class <p>Singleton class that allows multiple users
 * of the socket.
 * 
 * @author gkspencer
 */
public class NetBIOSDatagramSocket {
	...
	private NetBIOSDatagramSocket()
		...
		if (m_defBindAddr == null)
			m_broadcastAddr = InetAddress.getByName(
				NetworkSettings.GenerateBroadcastMask( null));
		else
			m_broadcastAddr = InetAddress.getByName(
				NetworkSettings.GenerateBroadcastMask(
					m_defBindAddr.getHostAddress()));
	}
	...
	public final int receiveDatagram(NetBIOSDatagram dgram)
		...
		DatagramPacket pkt =
			new DatagramPacket(dgram.getBuffer(), dgram.getBuffer().length);
	...
	public final void sendDatagram(
		NetBIOSDatagram dgram, InetAddress destAddr, int destPort)
		throws IOException {

		// Create a datagram packet using the NetBIOS datagram buffer

		DatagramPacket pkt = new DatagramPacket(
			dgram.getBuffer(), dgram.getLength(), destAddr, destPort);
	...
	public final void sendBroadcastDatagram(NetBIOSDatagram dgram)
		...
		DatagramPacket pkt =
			new DatagramPacket(
				dgram.getBuffer(), dgram.getLength(), m_broadcastAddr,
				m_defPort);
