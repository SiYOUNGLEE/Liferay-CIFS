...
/**
 * NetBIOS Selector Class <p>Selector type class for NetBIOSSocket objects.
 * Groups a set of sockets to wait for events on.
 * 
 * @author gkspencer
 */
public class NetBIOSSelector {
	 ...
	 */
	protected final NetBIOSSelectionKey registerSocket(
		NetBIOSSocket socket, int ops)
		...
		synchronized (m_selectionKeys) {
			...
			if (m_selectionKeys.containsKey(key))
				throw new IOException(
					"Socket already registered with selector");
	...
	public final NetBIOSSelectionKey deregisterSocket(NetBIOSSocket socket)
		...
		NetBIOSSelectionKey selKey = m_selectionKeys.remove(
			new Integer(socket.getSocket()));
	...
	public final int select()
		...
		synchronized (m_selectionKeys) {
			...
			// Copy sockets that are interested in accept or read events to the
			// socket id list
			...
			while (keys.hasNext()) {
				...
				// Get the current selection key and check if is interested in
				// accept or read events

				Integer curKey = keys.next();
				NetBIOSSelectionKey selKey = m_selectionKeys.get(curKey);

				if (m_triggeredKeys.contains(selKey) == false &&
					(selKey.interestOps() & NetBIOSSelectionKey.OP_ACCEPT +
						NetBIOSSelectionKey.OP_READ) != 0) {
		...
		int eventCnt = Win32NetBIOS.SelectReceiveSockets(
			idx, m_sockIds, m_eventIds);
		...
		if (eventCnt > 0) {
			...
			for (int i = 0; i < eventCnt; i++) {
				...
				// Get the selection key for the triggered socket, add to the
				// triggered set

				NetBIOSSelectionKey selKey = m_selectionKeys.get(
					new Integer(m_eventIds[i]));
				if (selKey != null) {
					selKey.setTriggers(selKey.socket().isListener()
						? NetBIOSSelectionKey.OP_ACCEPT
						: NetBIOSSelectionKey.OP_READ);
