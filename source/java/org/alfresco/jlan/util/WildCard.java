...
/**
 * Wildcard Utility Class. <p>The WildCard class may be used to check Strings
 * against a wildcard pattern using the SMB/CIFS wildcard rules. <p>A number of
...
public final class WildCard {
	...
	public final static String convertUnicodeWildcardToDOS(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (ch == SINGLECHAR_UNICODE_WILDCARD) {
				...
				// then translate to the DOS multi character
				// wildcard
				...
				// single/multi character wildcard

				if (i < (str.length() - 1)) {
					char nextCh = str.charAt(i + 1);
					if (nextCh == MULTICHAR_WILDCARD ||
	 ...
	 */
	/**
	 * public static final void main(String[] args) { // Test file names
	 * String[] names = { "TEST.TXT", "TEST.OUT", "NOTEPAD.EXE", "Notepad.com",
	 * "notepad.exe", "test.txt", "test.out" }; // Wildcard strings String[]
	 * wildcards = { "*.*", "*", "TEST.*", "*.TXT", "????.*", "N??????.???" };
	 * // Run the wildcard tests java.io.PrintStream out = System.out; try { //
	 * Loop though the wildcards for ( int i = 0; i < wildcards.length; i++) {
	 * // Create a wildcard, case insensitive check WildCard wcard = new
	 * WildCard(wildcards[i], false); out.println(wcard + ":"); // Test the file
	 * names for ( int j = 0; j < names.length; j++) { out.print("  ");
	 * out.print(names[j]); out.print(": ");
	 * out.println(wcard.matchesPattern(names[j]) ? "Match" : "NoMatch"); }
	 * out.println(); // Create a wildcard, case sensitive check wcard = new
	 * WildCard(wildcards[i], true); out.println(wcard + ":"); // Test the file
	 * names for ( int j = 0; j < names.length; j++) { out.print("  ");
	 * out.print(names[j]); out.print(": ");
	 * out.println(wcard.matchesPattern(names[j]) ? "Match" : "NoMatch"); }
	 * out.println("----------------------------------------"); } } catch (
	 * Exception ex) { out.println("Error: " + ex.toString()); } }
	 **/
