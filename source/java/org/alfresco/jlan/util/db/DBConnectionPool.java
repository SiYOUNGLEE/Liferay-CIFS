...
public class DBConnectionPool {
	...
	// Period that the database reaper checks the free connections to see if
	// they have timed out
	...
	/**
	 * Database Connection Reaper Thread Class <p>Check for connections whose
	 * lease has expired and return them to the free connection pool.
	 */
		...
		public void run() {
			...
			while (m_shutdown == false) {
				...
				synchronized (m_allocPool) {
					...
					// Debug.println("DBConnectionReaper allocPool=" +
					// m_allocPool.size() + ", freePool=" + m_freePool.size());
					...
					while (enm.hasMoreElements()) {
						...
						if (expire.longValue() != PermanentLease &&
							expire.longValue() < timeNow)
							removeConn = true;
						else if (expire.longValue() == PermanentLease) {

							try {

								// Check if the connection has been closed or
								// the server is down
								...
								if (conn.isClosed())
									...
									// Toggle the auto-commit flag on the
									// connection, this will check the
									// connection on most
									// databases
							...
							if (removeConn == true)
								Debug.println(
									"DBConnectionReaper Permanent lease " +
									"connection error");
						}
						...
						if (removeConn == true) {
							...
							// Connection lease has expired, remove from the
							// allocated list and close the connection

							m_allocPool.remove(conn);
							try {
								conn.close();
								Debug.println(
									"DBConnectionReaper closed expired " +
									"connection, conn=" +
									conn);
							}
							catch (SQLException ex) {
				...
				synchronized (m_freePool) {
					...
					// Release connections from the free pool until below the
					...
					while (m_freePool.size() > getMaximumPoolSize()) {
						...
						Debug.println(
							"DBConnectionReaper trimming free pool, " +
							m_freePool.size() + "/" + getMaximumPoolSize());

						// Remove a connection from the free pool and close the
						// connection
				...
				// Check the connections in the free pool to see if they have
				// been timed out by the server

				if (loopCnt % m_onlineCheckInterval == 0 ||
					isOnline() == false) {
					...
					synchronized (m_freePool) {
						...
						// Debug.println(
						// "DBConnectionReaper Checking free pool connection status ...");

						// Check if the connections in the free pool are still
						// connected/valid
						...
						while (idx < m_freePool.size()) {
							...
							try {
								...
								if (conn.isClosed()) {
									...
									// Debug.println(
									// "DBConnectionReaper Removed closed connection from free pool");
								}
								else {
								...
								// Remove the current connection from the free
								// pool
								...
								// Debug.println(
								// "DBConnectionReaper Removed closed connection from free pool (exception)");
							}
						}

						if (isOnline()) {

							// Check if the free and allocated pools are empty,
							// this indicates that the database server
							// is offline
							...
							// Try and get a connection from the pool, this will
						...
						// Debug.println(
						// "DBConnectionReaper Free pool check done.");
					}
				}
			}
		}
	...
	public DBConnectionPool(
		String driver, String dsn, String user, String pwd, int initConns,
		int maxConns)
		throws Exception {
	...
	/**
	 * Get a connection from the pool with the specified lease time. A lease
	 * time of -1 indicates that the connection lease is permanent.
	 * 
	...
	public final Connection getConnection(long expireTime) {
		...
		synchronized (m_freePool) {
			...
			if (m_freePool.size() > 0) {
				...
				try {
					...
					conn = null;
					Debug.println("%%%%% SQL Connection Error: " +
						ex.toString());
				}
			}
			else if (isOnline() == false) {

				// Try and create a new connection, if we succeed then the
				// database server is back online
		...
		synchronized (m_allocPool) {
			...
			if (conn != null)
				...
				// If a connection has not been allocated and there are spare
	...
	public final void releaseConnection(Connection conn) {
		...
		// Remove the connection from the in use pool and put it back in the
		// free list
	...
	public final void renewLease(Connection conn, long expireTime) {
		...
		synchronized (m_allocPool) {
			...
			// Update the expire time for the lease and add back to the
			// allocated pool
	...
	public final void setOnlineCheckInterval(int interval) {
		...
		// Set the reaper thread online check interval, convert seconds to
		// thread wakeups
	...
	protected final Connection createConnection()
		...
		return DriverManager.getConnection(
			getDSN(), getUserName(), getPassword());
	}
	...
	protected final void notifyConnectionPoolState() {
		...
		Debug.println("DBConnectionPool: Database server is " +
