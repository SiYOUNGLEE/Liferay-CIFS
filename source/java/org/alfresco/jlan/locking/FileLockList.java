...
/**
 * File Lock List Class <p>Contains a list of the current locks on a file.
 * 
 * @author gkspencer
 */
public class FileLockList {
	...
	public final FileLock removeLock(FileLock lock) {
		return removeLock(
			lock.getOffset(), lock.getLength(), lock.getProcessId());
	}
	...
	/**
	 * Check if the new lock should be allowed by comparing with the locks in
	 * the list.
	 * 
	 * @param lock FileLock
	...
	public final boolean canReadFile(long offset, long len, int pid) {
		...
		for (int i = 0; i < numberOfLocks(); i++) {
			...
			// Check if the process owns the lock, if not then check if there is
			// an overlap
