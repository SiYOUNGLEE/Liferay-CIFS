...
public class CoreServerConfigSection extends ConfigSection {
	...
	public final void setThreadPool(int initSize, int maxSize)
		...
		if (initSize <= 0 || maxSize <= 0)
			throw new InvalidConfigurationException(
				"Invalid initial or maximum thread count, " + initSize + "/" +
					maxSize);

		if (initSize > maxSize)
			throw new InvalidConfigurationException(
				"Invalid initial thread count, higher than maximum count, " +
					initSize + "/" + maxSize);
	...
	public final void setMemoryPool(
		...
		if ((pktSizes.length != initAlloc.length) ||
			(pktSizes.length != maxAlloc.length))
			throw new InvalidConfigurationException(
				"Invalid packet size/allocation lists, lengths do not match");
		...
		if (pktSizes.length > 1) {
			for (int i = 1; i <= pktSizes.length - 1; i++)
				if (pktSizes[i] < pktSizes[i - 1])
					throw new InvalidConfigurationException(
						"Packet size list is not in ascending order");
		}
		...
		if (m_memoryPool != null)
			throw new InvalidConfigurationException(
				"Memory pool already configured");
