...
public class SecurityConfigSection extends ConfigSection {
	...
	public SecurityConfigSection(ServerConfiguration config) {
		...
		try {
			m_shareMapper.initializeMapper(
				config, new GenericConfigElement("shareMapper"));
		}
		catch (InvalidConfigurationException ex) {
		}
	}
	 ...
	 */
	public final int setAccessControlManager(
		String aclMgrClass, ConfigElement params)
		...
		try {
			...
			if (aclObj instanceof AccessControlManager) {
				...
				throw new InvalidConfigurationException(
					"Access control manager does not implement required " +
					"interface");
		}
		catch (ClassNotFoundException ex) {
			throw new InvalidConfigurationException(
				"Access control manager class " + aclMgrClass + " not found");
		}
		catch (Exception ex) {
			throw new InvalidConfigurationException(
				"Access control manager class error");
		}
	 ...
	 * Set the access control manager to be used to control per share access
	 * 
	 * @param aclMgr the access control manager to be used to control per share
	 *            access
	 * @exception InvalidConfigurationException
	 */
		...
		try {
			...
			if (jceObj instanceof java.security.Provider) {
				...
				sts = fireConfigurationChange(
					ConfigId.SecurityJCEProvider, jceProvider);
				...
				throw new InvalidConfigurationException(
					"JCE provider class is not a valid Provider class");
		}
		catch (ClassNotFoundException ex) {
			throw new InvalidConfigurationException(
				"JCE provider class " + providerClass + " not found");
		}
		catch (Exception ex) {
	...
	public final int setShareMapper(String mapperClass, ConfigElement params)
		...
		try {
			...
			if (mapperObj instanceof ShareMapper) {
				...
				throw new InvalidConfigurationException(
					"Share mapper class is not implementation of ShareMapper " +
					"interface");
		}
		catch (ClassNotFoundException ex) {
			throw new InvalidConfigurationException(
				"Share mapper class " + mapperClass + " not found");
		}
		catch (Exception ex) {
	 ...
	 * @exception InvalidConfigurationException
	 */
	public final int setShareMapper(ShareMapper shareMapper)
		throws InvalidConfigurationException {
	...
	public final int setUsersInterface(String usersClass, ConfigElement params)
		...
		try {
			...
			if (usersObj instanceof UsersInterface) {
				...
				throw new InvalidConfigurationException(
					"Users interface class does not implement required " +
					"interface");
		}
		catch (ClassNotFoundException ex) {
			throw new InvalidConfigurationException(
				"Users interface class " + usersClass + " not found");
		}
		catch (Exception ex) {
			throw new InvalidConfigurationException(
				"Users interface class error");
		}
		...
		sts = fireConfigurationChange(
			ConfigId.SecurityUsersInterface, usersIface);
