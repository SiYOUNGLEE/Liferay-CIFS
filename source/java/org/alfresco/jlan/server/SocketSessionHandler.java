...
/**
 * Socket Session Handler Class <p> Implementation of a session handler that
 * uses a Java socket to listen for incoming session requests.
 * 
 * @author gkspencer
 */
public abstract class SocketSessionHandler extends SessionHandlerBase
	implements Runnable {
	...
	public void initializeSessionHandler(NetworkServer server)
		...
		if (hasBindAddress())
			m_srvSock = new ServerSocket(
				getPort(), getListenBacklog(), getBindAddress());
		else
			...
			while (hasShutdown() == false) {
				...
				try {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"[" + getProtocolName() +
							"] Failed to create session, " + ex.toString());
				}
			}
		}
		catch (SocketException ex) {
