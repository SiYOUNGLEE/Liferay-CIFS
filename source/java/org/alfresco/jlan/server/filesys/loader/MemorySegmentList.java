...
/**
 * Memory Segment List Class <p>Contains an ordered list of MemorySegment
 * objects.
 * 
 * @author gkspencer
 */
public class MemorySegmentList {
	...
	/**
	 * Remove memory segments with a read count equal or below the specified
	 * level
	 * 
	 * @param readLev int
	 * @return int
	...
	public final synchronized int removeSegments(int readLev) {
		...
		while (idx < m_segments.size()) {
			...
			// Check if the current memory segment is equal/below the read hit
			// threshold
	...
	/**
	 * Decrement the read hit counts for all memory segments by the specified
	 * amount, and optionally remove memory segments that have a hit count of
	 * zero.
	 * 
	 * @param decr int
