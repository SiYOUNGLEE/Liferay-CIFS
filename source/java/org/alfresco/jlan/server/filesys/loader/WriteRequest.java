...
/**
 * Write Request Class <p>Contains the details of a write request to be
 * performed using a background thread.
 * 
 * @author gkspencer
 */
public class WriteRequest {
	 ...
	 */
	public WriteRequest(
		NetworkFile file, TreeConnection tree, DiskInterface disk,
		int writeLen, long writeOff, byte[] data, int dataOff) {
	...
	/**
	 * Perform the write request, return the write length or -1 if an error
	 * occurs
	 * 
	 * @return int
	 */
	public final int doWrite() {
		...
		try {
			...
			synchronized (m_file) {
				...
				wlen = m_disk.writeFile(
					null, m_conn, m_file, m_buffer, m_dataOff, m_writeLen,
					m_writeOff);
