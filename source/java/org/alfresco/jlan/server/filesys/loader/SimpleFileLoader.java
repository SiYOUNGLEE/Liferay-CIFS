...
/**
 * Simple File Loader Class <p>The simple file loader class maps the file
 * load/store requests to files within the local filesystem.
 * 
 * @author gkspencer
 */
public class SimpleFileLoader implements FileLoader, NamedFileLoader {
	...
	public NetworkFile openFile(
		...
		String fullName =
			FileName.buildPath(
				getRootPath(), params.getPath(), null,
		...
		if (m_debug)
			Debug.println(
				"SimpleFileLoader.openFile() fname=" +
				params.getPath() + ", fid=" + fid + ", did=" + did +
				", fullName=" + fullName);
		...
		if (file.exists() == false) {
			...
			String mappedPath =
				FileName.mapPath(getRootPath(), params.getPath());
			if (mappedPath == null && create == false)
				throw new FileNotFoundException(
					"File does not exist, " + params.getPath());
		...
		LocalDataNetworkFile netFile = new LocalDataNetworkFile(
			params.getPath(), fid, did, file);
	...
	public void deleteFile(String fname, int fid, int stid)
		...
		if (m_debug)
			Debug.println(
				"SimpleFileLoader.deleteFile() fname=" + fname +
				", fid=" + fid);

		// Get the full path for the file

		String name = FileName.buildPath(
			getRootPath(), fname, null, java.io.File.separatorChar);
	...
	public void createDirectory(String dir, int fid)
		...
		if (m_debug)
			Debug.println(
				"SimpleFileLoader.createDirectory() dir=" + dir +
				", fid=" + fid);

		// Get the full path for the new directory

		String dirname = FileName.buildPath(
			getRootPath(), dir, null, java.io.File.separatorChar);
	...
	public void deleteDirectory(String dir, int fid)
		...
		if (m_debug)
			Debug.println(
				"SimpleFileLoader.deleteDirectory() dir=" + dir +
				", fid=" + fid);
	...
	public void renameFileDirectory(
		String curName, int fid, String newName, boolean isdir)
		throws IOException {
		...
		if (m_debug)
			Debug.println(
				"SimpleFileLoader.renameFileDirectory() curName=" +
				curName + ", fid=" + fid + ", newName=" + newName);

		// Get the full path for the existing file and the new file name

		String curPath = FileName.buildPath(
			getRootPath(), curName, null, java.io.File.separatorChar);
		String newPath = FileName.buildPath(
			getRootPath(), newName, null, java.io.File.separatorChar);
		...
		if (curFile.renameTo(newFile) == false)
			throw new IOException(
				"Rename " + curPath + " to " + newPath + " failed");
	}
	...
	public void initializeLoader(ConfigElement params, DeviceContext ctx)
		...
		if (nameVal == null || nameVal.getValue() == null ||
			nameVal.getValue().length() == 0)
			throw new FileLoaderException(
				"SimpleFileLoader RootPath parameter required");
		...
		if (root.exists() == false || root.isFile())
			throw new FileLoaderException(
				"SimpleFileLoader RootPath does not exist or is not " +
				"a directory, " + m_rootPath);
	}
