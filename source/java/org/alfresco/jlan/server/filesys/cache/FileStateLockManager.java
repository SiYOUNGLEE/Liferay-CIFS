...
/**
 * File State Lock Manager Class <p>File locking implementation that uses the
 * file state cache to keep track of locks on a file.
 * 
 * @author gkspencer
 */
public class FileStateLockManager implements LockManager {
	...
	public void lockFile(
		SrvSession sess, TreeConnection tree, NetworkFile file, FileLock lock)
		...
		if ((file instanceof NetworkFileStateInterface) == false)
			throw new IllegalArgumentException(
				"NetworkFile does not implement NetworkFileStateInterface");

		// Get the file state associated with the file

		NetworkFileStateInterface fstateIface =
			(NetworkFileStateInterface) file;
		FileState fstate = fstateIface.getFileState();

		if (fstate == null)
			throw new IOException("Open file without state (lock)");

		// Add the lock to the active lock list for the file, check if the new
		// lock conflicts with
		// any existing locks. Add the lock to the file instance so that locks
	...
	public void unlockFile(
		...
		// Remove the lock from the active lock list for the file, and the file
		// instance
	 ...
	 * @param pid int
	 */
	public FileLock createLockObject(
		SrvSession sess, TreeConnection tree, NetworkFile file, long offset,
		long len, int pid) {
	...
	/**
	 * Release all locks that a session has on a file. This method is called to
	 * perform cleanup if a file is closed that has active locks or if a session
	 ...
	 * @param file NetworkFile
	 */
	public void releaseLocksForFile(
			...
			synchronized (file) {
				...
				while (file.numberOfLocks() > 0) {
					...
					try {
						...
						// Unlock will remove the lock from the global list and
						// the local files list
