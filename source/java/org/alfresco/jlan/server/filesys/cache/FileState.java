...
/**
 * File State Class <p>Caches information about a file/directory so that the
 * core server does not need to make calls to the shared device driver.
 * 
 * @author gkspencer
 */
public class FileState {
	...
	private static final String[] _fileStates =
		{
			"LoadWait", "Loading", "Available", "Updated", "SaveWait",
			"Saving", "Saved", "Deleted"
		};
	...
	// File status, indicates if the file/folder exists and if it is a file or
	// folder.
	...
	/**
	 * Check if the file can be opened depending on any current file opens and
	 * the sharing mode of the first file open
	 * 
	...
	public final synchronized int incrementOpenCount() {
		...
		// if ( m_openCount > 1)
		// Debug.println("@@@@@ File open name=" + getPath() + ", count=" +
		// m_openCount);
	...
	public final void setPath(String path) {
		...
		// Split the path into directories and file name, only uppercase the
	...
	public final void addLock(FileLock lock)
		...
		if (m_lockList == null) {
			...
			synchronized (this) {
				...
				// Allocate the lock list, check if the lock list has been
				// allocated elsewhere
	...
	/**
	 * Check if the file is readable for the specified section of the file and
	 * process id
	 * 
	 * @param offset long
	 * @param len long
	...
	/**
	 * Check if the file is writeable for the specified section of the file and
	 * process id
	 * 
	 * @param offset long
	 * @param len long
	...
	/**
	 * Normalize the path to uppercase the directory names and keep the case of
	 * the file name.
	 * 
	 * @param path String
	 * @return String
