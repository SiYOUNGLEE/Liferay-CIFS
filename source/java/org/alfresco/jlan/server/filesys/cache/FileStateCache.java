...
/**
 * File State Cache Class <p>Contains a cache of file/directory information for
 * recently accessed files/directories to reduce the calls made by the core
 * server code to the shared device driver.
 * 
 * @author gkspencer
 */
public class FileStateCache implements Runnable {
	...
	private FileStateFactoryInterface m_stateFactory =
		new DefaultFileStateFactory();
	...
	public final synchronized void addFileState(FileState fstate) {
		...
		if (Debug.EnableInfo && hasDebug() &&
			m_stateCache.get(fstate.getPath()) != null)
			Debug.println(
				"***** addFileState() state=" + fstate.toString() +
				" - ALREADY IN CACHE *****");
	...
	/**
	 * Find the file state for the specified path, and optionally create a new
	 * file state if not found
	 * 
	 * @param path String
	 * @param create boolean
	 * @return FileState
	 */
	public final synchronized FileState findFileState(
	...
	/**
	 * Update the name that a file state is cached under, and the associated
	 * file state
	 * 
	 ...
	 * @return FileState
	 */
	public final synchronized FileState updateFileState(
		...
		// Rename the file state and add it back into the cache using the new
		// name
	...
	/**
	 * Rename a file state, remove the existing entry, update the path and add
	 * the state back into the cache using the new path.
	 * 
	 * @param newPath String
	 * @param state FileState
	 * @param isDir boolean
	 */
	public final void renameFileState(
		String newPath, FileState state, boolean isDir) {
		...
		synchronized (m_stateCache) {
			...
			// Remove the existing file state from the cache, using the original
			// name

			m_stateCache.remove(state.getPath());

			// Update the file state path and add it back to the cache using the
			// new name

			state.setPath(newPath);
			state.setFileStatus(isDir
				? FileStatus.DirectoryExists : FileStatus.FileExists);

			m_stateCache.put(state.getPath(), state);
		}

		// If the path is to a folder we must change the file status of all file
		// states that are using the old
		// path
		...
		if (isDir == true) {
			...
			while (enm.hasMoreElements()) {
				...
				if (statePath.length() > oldPath.length() &&
					statePath.startsWith(oldPath)) {

					// Get the associated file state, update and put back into
					// the cache

					FileState renState =
						(FileState) m_stateCache.remove(statePath);
		...
		while (enm.hasMoreElements()) {
			...
			if (state != null && state.hasNoTimeout() == false) {
				...
				synchronized (state) {
					...
					// Check if the file state has expired and there are no open
					...
					if (state.hasExpired(curTime) && state.getOpenCount() == 0) {
						...
						if (m_stateListener == null ||
							m_stateListener.fileStateExpired(state) == true) {
	...
	/**
	 * Set the file state factory class to be used to create new FileState
	 * objects
	 * 
	...
	public final void Dump(PrintStream out, boolean dumpAttribs) {
		...
		while (enm.hasMoreElements()) {
			...
			out.println(
				"++  " + fname + "(" +
				state.getSecondsToExpire(curTime) + ") : " + state.toString());
