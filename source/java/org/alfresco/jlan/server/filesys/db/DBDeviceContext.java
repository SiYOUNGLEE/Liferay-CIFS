...
public class DBDeviceContext extends DiskDeviceContext {
	...
	//
	// The database interface implementation may also provide the file loader
	...
	protected final void initialize(ConfigElement args)
		...
		if (dbParams != null) {
			...
			if (dbClass == null)
				throw new DeviceContextException(
					"Database interface class not specified");
			...
			try {
				...
				if (dbObj instanceof DBInterface) {
					...
					throw new DeviceContextException(
						"Database interface class is not an instance of " +
						"DBInterface");
			}
			catch (Exception ex) {
				throw new DeviceContextException(
					"Database interface error, " + ex.toString());
			}
		}
		else
			throw new DeviceContextException(
				"Invalid Database Interface configuration");
		...
		if (ldrParams != null) {
			...
			if (ldrClass == null || ldrClass.getValue().length() == 0)
				throw new DeviceContextException(
					"Invalid File Loader configuration");
		...
		if (nameVal != null) {
			...
			if (cacheCheck != null) {
				...
				try {
					...
					// Convert the cache check interval string to a numeric
					// value

					m_cacheCheckInterval =
						Long.valueOf(cacheCheck).longValue() * 1000L;
		...
		if (nameVal != null) {
			try {
				...
				if (m_offlineFileSize < 0)
					throw new DeviceContextException(
						"Invalid offline files size, " + nameVal.getValue());
		...
		if (nameVal != null) {
			...
			try {
				...
				if (m_maxFileSize < MinimumMaxfileSize)
					throw new DeviceContextException(
						"Maximum file size is below minimum allowed setting (" +
							MinimumMaxfileSize / MemorySize.KILOBYTE + "K)");
			}
			catch (NumberFormatException ex) {
				throw new DeviceContextException(
					"Invalid maximum file size value, " + nameVal.getValue());
			}
		}
		...
		if (args.getChild("QuotaManagement") != null) {
			...
			boolean quotaDebug =
				args.getChild("QuotaDebug") != null ? true : false;
		...
		if (nameVal != null) {
			try {
				...
				if (retainDays < 0 || retainDays > 3650)
					throw new DeviceContextException(
						"RetentionPeriod out of valid range (0 - 3650)");

				// Convert the retention period to a milliseconds interval that
				// can be added to the current date/time

				m_retentionPeriod = retainDays * MillisecondsPerDay;
			}
			catch (NumberFormatException ex) {
				throw new DeviceContextException(
					"RetentionPeriod is invalid, " + nameVal.getValue());
			}
		}

		// Check if the database interface supports retention, if retention has
		// been enabled

		if (hasRetentionPeriod() &&
			getDBInterface().supportsFeature(
				DBInterface.FeatureRetention) == false)
			throw new DeviceContextException(
				"Database interface does not support retention");

		// Create the file loader instance and get the database features
		...
		if (m_loaderClass != null) {
			...
			try {
				...
				m_loader =
					(FileLoader) Class.forName(m_loaderClass).newInstance();

				// Get the database features that the database interface must
				// implement to support this file loader
		...
		if (hasNTFSStreamsEnabled() == true &&
			m_loader.supportsStreams() == true)
		...
		try {
			...
			throw new DeviceContextException(
				"Failed to enable database features, " + ex.getMessage());
		}
		...
		try {
			...
			throw new DeviceContextException(
				"Database interface initialization failure, " + ex.toString());
		}
		...
		// Initialize the file loader, if it is a seperate class from the
		// database interface
	 ...
	 */
	public synchronized final FileRequestQueue getOfflineFileDeletes(
		boolean clearList) {
	 ...
	 */
	public synchronized final void addOfflineFileDelete(
	...
	public boolean fileStateExpired(FileState state) {
		...
		String tempName =
			(String) state.findAttribute(DBNetworkFile.DBCacheFile);
		...
		if (tempName != null) {
			...
			if (state.getOpenCount() > 0) {
				...
				// Do not expire the file state yet as the temporary file is in
				// use
				...
				if (m_debug)
					Debug.println(
						"$$ Deleted temporary file " + tempName +
						" (Expired) $$");
			}
		}

		// File is not open or does not have a temporary file, file state can be
		// expired
	...
	public void CloseContext() {
		...
		// Check if the file state cache is enabled, if so then release the file
