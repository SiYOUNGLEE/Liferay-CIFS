...
/**
 * JDBC Quota Manager Class <p>Filesystem quota management implementation that
...
public class DBQuotaManager implements QuotaManager {
	 ...
	 * @throws IOException
	 */
	public synchronized long allocateSpace(
		SrvSession sess, TreeConnection tree, NetworkFile file, long alloc)
		throws IOException {
		...
		if (m_freeSpace > alloc && alloc > 0) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"JDBCQuotaManager: Allocate=" + alloc +
					", free=" + MemorySize.asScaledString(m_freeSpace));
		}
		else
	...
	public synchronized void releaseSpace(
		SrvSession sess, TreeConnection tree, int fid, String path, long alloc)
		throws IOException {
		...
		if (alloc > 0) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"JDBCQuotaManager: Release=" + alloc + ", free=" +
					MemorySize.asScaledString(m_freeSpace));
		}
	}
	...
	public void startManager(DiskInterface disk, DiskDeviceContext ctx)
		...
		if (m_diskInfo == null)
			throw new QuotaManagerException(
				"Disk size information not available");
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"JDBCQuotaManager: Startup totalSpace=" +
				MemorySize.asScaledString(m_totalSpace) + ", freeSpace=" +
				MemorySize.asScaledString(m_freeSpace));
	}
	...
	public long getUserFreeSpace(SrvSession sess, TreeConnection tree) {
		...
		// All free space is available to all users with this quota manager
		// implementation
