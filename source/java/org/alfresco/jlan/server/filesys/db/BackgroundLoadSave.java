...
/**
 * Background Load Save Class <p>Utility class that can be used by FileLoader or
 * DBInterface implementations to provide a worker thread pool to load/save the
 * file data using a queue of file load/save requests.
 * 
 * @author gkspencer
 */
public class BackgroundLoadSave {
	...
	// Minimum number of requests that must be in the in-memory queue when a
	// requeue occurs to continue
	// without sleeping
	...
	// Associated file loader called by the worker threads to do the actual file
	// load/save
	...
	protected class ThreadWorker implements Runnable {
		 ...
		 */
		public ThreadWorker(
			String name, int id, FileRequestQueue queue, QueueLoader loader) {
			mi_id = id;
			mi_queue = queue;
			mi_loader = loader;
		...
		public void run() {
			...
			while (mi_shutdown == false) {
				...
				if (fileReq != null) {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"BackgroundLoadSave loader=" + getName() +
							", fileReq=" + fileReq + ", queued=" +
							mi_queue.numberOfRequests());

					// Check if the in-memory file request queue has reached the
					// low water mark, if so then wakeup the queue loader
					// thread to load more file request records from the
					// database
					...
					try {
						...
						// Set the thread id of the worker processing the
						// request
						...
						else if (fileReq.isType() == FileRequest.SAVE ||
					...
					if (reqSts == StsSuccess || reqSts == StsError) {
						...
						try {
							...
							if (Debug.EnableError && hasDebug())
								Debug.println(
									"BackgroundLoadSave Error: " +
									ex.toString());
						}

						// Reset the associated file state(s) to expire in a
						// short while
						...
						if (fileReq instanceof MultipleFileRequest) {
							...
							MultipleFileRequest multiReq =
								(MultipleFileRequest) fileReq;
							...
							// Reset all the associated file states to expire in
							// a short while

							for (int i = 0; i < multiReq.getNumberOfFiles(); 
								i++) {
							...
							// Reset the associated file state to expire in a
							// short while

							SingleFileRequest singleReq =
								(SingleFileRequest) fileReq;
							if (singleReq.hasFileState())
								singleReq.getFileState().setExpiryTime(
									System.currentTimeMillis() +
										RequestProcessedExpire);
						}
						...
						if (Debug.EnableInfo && reqSts == StsError &&
							hasDebug())
							Debug.println(
								"BackgroundLoadSave Error request=" + fileReq);
					}
					...
					// If the file request was not processed requeue it
						...
						if (Debug.EnableInfo && hasDebug())
							Debug.println(
								"BackgroundLoadSave ReQueue request=" +
								fileReq);

						// Check if we need to pause, if there are only a few
						// requests in the in-memory queue then we will
						// sleep for a short while so as not to process the
						// requeued request again too quickly
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"BackgroundLoadSave thread=" +
					mi_thread.getName() + " shutdown");
		}
	};
	...
	protected class QueueLoader implements Runnable {
		...
		// Flag to indicate new records have been added to the database since
		// last load
		 ...
		 */
		public QueueLoader(
			String name, int type, FileRequestQueue queue, Object lock) {
			mi_loadType = type;
			mi_queue = queue;
			mi_lockObj = lock;
		...
		/**
		 * Notify the queue loader that a new record has been added to the
		 * database
		 * 
		 * @param seqNo int
		 */
		public final synchronized void notifyNewRecord(int seqNo) {
			...
			// Update the last queue request sequence number, if higher than the
			// current value

			if (seqNo > mi_lastQueuedSeq)
				mi_lastQueuedSeq = seqNo;

			// Check if the queue loader thread should be woken up to load new
			// records
		...
		/**
		 * Check the request queue status and wakeup the loader thread if
		 * required
		 */
		public final void checkRequestQueue() {

			// Check if there is no load active, the queue is below the low
			// water mark and there are
			// new records to load.
		...
		public void run() {
			...
			// Create a local queue for loading the file request records before
			// passing to the main queue
			...
			while (mi_shutdown == false) {
				...
				try {
					...
					// Wait for a queue load request by waiting on the lock
					// object

					if (mi_queue.numberOfRequests() > 0 ||
						mi_lastQueuedSeq == 0 ||
						mi_lastSeqNo >= mi_lastQueuedSeq) {
						synchronized (mi_lockObj) {
							mi_lockObj.wait();
						}
					}
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"BackgroundLoadSave Queue load requested, seqNo=" +
						mi_lastSeqNo);
				...
				try {
					...
					// Indicate that there is a load in progress, clear the
					// temporary queue
					...
					int recCnt =
						getMaximumQueueSize() - mi_queue.numberOfRequests();

					// Load a block of file request records

					loadCnt =
						getDBQueueInterface().loadFileRequests(
							mi_lastSeqNo, mi_loadType, tempQueue, recCnt);
					...
					if (loadCnt > 0) {
						...
						while (tempQueue.numberOfRequests() > 0) {
							...
							int fsts =
								fileReq.isType() == FileRequest.LOAD
									? FileSegmentInfo.LoadWait
									: FileSegmentInfo.SaveWait;

							// Recreate the file state and associated data for
							// the file
							...
							try {
								fstate =
									createFileStateForRequest(
										fileReq.getFileId(),
										fileReq.getTemporaryFile(),
										fileReq.getVirtualPath(), fsts);
						...
						// If we loaded less records than expected clear the new
						// record flag
						...
						if (Debug.EnableInfo && hasDebug())
							Debug.println(
								"BackgroundLoadSave Loaded " +
								loadCnt +
								" records from queue db, type " +
								(mi_loadType == FileRequest.LOAD
									? "Read" : "Write"));
					}
				}
				catch (DBException ex) {
	...
	protected class TransactionQueueLoader implements Runnable {
		...
		/**
		 * Add a transaction to the queue, wakeup the transaction loader if
		 * required
		 * 
		 * @param name String
		 */
		...
		public void run() {
			...
			while (mi_shutdown == false) {
				...
				try {
					...
					// Wait for a transaction load request by waiting on the
					// transaction name queue
				...
				while (mi_transList.size() > 0) {
					...
					if (Debug.EnableInfo && mi_transList.size() > 0)
						Debug.println(
							"TransQueueLoader Processing tranName=" +
							tranName + ", queued=" + mi_transList.size());
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"BackgroundLoadSave Transaction load requested, " +
							"tran=" + tranId);
					...
					try {
						...
						// Create a multi-file request to hold the details of
						// all the files in the transaction

						MultipleFileRequest fileReq =
							new MultipleFileRequest(
								FileRequest.TRANSSAVE, tranId);

						// Load the file list for the transaction

						CachedFileInfo finfo = null;

						if (getDBQueueInterface().loadTransactionRequest(
							fileReq) != null) {

							// Recreate the file state and associated data for
							// each of the files in the transaction

							FileState fstate = null;

							for (int i = 0; i < fileReq.getNumberOfFiles(); 
								i++) {

								// Get the current file information from the
								// transaction
							...
							if (Debug.EnableInfo && hasDebug())
								Debug.println(
									"BackgroundLoadSave Loaded transaction " +
									fileReq);
						}
					}
					catch (DBException ex) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"BackgroundLoadSave Transaction queue loader shutdown");
		}
	};
	 ...
	 */
	public BackgroundLoadSave(
		DBQueueInterface dbQueue, FileStateCache stateCache,
	 ...
	 */
	public BackgroundLoadSave(
		String name, DBQueueInterface dbQueue, FileStateCache stateCache,
		BackgroundFileLoader bgLoader) {
	...
	public final void startThreads(int recCnt) {
		...
		m_readLoader = new QueueLoader(
			"ReadQueueLoader", FileRequest.LOAD, m_readQueue,
			m_readLoaderLock);
		m_writeLoader = new QueueLoader(
			"WriteQueueLoader", FileRequest.SAVE, m_writeQueue,
			m_writeLoaderLock);
		...
		for (int i = 0; i < m_readWorkers; i++)
			m_readThreads[i] = new ThreadWorker(
				getName() + "_RD_" + (i + 1), i, m_readQueue, m_readLoader);
		...
		for (int i = 0; i < m_writeWorkers; i++)
			m_writeThreads[i] = new ThreadWorker(
				getName() + "_WR_" + (i + 1), i, m_writeQueue, m_writeLoader);

		// DEBUG

		if (Debug.EnableInfo && hasDebug())
			Debug.println("FileLoader threadPool read=" + m_readWorkers +
				", write=" + m_writeWorkers);

		// If there are recovered files queued for saving then kick the queue
		// loader
	...
	public final void enableTransactions() {
		...
		if (m_tranLoader == null)
			m_tranLoader = new TransactionQueueLoader(
				getName() + "_TranLdr", m_writeQueue);
	}
		...
		if (req instanceof SingleFileRequest) {
			...
			try {
				...
				if (fileReq.isTransaction() == false) {
					...
					// Check if the in-memory queue is empty, if so then wakeup
					// the queue loader to load the new request

					if (fileReq.isType() == FileRequest.LOAD)
						m_readLoader.notifyNewRecord(
							fileReq.getSequenceNumber());
					else
						m_writeLoader.notifyNewRecord(
							fileReq.getSequenceNumber());
				}
				else {

					//
					// Check if this request is the last file in the current
					// transaction, if so then the transaction
					// is ready to be processed

					if (fileReq.isLastTransactionFile())
						m_tranLoader.addTransaction("" +
							fileReq.getTransactionId());
				}
			}
			catch (DBException ex) {
		...
		if (state == null) {
			...
			synchronized (state) {
				...
				FileSegmentInfo fileSegInfo =
					(FileSegmentInfo) state.findAttribute(DBFileSegmentInfo);
