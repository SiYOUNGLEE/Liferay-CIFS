...
/**
 * Cached Data Network File Class <p>Caches the file data in the local
 * filesystem in a temporary area.
 * 
 * @author gkspencer
 */
public class CachedNetworkFile extends DBNetworkFile {
	 ...
	 */
	public CachedNetworkFile(
		String name, int fid, int stid, int did, FileState state,
		FileSegment segment, FileLoader loader) {
	...
	public void openFile(boolean createFlag)
		...
		try {
			...
			if (m_cacheFile.fileExists() == false) {
				...
				if (DEBUG)
					Debug.println(
						"CachedNetworkFile.openFile() created " +
						m_cacheFile.getTemporaryFile());
			}
			...
			if (DEBUG) {
				Debug.println(
					"openFile() FAILED name=" + getFullName() +
					", error=" + ex.toString());
	...
	public int readFile(byte[] buf, int len, int pos, long fileOff)
		...
		if (DEBUG)
			Debug.println(
				"CachedNetworkFile.readFile() offset=" + fileOff +
				", len=" + len);
		...
		if (m_lastReadPos != -1L && fileOff == 
			(m_lastReadPos + m_lastReadLen)) {
		...
		if (m_cacheFile.hasStatus() == FileSegmentInfo.Initial &&
			m_cacheFile.isQueued() == false) {
			...
			synchronized (getFileState()) {
				...
				if (m_cacheFile.isQueued() == false)
					getLoader().queueFileRequest(
						createFileRequest(FileRequest.LOAD));
			}
		}
		...
		if (m_cacheFile.isDataAvailable()) {
			...
			if (DEBUG)
				Debug.println(
					"CachedNetworkFile DataAvailable Read, file=" +
					getName() + ", fid=" + getFileId());
			...
			if (rdlen <= 0) {
				...
				if (DEBUG) {
					Debug.println(
						"CachedNetworkFile.readFile() name=" +
						getFullName());
		...
		// Wait for the required amount of data to be written to the temporary
		// file
		...
		while (readDone == false && waitTime < DataLoadWaitTime &&
			...
			// Check if there is enough data available to satisfy the read
			// request
			...
			if (dataAvailable == false) {
				...
				// File loader thread is still loading the file data, check the
				// file length to see if
				// there is enough data to satisfy the read request. Check that
				// there is more data available
				// than required as the loader may have extended the file and
				// still be writing the last block
				// of data.
			...
			if (rdlen > 0) {
				...
				try {
					...
					// Wait for the file data to be loaded, or for large files
					// the wait may timeout and we can check
					// if there is enough data available to satisfy the current
					// read request.
					...
					if (DEBUG)
						Debug.println(
							"CachedNetworkFile waited " +
							(endTime - startTime) + "ms for data, available=" +
				...
				if (DEBUG) {
					Debug.println(
						"m_cacheFile=" + m_cacheFile + ", rdlen=" + rdlen);
					Debug.println("fstate=" + getFileState());
				}
			}
		}
		...
		if (DEBUG)
			Debug.println(
				"CachedNetworkFile.readFile() Waited " + waitTime +
				"ms for data");
		...
		if (readDone == true) {
			...
			if (DEBUG) {
				Debug.println(
					"ReadFault fname=" + getFullName() + ", fid=" +
					getFileId());
	...
	protected final void updateFileSize(long siz, long alloc) {
		...
		if (hasFileState()) {
			...
			FileInfo finfo =
				(FileInfo) getFileState().findAttribute(
	...
	/**
	 * Create a file load or save request. This method may be overridden to
	 * allow extending of the SingleFileRequest class.
	 * 
	 * @param typ int
	 * @return FileRequest
	 */
	protected FileRequest createFileRequest(int typ) {
		...
		return new SingleFileRequest(
			typ, getFileId(), getStreamId(), m_cacheFile.getInfo(),
			getFullNameStream(), getFileState());
	}
