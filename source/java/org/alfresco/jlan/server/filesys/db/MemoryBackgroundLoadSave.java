...
public class MemoryBackgroundLoadSave {
	...
	protected class ThreadWorker implements Runnable {
		...
		public void run() {
						...
						if (fileReq instanceof MultipleFileRequest) {
							...
							for (int i = 0; i < multiReq.getNumberOfFiles(); 
								...
								if (finfo.hasFileState())
									finfo.getFileState().setExpiryTime(
										expireAt);
							}
						}
						else {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"BackgroundLoadSave thread=" + mi_thread.getName() + 
					" shutdown");
		}
	};
	 ...
	 */
	public MemoryBackgroundLoadSave(
		FileStateCache stateCache, BackgroundFileLoader bgLoader) {
	 ...
	 */
	public MemoryBackgroundLoadSave(
		String name, FileStateCache stateCache, BackgroundFileLoader bgLoader) {
	...
	public final void startThreads() {
		...
		for (int i = 0; i < m_readWorkers; i++)
			m_readThreads[i] =
				new ThreadWorker(getName() + "_RD_" + (i + 1), i, m_readQueue);
		...
		for (int i = 0; i < m_writeWorkers; i++)
			m_writeThreads[i] =
				new ThreadWorker(getName() + "_WR_" + (i + 1), i, m_writeQueue);
	...
	public void queueFileRequest(FileRequest req) {
		...
		if (req instanceof SingleFileRequest) {
			...
			if (fileReq.hasFileState()) {
				...
				// Lock the file state so it does not get expired during the
				// load/save
