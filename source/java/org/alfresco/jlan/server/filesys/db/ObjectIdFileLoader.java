...
/**
 * Object Id File Loader Class <p>The object id file loader loads/saves file
 * data to a repository/storage device using an associated object id. <p>This
 * class relies on a seperate DBObjectIdInterface implementation to provide the
 * methods to load and save the file id/object id mappings to the database
 * table.
 * 
 * @author gkspencer
 */
public abstract class ObjectIdFileLoader
	implements FileLoader, BackgroundFileLoader, FileStateListener {
	...
	// Database object id interface used to load/save the file id/object id
	// mappings
	...
	public NetworkFile openFile(
		FileOpenParams params, int fid, int stid, int did, boolean create,
		boolean dir)
		...
		FileState fstate = m_stateCache.findFileState(
			params.getFullPath(), true);
		fstate.setExpiryTime(
			System.currentTimeMillis() + getContext().getCacheTimeout());
		...
		if (dir == false) {
			...
			CachedNetworkFile cacheFile = createNetworkFile(
				fstate, params, name, fid, stid, did);
			netFile = cacheFile;
			...
			if (create == true || params.isOverwrite() == true) {
				...
				// or the existing file is being overwritten
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"ObjIdLoader Queued file load, SEQUENTIAL access");
			}
		}
		else {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"ObjIdLoader.openFile() DIR name=" + name +
					", state=" + fstate);
		}
	...
	public void closeFile(SrvSession sess, NetworkFile netFile)
		...
		if (netFile instanceof CachedNetworkFile) {
			...
			if (fileSeg.isUpdated() && netFile.hasDeleteOnClose() == false) {
				...
				// repository, if not already queued
				...
				if (fileSeg.isSaveQueued() == false) {
					...
					SingleFileRequest fileReq = new SingleFileRequest(
						FileRequest.SAVE, cacheFile.getFileId(),
						cacheFile.getStreamId(), fileSeg.getInfo(),
						netFile.getFullNameStream(),
						cacheFile.getFileState());

					// Check if there are any attributes to be added to the file
					// request
					...
					if (hasRequiredAttributes() && sess != null) {
						...
						if (m_reqAttributes.containsString(
							FileRequest.AttrUserName)) {
							...
							fileReq.addAttribute(new NameValue(
								FileRequest.AttrUserName, userName));
						}

						// Check if the protocol type is required

						if (m_reqAttributes.containsString(
							FileRequest.AttrProtocol)) {
					...
					Debug.println(
						"## FileLoader Save already queued for " + fileSeg);
				}
			}

			// Update the cache timeout for the temporary file if there are no
			// references to the file. If the file was
			// opened for sequential access only it will be expired quicker.
				...
				// delete it from the temporary area sooner
				...
				// deleted when the file state expires (if there
				// are still no references to the file).

				cacheFile.getFileState().setExpiryTime(tmo);
			}

			// If the database is not online and the file is marked for delete
			// then queue a delete file request to do the
			...
			if (m_dbCtx.isAvailable() == false && netFile.hasDeleteOnClose()) {
				...
				DeleteFileRequest deleteReq = new DeleteFileRequest(
					cacheFile.getFileId(), cacheFile.getStreamId(),
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"## FileLoader queued offline delete, " + deleteReq);
			}
		}
	}
		...
		try {
			...
			if (fstate != null) {
				...
				FileSegmentInfo fileSegInfo =
					(FileSegmentInfo) fstate.removeAttribute(DBFileSegmentInfo);
				if (fileSegInfo != null) {
					try {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"## ObjIdLoader deleteFile() error, " + ex.toString());
		}
	}
	...
	public int loadFile(FileRequest req)
		...
		try {
			...
			if (Debug.EnableInfo && hasDebug()) {
				...
				Debug.println(
					...
					loadReq.getFileId() + ", stream=" + loadReq.getStreamId() +
		...
		// Clear the last modified date/time of the temporary file to indicate
		...
		if (loadSts == StsSuccess) {
			...
			runFileLoadedProcessors(
				getContext(), loadReq.getFileState(), fileSeg);
		}
	...
	public int storeFile(FileRequest req)
		...
		try {
			...
			String objectId = saveFileData(
				saveReq.getFileId(), saveReq.getStreamId(), fileSeg,
				req.getAttributes());

			// Save the object id to the mapping database

			getDBObjectIdInterface().saveObjectId(
				saveReq.getFileId(), saveReq.getStreamId(), objectId);
	...
	public abstract void loadFileData(
		int fileId, int streamId, String objectId, FileSegment fileSeg)
		throws IOException;
	 ...
	 * @throws IOException
	 */
	public abstract String saveFileData(
		int fileId, int streamId, FileSegment fileSeg, NameValueList attrs)
		throws IOException;
	...
	public void initializeLoader(ConfigElement params, DeviceContext ctx)
		...
		if (nameVal != null) {
			try {
				...
				throw new FileLoaderException(
					"ObjIdLoader Invalid ThreadPoolSize value, " +
						ex.toString());
			}
		}
		...
		if (m_tempDir.exists() == false || m_tempDir.isDirectory() == false)
			throw new FileLoaderException(
				"FileLoader TempDirectory does not exist, or is not a " +
				"directory, " + m_tempDirName);

		if (m_tempDir.canWrite() == false)
			throw new FileLoaderException(
				"FileLoader TempDirectory is not writeable, " + m_tempDirName);
		...
		if (maxFiles != null) {
			try {
				...
				if (m_tempMax < 10 || m_tempMax > 20000)
					throw new FileLoaderException(
						"FileLoader MaximumFilesPerDirectory out of valid " +
						"range (10-20000)");
			}
			catch (NumberFormatException ex) {
				throw new FileLoaderException(
		...
		if (fileProcs != null) {
			...
			for (ConfigElement className : elems) {
				...
				if (className == null || className.getValue() == null ||
				...
				try {
					...
					Object procObj =
						Class.forName(className.getValue()).newInstance();
					...
					throw new FileLoaderException(
						"File processor exception, " + ex.toString());
				}
		...
		if (ctx instanceof DBDeviceContext) {
			...
			// Check if the object id feature is supported by the database
			// interface

			if (getContext().getDBInterface().supportsFeature(
				DBInterface.FeatureObjectId) == false)
				throw new FileLoaderException(
					"DBLoader requires data support in database interface");

			if (getContext().getDBInterface() instanceof DBObjectIdInterface)
				m_dbObjectIdInterface =
					(DBObjectIdInterface) getContext().getDBInterface();
			else
				throw new FileLoaderException(
					"Database interface does not implement object id " +
					"interface");
		}
		else
		...
		// check the temporary cache area and delete
		...
		try {
			...
			recoveredQueue = dbQueue.performQueueCleanup(
				m_tempDir, TempDirPrefix, TempFilePrefix, JarFilePrefix);
			...
			if (recoveredQueue != null && Debug.EnableInfo && hasDebug())
				Debug.println(
					...
					recoveredQueue.numberOfRequests() + " pending save files");
		}
		catch (DBException ex) {
		...
		m_backgroundLoader = new BackgroundLoadSave(
			"DBLdr", dbQueue, m_stateCache, this);
		...
		// Start the file loader threads, start the request loading if there are
		// pending file save requests
	 ...
	 */
	protected final void runFileStoreProcessors(
	...
	protected final FileState createFileStateForRequest(
		...
		synchronized (state) {
			...
			// Prevent the file state from expiring whilst the request is queued
			// against it
	...
	private final void createNewTempDirectory() {
		...
		m_curTempName =
			m_tempDirName + getTempDirectoryPrefix() + m_curTempIdx++;
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"ObjIdLoader Created new temp directory - " + m_curTempName);
	}
		...
		if (segInfo != null) {
			...
			if (segInfo.isQueued() == false) {
				...
				try {
					...
					// that the data may be loaded again
					// if required.
					...
					if (segInfo.hasStatus() != FileSegmentInfo.Initial) {
						...
						// empty, and it is not the current temporary
						// sub-directory

						if (segInfo.getTemporaryFile().startsWith(
							m_curTempName) == false) {

							// Check if the sub-directory is empty

							File tempFile =
								new File(segInfo.getTemporaryFile());
	...
	public void fileStateClosed(FileState state) {
		...
		if (state == null) {
			Debug.println(
				"%%%%% FileLoader.fileStateClosed() state=NULL %%%%%");
			return;
		}
	...
	private final CachedNetworkFile createNetworkFile(
		FileState state, FileOpenParams params, String fname, int fid,
		int stid, int did)
		throws IOException {

		// The file state is used to synchronize the creation of the file
		// segment as there may be other
		// sessions opening the file at the same time. We have to be careful
		// that only one thread creates the
		// file segment.
		...
		synchronized (state) {
			...
			if (fileSegInfo == null) {
				...
				fileSeg =
					FileSegment.createSegment(
						fileSegInfo, tempName.toString(), m_curTempDir,
				...
				// segment state to indicate it is available
				...
				fileSeg =
					new FileSegment(
						fileSegInfo, params.isReadOnlyAccess() == false);
			...
			netFile =
				new CachedNetworkFile(
					fname, fid, stid, did, state, fileSeg, this);

			netFile.setGrantedAccess(params.isReadOnlyAccess()
				? NetworkFile.READONLY : NetworkFile.READWRITE);
