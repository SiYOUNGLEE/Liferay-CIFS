...
public class DBDiskDriver
	implements DiskInterface, DiskSizeInterface, DiskVolumeInterface,
	NTFSStreamsInterface, FileLockingInterface, FileIdInterface,
	...
	public void closeFile(
		...
		if (file instanceof DBNetworkFile) {
			...
			if (fstate != null) {
				...
				DBFileInfo finfo =
					(DBFileInfo) fstate.findAttribute(
						FileState.FileInformation);
				if (finfo != null && file.getWriteCount() > 0) {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  File size=" + jdbcFile.getFileSize() +
		...
		if (file.getGrantedAccess() != NetworkFile.READONLY &&
			...
			if (file.hasCreationDate() &&
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"Close file using creation date/time for "
						+ "modified date/time");
			}
			...
			try {
				...
				dbCtx.getDBInterface().setFileInformation(
					file.getDirectoryId(), file.getFileId(), finfo);
			}
	...
	public void createDirectory(
		...
		try {
			...
			int fid =
				dbCtx.getDBInterface().createFileRecord(
					dname, dirId, params, retain);
			...
			if (dbCtx.hasRetentionPeriod() && retain == true) {
				RetentionDetails retDetails =
					dbCtx.getDBInterface().getFileRetentionDetails(dirId, fid);
				if (retDetails != null)
			...
			if (fid != -1 && dbCtx.getFileLoader() instanceof NamedFileLoader) {
				...
				NamedFileLoader namedLoader =
					(NamedFileLoader) dbCtx.getFileLoader();
	...
	public NetworkFile createFile(
		...
		try {
			...
			file = (DBNetworkFile) dbCtx.getFileLoader().openFile(
				params, fid, 0, dirId, true, false);
	...
	public void deleteDirectory(
		...
		try {
			...
			dbCtx.getDBInterface().deleteFileRecord(
				dinfo.getDirectoryId(), dinfo.getFileId(),
	...
	public void deleteFile(SrvSession sess, TreeConnection tree, String name)
		...
		try {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"DBDiskDriver deleteFile() name=" + name +
					", state=" + fstate);
			...
			if (dbInfo.isFileType() == FileType.SymbolicLink)
				dbCtx.getDBInterface().deleteSymbolicLinkRecord(
					dbInfo.getDirectoryId(), dbInfo.getFileId());
			...
			if (dbCtx.hasQuotaManager()) {
				...
				dbCtx.getQuotaManager().releaseSpace(
					sess, tree, dbInfo.getFileId(), null, dbInfo.getSize());
			}
		}
		catch (DBException ex) {
			Debug.println(ex);
	...
	public int fileExists(SrvSession sess, TreeConnection tree, String name) {
		...
		if (FileName.containsStreamName(name)) {
			...
			if (fstate != null && fstate.fileExists() == true) {
				...
				if (dbInfo != null) {
					...
					StreamInfoList streams =
						(StreamInfoList) fstate.findAttribute(DBStreamList);
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"DB fileExists() name=" + filePath + ", stream=" +
					paths[2] + ", fileSts=" + FileStatus.asString(fileSts));
		}
		else {
			...
			if (fstate.getFileStatus() == FileStatus.Unknown) {
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"@@ Cache hit - fileExists() name=" + name +
						", fileSts=" + FileStatus.asString(fileSts));
			}
		}
	...
	public FileInfo getFileInformation(
		...
		if (FileName.containsStreamName(name)) {
			...
			if (fstate != null) {
				...
				finfo =
					(FileInfo) fstate.findAttribute(FileState.FileInformation);
			}
			...
			if (finfo == null) {
				...
				// Split the path into directory, file and stream name
				// components
				...
				if (parent != null && parent.fileExists() == true) {
					...
					if (dbInfo != null) {
						...
						StreamInfoList streams =
							loadStreamList(parent, dbInfo, dbCtx, true);
						...
						if (streams != null && streams.numberOfStreams() > 0) {
							...
							if (sInfo != null) {
								...
								finfo.setCreationDateTime(
									dbInfo.getCreationDateTime());
								finfo.setAccessDateTime(
									dbInfo.getAccessDateTime());
								finfo.setModifyDateTime(
									dbInfo.getModifyDateTime());

								// Attach to the file state

								fstate.addAttribute(
									FileState.FileInformation, finfo);
	...
	public NetworkFile openFile(
		...
		if (dbCtx.hasRetentionPeriod()) {
			try {
				...
				RetentionDetails retDetails =
					dbCtx.getDBInterface().getFileRetentionDetails(
						finfo.getDirectoryId(), finfo.getFileId());
				if (retDetails != null)
					fstate.setRetentionExpiryDateTime(retDetails.getEndTime());
			}
			catch (DBException ex) {
				throw new AccessDeniedException(
					"Retention error, " + ex.getMessage());
			}
		}
		...
		DBNetworkFile jdbcFile = (DBNetworkFile) dbCtx.getFileLoader().openFile(
			params, finfo.getFileId(), 0, finfo.getDirectoryId(), false,
			finfo.isDirectory());
		...
		fstate.setFileStatus(finfo.isDirectory()
			? FileStatus.DirectoryExists : FileStatus.FileExists);
		fstate.incrementOpenCount();
	...
	public int readFile(
		...
		if (file instanceof DBNetworkFile) {
			...
			if (jfile.hasFileState() && jfile.getFileState().hasActiveLocks()) {
				...
				if (jfile.getFileState().canReadFile(
					pos, siz, sess.getProcessId()) == false)
	...
	public void renameFile(
		...
		try {
			...
			if (fname != null && fname.length() > MaxFileNameLen)
				throw new FileNameException(
					"Destination name too long, " + fname);
			...
			if (newInfo != null)
				throw new FileExistsException(
					"Rename to file/folder already exists," + newName);

			// Check if the loader handles rename requests, an exception may be
			...
			if (dbCtx.getFileLoader() instanceof NamedFileLoader) {
				...
				NamedFileLoader namedLoader =
					(NamedFileLoader) dbCtx.getFileLoader();
				namedLoader.renameFileDirectory(
					oldName, fid, newName, curInfo.isDirectory());
			}
			...
			// Rename the file/folder, this may also link the file/folder to a
			// new parent directory

			dbCtx.getDBInterface().renameFileRecord(
				dirId, fid, newFname, newDirId);

			// Update the file state with the new file name/path

			dbCtx.getStateCache().renameFileState(
				newName, fstate, curInfo.isDirectory());
	...
	public void setFileInformation(
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"DB setFileInformation() name=" + name + ", info=" +
				info.toString());
		...
		if (dbCtx.getDBInterface().isOnline() == false) {
			...
			if (info.getSetFileInformationFlags() == FileInfo
							.SetDeleteOnClose) {
		...
		try {
			...
			if (info.hasChangeDateTime() == false) {
				...
				if (info.hasSetFlag(FileInfo.SetChangeDate) == false)
					info.setFileInformationFlags(
						info.getSetFileInformationFlags() +
		...
		try {
			...
			if (WildCard.containsWildcards(searchPath) == false) {
				...
				if (searchState != null && searchState.fileExists() == true) {
					...
					DBFileInfo finfo = (DBFileInfo) searchState.findAttribute(
						FileState.FileInformation);
					...
					if (finfo != null) {
						...
						if (Debug.EnableInfo && hasDebug())
							Debug.println(
								"DB StartSearch using cached file " +
								"information, path=" +
			...
			if (search == null) {
				...
				DBSearchContext dbSearch =
					dbCtx.getDBInterface().startSearch(
	...
	public void truncateFile(
		...
		if (file instanceof DBNetworkFile) {
			...
			DBFileInfo dbInfo =
				getFileDetails(jfile.getFullName(), dbCtx, fstate);
			if (dbInfo == null)
			...
			try {
				...
				if (allocSize > 0 && quotaMgr != null)
					quotaMgr.releaseSpace(
						sess, tree, file.getFileId(), null, allocSize);
			...
			if (releaseSize > 0 && quotaMgr != null)
				quotaMgr.releaseSpace(
					sess, tree, file.getFileId(), null, releaseSize);
	...
	public int writeFile(
		...
		if (file instanceof DBNetworkFile) {
			...
			if (jfile.hasFileState() && jfile.getFileState().hasActiveLocks()) {
				...
				if (jfile.getFileState().canWriteFile(
					fileoff, siz, sess.getProcessId()) == false)
					throw new LockConflictException();
			}
			...
			if (quotaMgr != null) {
				...
				DBFileInfo finfo =
					getFileDetails(
						jfile.getFullName(), dbCtx, jfile.getFileState());
				if (finfo == null)
					throw new FileNotFoundException(jfile.getFullName());
				...
				try {
					...
					if (extendSize > 0 && quotaMgr != null)
						quotaMgr.releaseSpace(
							sess, tree, file.getFileId(), null, extendSize);
	 ...
	 * @return DBFileInfo
	 */
	protected final DBFileInfo getFileDetails(
		String path, DBDeviceContext dbCtx) {
	 ...
	 */
	protected final DBFileInfo getFileDetails(
		String path, DBDeviceContext dbCtx, FileState fstate) {
		...
		if (fstate != null) {
			...
			DBFileInfo finfo =
				(DBFileInfo) fstate.findAttribute(FileState.FileInformation);
			if (finfo != null)
				return finfo;
		}
	...
	protected final int getFileId(
		String path, String name, int dirId, DBDeviceContext dbCtx) {
		...
		try {
			...
			fileId =
				dbCtx.getDBInterface().getFileId(dirId, name, false, false);
		}
		catch (DBException ex) {
		}
	 ...
	 */
	protected final void getRetentionDetailsForState(
		DBDeviceContext dbCtx, FileState fstate)
		throws DBException {
	 ...
	 * @return int
	 */
	protected final int findParentDirectoryId(
		DBDeviceContext ctx, String path, boolean filePath) {
		...
		if (cache != null) {
			...
			if (state != null && state.getFileId() != -1) {
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"@@ Cache hit - findParentDirectoryId() path=" +
						paths[0]);
	 ...
	 * @return int[]
	 */
	protected final int[] findParentDirectoryIdList(
		DBDeviceContext ctx, String path, boolean filePath) {
		...
		try {
			...
			while (idx <= endIdx) {
				...
				if (fstate != null && fstate.getFileId() != -1) {
					...
					parentId = dirId;
					dirId =
						ctx.getDBInterface().getFileId(
							dirId, curPath, true, true);
					...
					if (dirId != -1) {
						...
						if (fstate != null) {
							...
							fstate =
								cache.findFileState(pathStr.toString(), true);

							// Get the file information

							DBFileInfo finfo =
								ctx.getDBInterface().getFileInformation(
									parentId, dirId, DBInterface.FileAll);
							fstate.addAttribute(
								FileState.FileInformation, finfo);
							fstate.setFileStatus(finfo.isDirectory()
	...
	public DBFileInfo getFileInfo(
		String path, int dirId, int fid, DBDeviceContext dbCtx) {
		...
		if (state != null && state.getFileId() != -1) {
			...
			DBFileInfo finfo =
				(DBFileInfo) state.findAttribute(FileState.FileInformation);
			if (finfo != null)
				return finfo;
		}
		...
		try {
			...
			finfo =
				dbCtx.getDBInterface().getFileInformation(
					dirId, fid, DBInterface.FileAll);
		}
		catch (DBException ex) {
			Debug.println(ex);
			finfo = null;
		...
		if (state != null && finfo != null) {
			...
			state.setFileStatus(finfo.isDirectory()
				? FileStatus.DirectoryExists : FileStatus.FileExists);
		}
	 ...
	 * @return DBFileInfo
	 */
	public DBFileInfo getStreamInfo(
		FileState parent, String[] paths, DBDeviceContext dbCtx) {
		...
		if (state != null && state.getFileId() != -1) {
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"@@ Cache hit - getStreamInfo() path=" + streamPath);
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"DBDiskDriver getStreamInfo parent=" +
				parent.getPath() + ", stream=" + paths[2]);
		...
		try {
			...
			StreamInfoList sList =
				(StreamInfoList) parent.findAttribute(DBStreamList);
	 ...
	 * @return FileState
	 */
	protected final FileState getFileState(
		String path, DBDeviceContext ctx, boolean create) {
	 ...
	 * @exception IOException
	 */
	public final void getDiskInformation(
		DiskDeviceContext ctx, SrvDiskInfo info)
		throws IOException {
		...
		if (ctx instanceof DBDeviceContext) {
			...
			if (dbCtx.getFileLoader() instanceof DiskSizeInterface) {
				...
				DiskSizeInterface sizeInterface =
					(DiskSizeInterface) dbCtx.getFileLoader();
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"DBDiskDriver getDiskInformation() handed " +
						"to file loader");
			}
		}
	...
	public StreamInfoList getStreamList(
		...
		StreamInfo sinfo = new StreamInfo(
			"::$DATA", finfo.getFileId(), 0, finfo.getSize(),
	 ...
	 * @exception IOException
	 */
	protected final NetworkFile createStream(
		FileOpenParams params, FileState parent, DBDeviceContext dbCtx)
		throws IOException {
		...
		if (streamList.findStream(params.getStreamName()) != null)
			throw new FileExistsException(
				"Stream exists, " + params.getFullPath());
		...
		try {
			...
			StreamInfo stream =
				new StreamInfo(params.getStreamName(), file.getFileId(), stid);
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"createStream() file=" + params.getPath() +
	 ...
	 * @exception IOException
	 */
	protected final NetworkFile openStream(
		FileOpenParams params, FileState parent, DBDeviceContext dbCtx)
		throws IOException {
		...
		if (sInfo == null)
			throw new FileNotFoundException(
				"Stream does not exist, " + params.getFullPath());
		...
		DBFileInfo sfinfo =
			new DBFileInfo(
				sInfo.getName(), params.getFullPath(), finfo.getFileId(),
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println("DB openStream() file=" + params.getPath() +
				", stream=" + sInfo.getName());

		DBNetworkFile jdbcFile =
			(DBNetworkFile) dbCtx.getFileLoader().openFile(
				params, finfo.getFileId(), sInfo.getStreamId(),
	 ...
	 * @exception IOException
	 */
	protected final void closeStream(
		SrvSession sess, TreeConnection tree, NetworkFile stream)
		throws IOException {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"DB closeStream() file=" + stream.getFullName() +
			...
			if (fstate != null) {
				...
				if (finfo != null && stream.getWriteCount() > 0) {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"  Stream size=" +
							jdbcFile.getFileSize() + ", modifyDate=" +
							jdbcFile.getModifyDate());
				}
			}
		}
		else
		...
		if (stream.getGrantedAccess() != NetworkFile.READONLY &&
			...
			if (stream.hasCreationDate() &&
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"Close stream using creation date/time for" +
						" modified date/time");
			}
			...
			if (parent != null) {
				...
				StreamInfoList streamList =
					loadStreamList(parent, null, dbCtx, false);
				if (streamList != null) {
	 ...
	 * @exception IOException
	 */
	protected final void deleteStream(
		SrvSession sess, TreeConnection tree, String name)
		throws IOException {
		...
		try {
			...
			dbCtx.getFileLoader().deleteFile(
				name, sInfo.getFileId(), sInfo.getStreamId());

			// Delete the stream record

			dbCtx.getDBInterface().deleteStreamRecord(
				sInfo.getFileId(), sInfo.getStreamId(),
				dbCtx.isTrashCanEnabled());
	 ...
	 * @return StreamInfoList
	 */
	protected final StreamInfoList loadStreamList(
		FileState fstate, DBFileInfo finfo, DBDeviceContext dbCtx,
		boolean dbLoad) {

		// Check if the stream list has already been loaded

		StreamInfoList sList =
			(StreamInfoList) fstate.findAttribute(FileState.StreamsList);
		...
		if (sList == null && dbLoad == true) {
			...
			try {
				...
				sList =
					dbCtx.getDBInterface().getStreamsList(
	 ...
	 * @exception FileNotFoundException
	 */
	public String buildPathForFileId(
		SrvSession sess, TreeConnection tree, int dirid, int fileid)
		throws FileNotFoundException {
		...
		try {
			...
			do {
				...
				finfo =
					dbCtx.getDBInterface().getFileInformation(
						curDid, curFid, DBInterface.FileIds);
	 ...
	 * @return boolean
	 */
	public boolean hasSymbolicLinksEnabled(
		SrvSession sess, TreeConnection tree) {

		// Access the associated database interface to check if it supports
		// symbolic links

		DBDeviceContext dbCtx = (DBDeviceContext) tree.getContext();
		if (dbCtx.getDBInterface().supportsFeature(
	...
	public String readSymbolicLink(
		SrvSession sess, TreeConnection tree, String path)
