...
/**
 * JDBC Database Interface Abstract Class <p>Provides the standard variables for
 * a JDBC based database interface, including the parsing of the parameters in
 * the initialization method.
 * 
 * @author gkspencer
 */
public abstract class JdbcDBInterface
	implements DBInterface, DBConnectionPoolListener {
	...
	// Database table that contains the file system structure records, streams
	// information
	// records, data retention information records, file loader queue and file
	// loader transaction
	...
	// Retention period, milliseconds to add to current date/time value, or -1
	// if not enabled
	...
	// Data fragment size to store per BLOB when the file data is stored in the
	// database
	...
	/**
	 * Check if the database is online, return the database connection pool
	 * status
	 * 
	 * @return boolean
	 */
	public boolean isOnline() {
		...
		if (m_connPool != null) {
			...
			// Try and open a connection to the database to see if it's back
			// online
	...
	/**
	 * Return the data fragment size to store per BLOB when using the database
	...
	protected final void createConnectionPool()
		...
		m_connPool = new DBConnectionPool(
			m_driver, m_dsn, m_userName, m_password, m_dbInitConns,
			m_dbMaxConns);
	...
	/**
	 * Default implementation of the delete file request, throws an exception
	 * indicating that the feature is not implemented.
	 * 
	...
	/**
	 * Queue a file save request to the pending save queue as the database is
	 * offline
	 * 
	 * @param saveReq FileRequest
	 */
	protected synchronized final void queueOfflineSaveRequest(
		FileRequest saveReq) {
	...
	public void databaseOnlineStatus(boolean dbonline) {
		...
		if (hasDebug())
			Debug.println(
				"JDBCInterface: Database connection event, status=" +
				(dbonline ? "OnLine" : "OffLine"));

		// Set the shared device availabel status depending on the database
		// state

		m_dbCtx.setAvailable(dbonline);

		// If the database is back online then check if there are queued
		...
		if (dbonline == true) {
			...
			if (m_dbCtx.hasOfflineFileDeletes()) {
				...
				FileRequestQueue deleteList =
					m_dbCtx.getOfflineFileDeletes(true);
				...
				for (int i = 0; i < deleteList.numberOfRequests(); i++) {
					...
					DeleteFileRequest deleteReq =
						(DeleteFileRequest) deleteList.removeRequestNoWait();
					...
					try {
						...
						if (deleteReq.getStreamId() == 0)
							deleteFileRecord(
								-1, deleteReq.getFileId(),
								m_dbCtx.isTrashCanEnabled());
						else
							deleteStreamRecord(
								deleteReq.getFileId(), deleteReq.getStreamId(),
								m_dbCtx.isTrashCanEnabled());

						// Remove the file state for the file/stream

						m_dbCtx.getStateCache().removeFileState(
							deleteReq.getFileState().getPath());

						// DEBUG

						if (hasDebug())
							Debug.println(
								"JDBCInterface: Offline delete  of file " +
								deleteReq.getVirtualPath() +
								", fid=" +
								deleteReq.getFileId());
					}
					catch (Exception ex) {
			...
			if (m_pendingSaveRequests != null) {
				...
				if (hasDebug())
					Debug.println(
						"JDBCInterface: Requeueing pending save " +
						"requests, count=" +
						saveReqQueue.numberOfRequests());
				...
				while (saveReqQueue.numberOfRequests() > 0) {
					...
					// Get a request from the queue and requeue the request to
					// save the data
					...
					try {
						...
						if (hasDebug())
							Debug.println(
								"JDBCInterface: Requeued save " +
								fileReq);

						// Update the file size for the file being saved, the
						// close method may not have updated the final file size
						...
						if (fileReq instanceof SingleFileRequest) {
							...
							if (singleReq.hasFileState()) {
								...
								File tempFile =
									new File(singleReq.getTemporaryFile());

								// Create the set file information details to
								// set the file size

								FileInfo fInfo = new FileInfo("", 0L, 0);
								fInfo.setFileInformationFlags(
								...
								setFileInformation(
									-1, singleReq.getFileId(), fInfo);

								// DEBUG

								if (hasDebug())
									Debug.println(
										"JDBCInterface: Updated " +
										"file size for " +
										singleReq.getVirtualPath() +
										" size=" +
										tempFile.length());
							}
						}
						...
						// If there was an error the request will go back onto a
						// new in-memory pending queue
					}
				}
			}
		}
	...
	public final boolean hasOfflineFileRequests() {
		if (m_pendingSaveRequests != null &&
			m_pendingSaveRequests.numberOfRequests() > 0)
			return true;
		return false;
	}
	...
	/**
	 * Default implementation of the queued request check, throws an exception
	 * indicating that the feature is not implemented.
	 * 
	...
	/**
	 * Default implementation of load file requests, throws an exception
	 * indicating that the feature is not implemented.
	 * 
	 * @param seqNo int
	 * @param reqType int
	...
	public int loadFileRequests(
		int seqNo, int reqType, FileRequestQueue reqQueue, int recLimit)
		throws DBException {
	...
	/**
	 * Default implementation of queue file request, throws an exception
	 * indicating that the feature is not implemented.
	 * 
	 * @param fileReq FileRequest
	 * @throws DBException
	...
	/**
	 * Default implementation of the queue transaction request, throws an
	...
	/**
	 * Default implementation of load transaction request, throws an exception
	 * indicating that the feature is not implemented.
	 * 
