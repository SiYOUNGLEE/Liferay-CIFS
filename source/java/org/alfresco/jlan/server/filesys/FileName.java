...
public final class FileName {
	...
	public static String buildPath(
		String dev, String path, String filename, char sep) {

		// Debug.println ( "BuildPath: dev=" + dev + ", path=" + path +
		// ",filename=" + filename);
		...
		if (filename != null) {
			...
			if (fullPath.length() > 0 &&
				filename.length() > 0 &&
				(filename.charAt(0) == sep || 
					filename.charAt(0) == DOS_SEPERATOR))
		...
		// Convert the file seperator characters in the path if we are not using
		// the normal
	...
	/**
	 * Convert the file seperators in a path to the specified path seperator
	 * character.
	 * 
	 * @param path java.lang.String
	 * @param sep char
	...
	public static String convertSeperators(String path, char sep) {
		...
		while (idx < path.length()) {
			...
			// Get the current character from the path and check if it is a DOS
			// path
			// seperator character.
	...
	public static String[] splitPath(String path, char sep, String[] list) {
		...
		if (path != null && path.length() > 0) {
			...
			// Check if the path has a trailing seperator, if so then there is
			// no
			// file name.
		...
		// Debug.println ( "SplitPath: original=" + path + ", pathStr[0]=" +
		// pathStr[0] + ", pathStr[1]=" + pathStr[1]);
	...
	/**
	 * Split a path string into directory path, file name and stream name
	 * components
	 * 
	 * @param path Full path string.
	 * @return java.lang.String[]
	 ...
	 * @return String
	 */
	public final static String makeRelativePath(
		...
		if (baseNames != null && baseNames.length > 0 && fullNames != null &&
			fullNames.length > 0 &&
