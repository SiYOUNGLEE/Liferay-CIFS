...
/**
 * Byte buffer Pool Class <p>Memory pool of different sized byte buffers.
 * 
 * @author gkspencer
 */
public class ByteBufferPool {
	...
	public ByteBufferPool(int[] bufSizes, int[] initAlloc, int[] maxAlloc) {
		...
		// Validate the buffer size, initial allocation and max allocation
		// arrays

		if ((bufSizes.length != initAlloc.length) &&
			(bufSizes.length != maxAlloc.length))
		...
		for (int i = 0; i < m_bufSizes.length; i++) {
			...
			ByteBufferList bufList = new ByteBufferList(
				m_bufSizes[i], m_initAlloc[i], m_maxAlloc[i]);
	...
	public final byte[] allocateBuffer(int siz, long waitTime) {
		...
		if (idx == m_bufSizes.length)
			throw new RuntimeException(
				"Requested allocation size too long for pool, " + siz);
	...
	public final void releaseBuffer(byte[] buf) {
		...
		if (idx == m_bufSizes.length)
			throw new RuntimeException(
				"Released buffer does not match any buffer sizes, " +
					buf.length);
