...
/**
 * CIFS Authenticator Interface <p> An authenticator is used by the CIFS server
 * to authenticate users when in user level access mode and authenticate
 * requests to connect to a share when in share level access.
 * 
 * @author gkspencer
 * @author dward
 */
public interface ICifsAuthenticator {
	 ...
	 * @return int Granted file permission level or disallow status if negative.
	 *         See the FilePermission class.
	 */
	...
	/**
	 * Authenticate a user. A user may be granted full access, guest access or
	 * no access.
	 * 
