...
/**
 * <p>Local Authenticator Class. <p>The local authenticator implementation
 * enables user level security mode and uses the user account list that is part
 * of the server configuration to determine if a user is allowed to access the
 * server/share. <p>Note: Switching off encrypted password support will cause
 * later NT4 service pack releases and Win2000 to refuse to connect to the
 * server without a registry update on the client.
 * 
 * @author gkspencer
 */
public class LocalAuthenticator extends CifsAuthenticator {
	...
	/**
	 * Local Authenticator Constructor <p>Default to user mode security with
	...
	public int authenticateShareConnect(
		ClientInfo client, SharedDevice share, String pwd, SrvSession sess) {
