...
public class DEROid extends DERObject {
	...
	public void derDecode(DERBuffer buf)
		...
		if (buf.unpackType() == DER.ObjectIdentifier) {
			...
			for (int i = 0; i < len; i++) {
				...
				// Get a byte from the Oid data, and add the bottom 7 bits to
				// the current value
			...
			throw new IOException(
				"Invalid type in buffer, not ObjectIdentifier");
	}
	...
	public void derEncode(DERBuffer buf)
		...
		// Pack the data type and skip the length, save the buffer position for
		// the length
		...
		packOidField(buf, (Integer.parseInt(tok.nextToken()) * 40) +
