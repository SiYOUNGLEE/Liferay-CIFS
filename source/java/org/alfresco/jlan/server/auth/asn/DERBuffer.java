...
/**
 * DER Buffer Class <p>Pack/unpack objects from a ASN.1 DER encoded blob.
 * 
 * @author gkspencer
 */
public class DERBuffer {
	...
	public final DERObject unpackObject()
		...
		if ((objTyp & DER.Application) != 0) {
			...
			if (DebugEnable)
				Debug.println(
					"DER unpackObject typ=Applicationspecific objTyp=0x" +
					Integer.toHexString(objTyp));
		}
		else {
			...
			if (DebugEnable)
				Debug.println(
					"DER unpackObject typ=" +
					DER.isTypeString(DER.isType(objTyp)) + " objTyp=0x" +
					Integer.toHexString(objTyp));
		...
		if (derObj != null)
			...
			throw new IOException(
				"ASN.1 type 0x" +
				Integer.toHexString(objTyp) + " decode not supported");
	...
	public final void packLength(int len) {
		...
		if (len < 128) {
			...
			// Calculate the number of bytes required to store the length and
			// pack
	...
	public final void packObject(DERObject derObj)
		...
		// If the object is tagged we need to pack to a seperate buffer first to
	...
	public final DERObject unpackApplicationSpecific()
		...
		if (DER.isApplicationSpecific(typ)) {
			...
			// Unpack the top level object, this will cause all objects to be
			// loaded
			...
			throw new IOException(
				"Wrong DER type, expected Application or Context");
