...
/**
 * Password Encryptor Class <p> Generates LanMan and NTLMv1 encrypted passwords
 * from the plain text password and challenge key.
 * 
 * @author GKSpencer
 */
public class PasswordEncryptor {
	...
	private final static String[] _algNames = {
	 ...
	 */
	public byte[] generateEncryptedPassword(
		...
		case NTLM2:
			...
			byte[] md4Hash = generateEncryptedPassword(
				plainPwd, encryptKey, MD4, null, null);

			// HMAC-MD5 the username + domain string using the MD4 hash as the
			// key

			Mac hmacMd5 = Mac.getInstance("HMACMD5");
			SecretKeySpec key = new SecretKeySpec(
				md4Hash, 0, md4Hash.length, "MD5");
	...
	public final byte[] P16(String pwd, byte[] s8)
		...
		// Convert the P14 string to an array of bytes. Allocate a 21 byte
		// buffer as the result is
		// usually passed
		...
		try {
			...
			// Set the encryption seed using the first 7 bytes of the password
			// string.
	...
	private final byte[] P24(String pwd, byte[] c8)
		...
		// Generate the 16 byte encrypted value using the password string and
		// well
		// known value.
	 ...
	 *                not available
	 */
	private final byte[] P24(byte[] p21, byte[] ch)
		throws NoSuchAlgorithmException {
	...
	private byte[] generateKey(byte[] byt, int off) {
		...
		key[1] = (byte) (((byt[off + 0] & 0x01) << 6) | 
						((byt[off + 1] & 0xFF) >> 2));
		key[2] = (byte) (((byt[off + 1] & 0x03) << 5) |
						((byt[off + 2] & 0xFF) >> 3));
		key[3] = (byte) (((byt[off + 2] & 0x07) << 4) | 
						((byt[off + 3] & 0xFF) >> 4));
		key[4] = (byte) (((byt[off + 3] & 0x0F) << 3) | 
						((byt[off + 4] & 0xFF) >> 5));
		key[5] = (byte) (((byt[off + 4] & 0x1F) << 2) |
						((byt[off + 5] & 0xFF) >> 6));
		key[6] = (byte) (((byt[off + 5] & 0x3F) << 1) | 
						((byt[off + 6] & 0xFF) >> 7));
	 ...
	 * @exception NoSuchAlgorithmException
	 */
	public final byte[] doNTLM2Encryption(
		byte[] md4Hash, String userName, String domain)
		throws NoSuchAlgorithmException, InvalidKeyException {
	...
	/**
	 * LanMan encryption of a password previously encrypted with the well known
	 * value
	 * 
	 * @param p21 byte[]
	 * @param c8 byte[]
