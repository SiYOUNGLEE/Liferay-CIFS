...
/**
 * Ip Address Access Control Class <p>Allow/disallow access by checking for a
 * particular TCP/IP address or checking that the address is within a specified
 * subnet.
 * 
 * @author gkspencer
 */
public class IpAddressAccessControl extends AccessControl {
	 ...
	 * @param access int
	 */
	public IpAddressAccessControl(
		String address, String mask, String type, int access) {
	...
	/**
	 * Check if the TCP/IP address matches the specifed address or is within the
	 * subnet.
	 * 
	 ...
	 * @return int
	 */
	public int allowsAccess(
		SrvSession sess, SharedDevice share, AccessControlManager mgr) {
		...
		if (m_netMask == null) {
			...
			if (IPAddress.parseNumericAddress(ipAddr) == IPAddress
							.parseNumericAddress(getName()))
				sts = getAccess();
		}
		else {
