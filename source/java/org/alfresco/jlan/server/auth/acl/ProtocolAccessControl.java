...
/**
 * Protocol Access Control Class <p>Allow/disallow access to a share based on
 * the protocol type.
 * 
 * @author gkspencer
 */
public class ProtocolAccessControl extends AccessControl {
	...
	private static final String[] _protoTypes = {
		"SMB", "CIFS", "NFS", "FTP"
	};
	...
	/**
	 * Check if the protocol matches the access control protocol list and return
	 * the allowed access.
	 * 
	...
	public int allowsAccess(
		...
		if (sessProto != null &&
			indexFromList(sessProto, m_checkList, false) != -1)
			return getAccess();
		return Default;
	}
