...
/**
 * Group Id Access Control Class <p>Allow/disallow access to a shared device by
 * checking the Unix group ids of the client.
 * 
 * @author gkspencer
 */
public class GidAccessControl extends AccessControl {
	...
	/**
	 * Check if the session is an RPC session (NFS/mount) and the client is a
	...
	public int allowsAccess(
		...
		// Check if the client main group id is set and matches the required
		// group id
		...
		// Check if the client has a group list, if so check if any of the group
		// match the required group id
