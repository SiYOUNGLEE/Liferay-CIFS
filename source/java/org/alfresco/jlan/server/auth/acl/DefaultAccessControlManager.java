...
/**
 * Default Access Control Manager Class <p>Default access control manager
 * implementation.
 * 
 * @author gkspencer
 */
public class DefaultAccessControlManager implements AccessControlManager {
	...
	public int checkAccessControl(SrvSession sess, SharedDevice share) {
		...
		if (share.hasAccessControls() == false) {
			...
			if (Debug.EnableInfo && hasDebug())
				sess.debugPrintln(
					"Check access control for " +
					share.getName() + ", no ACLs");
		...
		if (Debug.EnableInfo && hasDebug())
			sess.debugPrintln(
				"Check access control for " + share.getName() +
				", ACLs=" + acls.numberOfControls());
		...
		for (int i = 0; i < acls.numberOfControls(); i++) {
			...
			if (Debug.EnableInfo && hasDebug())
				sess.debugPrintln(
					"  Check access ACL=" + acl + ", access=" +
					AccessControl.asAccessString(curAccess));
		...
		// Check if the default access level is still selected, if so then get
		// the default level from the
		// access control list
		...
		if (access == AccessControl.Default) {
			...
			if (Debug.EnableInfo && hasDebug())
				sess.debugPrintln(
					"Access defaulted=" +
					AccessControl.asAccessString(access) + ", share=" + share);
		}
	...
	/**
	 * Filter the list of shared devices to return a list that contains only the
	 * shares that are visible or accessible by the session.
	 * 
	 * @param sess SrvSession
	 * @param shares SharedDeviceList
	 * @return SharedDeviceList
	 */
	public SharedDeviceList filterShareList(
		SrvSession sess, SharedDeviceList shares) {
		...
		if (Debug.EnableInfo && hasDebug())
			sess.debugPrintln(
				"Filter share list for " + sess + ", shares=" + shares);

		// For each share in the list check the access, remove any shares that
	...
	public void initialize(ServerConfiguration config, ConfigElement params)
		...
		if (ruleList != null && ruleList.hasChildren()) {
			...
			for (ConfigElement ruleVal : rules) {
				...
				if (ruleVal.getValue() == null ||
					ruleVal.getValue().length() == 0)
					throw new InvalidConfigurationException(
						"Empty rule definition");

				// Create an instance of the rule parser and check that it is
				...
				try {
					...
					Object ruleObj =
						Class.forName(ruleVal.getValue()).newInstance();
					...
					throw new InvalidConfigurationException(
						"Rule class not found, " + ruleVal.getValue());
				}
				catch (InstantiationException ex) {
					throw new InvalidConfigurationException(
						"Error creating rule object, " + ruleVal.getValue() +
							", " + ex.toString());
				}
	...
	/**
	 * Add an access control parser to the list of available access control
	 * types.
	 * 
	...
	public void addAccessControlType(AccessControlParser parser) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"AccessControlManager Add rule type " + parser.getType());
