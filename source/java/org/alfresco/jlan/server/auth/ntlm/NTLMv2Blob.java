...
/**
 * NTLMv2 Blob Class <P>Contains methods to pack/unpack and calculate the hash
 * of an NTLMv2 blob.
 * 
 * @author gkspencer
 */
public class NTLMv2Blob {
	...
	public final byte[] getClientChallenge() {
		...
		System.arraycopy(
			m_blob, m_offset + OFFSET_CHALLENGE, challenge, 0, CHALLENGE_LEN);

		return challenge;
	}

	/**
	 * Calculate the HMAC of the blob using the specified NTLMv2 hash and
	 * challenge
	 * 
	...
	public final byte[] calculateHMAC(byte[] challenge, byte[] v2hash)
		...
		System.arraycopy(
			m_blob, m_offset + OFFSET_HEADER, blob, CHALLENGE_LEN, m_len -
				HMAC_LEN);

		// Generate the HMAC of the blob using the v2 hash as the key

		Mac hmacMd5 = Mac.getInstance("HMACMD5");
		SecretKeySpec blobKey =
			new SecretKeySpec(v2hash, 0, v2hash.length, "MD5");
	 ...
	 * @return byte[]
	 */
	public final byte[] calculateLMv2HMAC(
		byte[] v2hash, byte[] srvChallenge, byte[] clChallenge)
		throws Exception {
	...
	public final void Dump() {
		...
		System.out.println("     Header : 0x" +
			Integer.toHexString(DataPacker.getIntelInt(
				m_blob, m_offset + OFFSET_HEADER)));
		System.out.println(
			"  Timestamp : " +
			new Date(NTTime.toJavaDate(getTimeStamp())));
		System.out.println(
			"  Challenge : " +
			HexDump.hexString(getClientChallenge()));
	}
