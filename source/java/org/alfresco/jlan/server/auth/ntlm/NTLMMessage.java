...
public abstract class NTLMMessage {
	...
	protected void initializeHeader(int typ, int flags) {
		...
		System.arraycopy(
			NTLM.Signature, 0, m_buf, m_offset, NTLM.Signature.length);
	...
	protected final String getStringValue(int offset, boolean isuni) {
		...
		try {
			str =
				new String(m_buf, m_offset + pos, len, isuni
					? "UnicodeLittle" : "US-ASCII");
		}
	 ...
	 * @return int
	 */
	protected final int setStringValue(
		int offset, String val, int strOffset, boolean isuni) {
		...
		return DataPacker.putString(
			val, m_buf, m_offset + strOffset, false, isuni) -
			m_offset;
	}
