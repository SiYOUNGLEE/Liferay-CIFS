...
public class KerberosApRep {
	 ...
	 * @param int encVno
	 */
	public final void setEncryptedPart(
		int encType, byte[] encData, int encVno) {
				...
				if (derObj != null && derObj.isTagged()) {
						...
						if (derObj instanceof DERInteger) {
							...
							if (derInt.getValue() != 5)
								throw new IOException(
									"Unexpected PVNO value in AP-REP");
						}
						break;
					...
					case 1:
						if (derObj instanceof DERInteger) {
							...
							if (derInt.getValue() != 15)
								throw new IOException(
									"Unexpected msg-type value in AP-REP");
						}
						break;
					...
					case 2:
						if (derObj instanceof DERSequence) {
							...
							for (int i = 0; i < derAuthSeq.numberOfObjects();
								i++) {
