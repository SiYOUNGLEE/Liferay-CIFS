...
/**
 * Passthru Servers Class <p>Contains a list of one or more servers that are
 * used for passthru authentication. The status of the servers is tracked so
 * that offline servers are not used but periodically monitored so that they can
 * be returned to the online list of servers. <p>The server list may be
 * initialized from a list of server names or addresses, or by specifying a
 * domain name in which case the primary and backup domain controllers will be
 * used.
 * 
 * @author GKSpencer
 */
public class PassthruServers {
	...
	private static final long DefaultOfflineCheckInterval = 5 * 60000; // 5
																		// minutes
	...
	// Timeout value when opening a session to an authentication server, in
	// milliseconds
	...
	/**
	 * Inner class used to periodically check offline servers to see if they are
	 * back online
	 */
			...
			while (m_ishutdown == false) {
				...
				if (getOfflineServerCount() > 0) {
					...
					while (idx < getOfflineServerCount()) {
						...
						offlineServer =
							(PassthruServerDetails) m_offlineList.get(idx);
						...
						if (offlineServer != null) {
							try {
								...
								authShare.setNodeName(offlineServer.getAddress()
									.getHostAddress());

								// Try and connect to the authentication server

								authSess =
									AuthSessionFactory.OpenAuthenticateSession(
										authShare, getConnectionTimeout());
								...
								// Authentication server is online, move it to
								// the online list
								...
								if (hasDebug())
									Debug.println(
										"Passthru offline check failed for " +
										offlineServer.getName());
							}
	 ...
	 */
	public final AuthenticateSession openSession(
		boolean useExtSec, String clientDomain) {
		...
		if (hasDebug())
			Debug.println(
				...
				passthruServer +
				(clientDomain != null ? " (routed for client domain " +
					clientDomain + ")" : ""));
		...
		PCShare authShare =
			new PCShare(
				passthruServer.getAddress().getHostAddress(), "IPC$", "", "");
		AuthenticateSession authSess = null;

		while (authSess == null && passthruServer != null &&
			hasOnlineServers()) {
			...
			try {
				...
				if (hasDebug())
					Debug.println(
						"Failed to connect to " + passthruServer + " : " + 
						ex.getMessage());

				// Failed to connect to the current authentication server, mark
			...
			if (authSess == null) {
				...
				if (hasDebug())
					Debug.println(
						"Trying authentication server " + passthruServer);
			}
		}
	...
	protected PassthruServerDetails getAuthenticationServer() {
		...
		// Check if any available passthru server or a passthru server that does
		// not have a domain name set
		// should be used
		...
		if (getNullDomainUseAnyServer()) {
			...
			synchronized (m_onlineList) {
				...
				// Rotate the head of the list and return the new head of list
				// server details
				...
				if (m_onlineList.size() > 0)
					passthruServer =
						(PassthruServerDetails) m_onlineList.get(0);
			}
		}
		else {

			// Search for an online passthru server that does not have a domain
			// name set
			...
			synchronized (m_onlineList) {
				...
				while (idx < m_onlineList.size() && passthruServer == null) {
					...
					if (curServer.getDomain() == null ||
						curServer.getDomain().length() == 0) {
	...
	/**
	 * Return the details of an online server to use for authentication of the
	 * specified client domain
	 * 
	 * @param clientDomain String
	 * @return PassthruServerDetails
	 */
	protected PassthruServerDetails getAuthenticationServer(
		...
		// Rotate the head of the list and return the new head of list server
		// details
		...
		synchronized (m_onlineList) {
			...
			while (idx < m_onlineList.size() && passthruServer == null) {
				...
				if (curServer.getDomain() != null &&
					curServer.getDomain().equals(clientDomain)) {
	...
	/**
	 * Set the list of servers to use for passthru authentication using a comma
	 * delimeted list of server names/addresses
	 * 
	...
	public final void setServerList(String servers) {
		...
		while (tokens.hasMoreTokens()) {
			...
			// If a name a has been specified convert it to an address, if an
			...
			if (IPAddress.isNumericAddress(srvName)) {
				...
				try {
					...
					if (hasDebug())
						Debug.println(
							"Passthru failed to find name/address for " +
							srvName);
				}
			}
			else {
				...
				try {
					...
					if (hasDebug())
						Debug.println(
							"Passthru failed to find address for " +
							srvName);
				}
			}

			// Create the passthru server details and add to the list of offline
			// servers
			...
			if (srvName != null && srvAddr != null) {
				...
				PassthruServerDetails passthruServer =
					new PassthruServerDetails(srvName, domain, srvAddr, false);
		...
		// Wakeup the server checker thread to check each of the servers just
		// added and move servers that are
	...
	public final void setDomain(String domain)
		...
		if (hasDebug())
			Debug.println(
				"Passthru finding domain controller for " + domain + " ...");
		...
		try {
			...
			nbName = NetBIOSSession.FindName(
				domain, NetBIOSName.DomainControllers,
			...
			if (hasDebug())
				Debug.println(
					"  Found " + nbName.numberOfAddresses() +
					" domain controller(s)");
		}
		...
		if (nbName == null) {
			...
			try {
				...
				nbName = NetBIOSSession.FindName(
					domain, NetBIOSName.MasterBrowser,
					getConnectionTimeout());
				...
				if (hasDebug())
					Debug.println(
						"  Found browse master at " +
						nbName.getIPAddressString(0));
			}
			catch (IOException ex) {
				throw new IOException(
					"Failed to find domain controller or browse master for " +
						domain);
			}
		}
		...
		for (int i = 0; i < nbName.numberOfAddresses(); i++) {
			...
			try {
				...
				NetBIOSNameList nameList = NetBIOSSession.FindNamesForAddress(
					dcAddr.getHostAddress());
				NetBIOSName dcNBName = nameList.findName(
					NetBIOSName.FileServer, false);
				...
				if (hasDebug())
					Debug.println(
						"Failed to get NetBIOS name for server " + dcAddr);
			}
			...
			if (dcAddr != null) {
				...
				PassthruServerDetails passthruServer =
					new PassthruServerDetails(dcName, domain, dcAddr, false);
				m_offlineList.add(passthruServer);
