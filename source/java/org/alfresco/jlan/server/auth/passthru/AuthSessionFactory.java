...
/**
 * <p>The AuthSessionFactory static class is used to create sessions to remote
 * shared resources using the SMB/CIFS protocol. A PCShare object is used to
 * specify the remote node and share details, as well as required access control
 * details.
 * 
 * @author gkspencer
 */
public final class AuthSessionFactory {
	...
	// If this is an evaluation version then only 5 sessions may be opened
	// before an exception is
	// thrown.
	//
	// FULL VERSION = -1
	// DEMO VERSION = 5
	...
	// Default user name, password and domain used by methods that create their
	// own connections.
	...
	// NetBIOS port to connect to when setting up a new session. The default
	// port is 139.
	...
	private final static Vector BuildNegotiatePacket(
		SMBPacket pkt, DialectSelector dlct, int pid) {
		...
		// If the NT dialect is enabled set the Unicode flag in the request
		// flags
		...
		// Loop through all SMB dialect types and add the appropriate dialect
		// strings
		...
		while (d < Dialect.Max) {
			...
			if (dlct.hasDialect(d)) {
				...
				// Search the SMB dialect type string list and add all strings
				// for the
				// current dialect

				for (int i = 0; i < Dialect.NumberOfDialects(); i++) {

					// Check if the current dialect string should be added to
					// the list
					...
					if (Dialect.DialectType(i) == d) {
						...
						// Add the current SMB dialect type string to the
						// negotiate packet
	...
	private static void CheckLicence()
		...
		// Check if this is an evaluation version, if so then check if we can
		// create any more sessions
		...
		if (m_evalCount >= 0) {
			...
			if (m_evalCount == 0)
				throw new SMBException(
					SMBStatus.JLANErr, SMBStatus.JLANNoMoreSessions);
			m_evalCount--;
		}
		...
		if (m_evalCount != -1 && System.currentTimeMillis() > 1122678055578L)
			throw new SMBException(
				SMBStatus.JLANErr, SMBStatus.JLANEvalExpired);
	}
	...
	/**
	 * Return the primary connection protocol (either Protocol.TCPNetBIOS or
	 * Protocol.NativeSMB)
	 * 
	 * @return int
	 */
	...
	/**
	 * Return the secondary connection protocol (Protocol.TCPNetBIOS,
	 * Protocol.NativeSMB or Protocol.None)
	 * 
	 * @return int
	 */
	...
	private static synchronized InetAddress[] getLocalTcpipAddresses() {
		...
		if (m_localAddrList == null) {
			try {
				m_localAddrList = InetAddress.getAllByName(
	...
	/**
	 * Open a session to a remote server, negotiate an SMB dialect and get the
	 * returned challenge key. Returns an AuthenticateSession which can then be
	 * used to provide passthru authentication.
	 * 
	 ...
	 */
	public static AuthenticateSession OpenAuthenticateSession(
		PCShare shr, int tmo)
	 ...
	 */
	public static AuthenticateSession OpenAuthenticateSession(
		PCShare shr, int tmo, DialectSelector dia)
		throws java.io.IOException, java.net.UnknownHostException, 
		SMBException {
		...
		StringBuffer nameBuf =
			new StringBuffer(InetAddress.getLocalHost().getHostName() + "_" +
				pid);
		...
		if (Debug.EnableInfo && AuthenticateSession.hasDebug()) {
			Debug.println("** New auth session from " + localName + " to " +
				shr.toString());
		...
		case Protocol.TCPNetBIOS:
			netSession =
				connectNetBIOSSession(shr.getNodeName(), localName, tmo);
			break;
		...
		case Protocol.NativeSMB:
			netSession =
				connectNativeSMBSession(shr.getNodeName(), localName, tmo);
			break;
		}

		// If the connection was not made using the primary protocol try the
		// secondary protocol, if configured
		...
		if (Debug.EnableInfo && AuthenticateSession.hasDebug())
			Debug.println("** Connected session, protocol : " +
				netSession.getProtocolName());
		...
		// Build the negotiate SMB dialect packet and exchange with the remote
		// server
		...
		if (Debug.EnableInfo && AuthenticateSession.hasDebug())
			Debug.println(
				"** SessionFactory: Negotiated SMB dialect " + diaStr);
		...
		AuthenticateSession authSess =
			new AuthenticateSession(shr, netSession, dialectId, pkt);
		return authSess;
	}
	...
	/**
	 * Set the subnet mask string for network broadcast requests If the subnet
	 * mask is not set a default broadcast mask for the TCP/IP address class
	 * will be used.
	 * 
	...
	/**
	 * Setup the default SMB dialects to be negotiated when creating new
	 * sessions.
	 */
	 ...
	 */
	private static final NetworkSession connectNetBIOSSession(
		String toName, String fromName, int tmo)
		throws IOException {

		// Connect to the requested server

		NetBIOSSession nbSession = new NetBIOSSession(
			tmo, getNetBIOSPort(), RFCNetBIOSProtocol.NAME_PORT);
		...
		if (IPAddress.isNumericAddress(toName)) {
			...
			try {
				...
				toAddr = toName;
				NetBIOSNameList nameList =
					NetBIOSSession.FindNamesForAddress(toAddr);
			...
			// Find the remote host and get a list of the network addresses it
			// is using

			nbName = NetBIOSSession.FindName(
				toName, NetBIOSName.FileServer, 500);
		}

		// Check if the NetBIOS name scope has been set, if so then update the
		// names to add the scope id
		...
		// If the NetBIOS name has more than one TCP/IP address then find the
		// best match for the client and
		// try to connect on that address first, if that fails then we will have
		// to try each address in turn.

		if (nbName.numberOfAddresses() > 1) {

			// Get the local TCP/IP address list and search for a best match
			// address to connect to the server on
			...
			if (addrIdx != -1) {
				...
				try {
					...
					if (Debug.EnableInfo && hasSessionDebug())
						Debug.println(
							"** Server is multi-homed, trying to " +
							"connect to " + ipAddr);
					...
					else if (Debug.EnableInfo && hasSessionDebug() &&
						nbSession.isConnected())
		...
		if (Debug.EnableInfo && hasSessionDebug() &&
			nbSession.isConnected() == false && nbName.numberOfAddresses() > 1)
			Debug.println("** Server is multi-homed, trying all addresses");

		// Loop through the available addresses for the remote file server until
		// we get a successful
		...
		while (nbSession.isConnected() == false &&
			addrIdx < nbName.numberOfAddresses()) {
	 ...
	 * @exception IOException If a network error occurs
	 */
	private static final NetworkSession connectNativeSMBSession(
		String toName, String fromName, int tmo)
		throws IOException {
