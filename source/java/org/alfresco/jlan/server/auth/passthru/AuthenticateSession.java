...
/**
 * Authenticate Session Class <p>Used for passthru authentication mechanisms.
 * 
 * @author gkspencer
 */
public class AuthenticateSession {
	...
	// Maximum virtual circuits allowed on this session, and maximum multiplxed
	// read/writes

	private int m_maxVCs;
	private int m_maxMPX;

	// Indicate if the session was created as a guest rather than using the
	...
	protected AuthenticateSession(
		PCShare shr, NetworkSession sess, int dialect, SMBPacket pkt)
	...
	public final void pingServer(int cnt)
		...
		m_pkt.setParameter(0, cnt); // number of packets that the server should
									// return
		String echoStr = "ECHO";
	...
	protected void processAsynchResponse(SMBPacket pkt) {
		...
		//
		// This method is overridden by SMB dialects that can generate
		// asynchronous responses

		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"++ Asynchronous response received, command = 0x" +
				pkt.getCommand());
	}
	...
	/**
	 * Perform a session setup to create a session on the remote server
	 * validating the user.
	 * 
	 ...
	 */
	public final void doSessionSetup(
		String userName, byte[] ascPwd, byte[] uniPwd)
		throws IOException, SMBException {
	 ...
	 * @exception SMBException If a CIFS error occurs
	 */
	public final void doSessionSetup(Type3NTLMMessage type3Msg)
		throws IOException, SMBException {
		doSessionSetup(
			type3Msg.getDomain(), type3Msg.getUserName(),
			type3Msg.getWorkstation(), type3Msg.getLMHash(),
			type3Msg.getNTLMHash(), 0);
	}
	 ...
	 */
	public final void doSessionSetup(
		String domain, String userName, String wksName, byte[] ascPwd,
		byte[] uniPwd, int vcNum)
		throws IOException, SMBException {
		...
		if (isUsingExtendedSecurity()) {
			...
			doExtendedSessionSetupPhase2(
				domain, userName, wksName, ascPwd, uniPwd, vcNum);
			return;
		}
		...
		// Check if the negotiated SMB dialect is NT LM 1.2 or an earlier
		// dialect
		...
		if (getDialect() == Dialect.NT) {
			...
			pkt.setParameter(7, ascPwd != null ? ascPwd.length : 0); // ANSI
																	 // password
																	 // length
			pkt.setParameter(8, uniPwd != null ? uniPwd.length : 0); // Unicode
																	 // password
																	 // length
			...
			int caps =
				Capability.LargeFiles + Capability.Unicode + Capability.NTSMBs +
					Capability.NTStatus + Capability.RemoteAPIs;
			...
			// Check if the share has a domain, if not then use the default
			// domain string
			...
			// Check if the share has a domain, if not then use the unknown
			// domain string
	...
	private void processNegotiateResponse()
		...
		if (getDialect() == Dialect.NT) {
			...
			// Read the returned negotiate parameters, for NT dialect the
			...
			// Set the maximum virtual circuits and multiplxed requests allowed
			// by the server
			...
			defFlags2 =
				SMBPacket.FLG2_LONGFILENAMES + SMBPacket.FLG2_UNICODE +
					SMBPacket.FLG2_LONGERRORCODE + SMBPacket.FLG2_SECURITYSIG;
			...
			setSecurityMode((secMode & 0x01) != 0
				? SecurityModeUser : SecurityModeShare);
		...
		if (m_pkt.getByteCount() > 0) {
			...
			if (isUsingExtendedSecurity() == false) {
				...
				// Run the first phase of the extended security session setup to
				// get the challenge
				// from the server
	 ...
	 * @eception SMBException If a CIFS error occurs
	 */
	private final void doExtendedSessionSetupPhase1()
		throws IOException, SMBException {
		...
		Type1NTLMMessage type1Msg =
			new Type1NTLMMessage(pkt.getBuffer(), pos, 0);

		int type1Flags = getPCShare().getExtendedSecurityFlags();
		if (type1Flags == 0)
			type1Flags =
				NTLM.FlagNegotiateUnicode + NTLM.FlagNegotiateNTLM +
					NTLM.FlagRequestTarget;
		...
		// Check the error status, should be a warning status to indicate more
		// processing required

		if (pkt.isLongErrorCode() == false ||
			pkt.getLongErrorCode() != SMBStatus.NTMoreProcessingRequired)
			pkt.checkForError();
	 ...
	 * @eception SMBException If a CIFS error occurs
	 */
	private final void doExtendedSessionSetupPhase2(
		String domain, String userName, String wksName, byte[] lmPwd,
		byte[] ntlmPwd, int vcNum)
		throws IOException, SMBException {

		// Check if the domain name has been specified, if not then use the
		// domain name from the
		// original connection details or the servers domain name

		if (domain == null) {
			if (getPCShare().hasDomain() &&
				getPCShare().getDomain().length() > 0)
		...
		Type3NTLMMessage type3Msg =
			new Type3NTLMMessage(pkt.getBuffer(), pos, 0, true);

		type3Msg.buildType3(lmPwd, ntlmPwd, domain, userName, wksName != null
			? wksName : "", null, m_type2Msg.getFlags());
