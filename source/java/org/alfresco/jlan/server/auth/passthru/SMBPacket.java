...
public class SMBPacket {
	 ...
	 */
	protected final synchronized void ExchangeLowLevelSMB(
		NetworkSession sess, SMBPacket rxPkt, boolean throwerr)
	 ...
	 * @exception SMBException If an SMB level error occurs.
	 */
	public synchronized final void ExchangeSMB(
		AuthenticateSession sess, SMBPacket rxPkt)
	 ...
	 */
	public synchronized final void ExchangeSMB(
		AuthenticateSession sess, SMBPacket rxPkt, boolean throwerr)
		throws SMBException, IOException {
		...
		// Receive the response, other asynchronous responses may be received
		// before the response for this request
	 ...
	 * 
	 * @param sess SMB session to receive the packet on.
	 * @param throwErr Flag to indicate if an error is thrown if an error
	...
	protected final void ReceiveSMB(AuthenticateSession sess, boolean throwErr)
		...
		while (rxValid == false) {
			...
			if (netSess.Receive(getBuffer()) >= MIN_RXLEN) {
				...
				// Asynchronous response received, pass the packet to the
	 ...
	 * @throws SMBException
	 */
	protected final void ReceiveAsynchSMB(
		AuthenticateSession sess, int waitTime)
		throws java.io.IOException, SMBException {
		...
		if (netSess.Receive(getBuffer()) >= MIN_RXLEN) {
			...
			// Asynchronous response received, pass the packet to the session
			// for processing
	 ...
	 * @exception java.io.IOException If an I/O error occurs.
	 */
	protected final void SendSMB(AuthenticateSession sess)
		throws java.io.IOException {
