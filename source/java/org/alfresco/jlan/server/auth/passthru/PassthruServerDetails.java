...
/**
 * <p>Contains the details of a server used for passthru authentication, the
 * current status of the server and count of authentications done via this
 * server.
 * 
 * @author GKSpencer
 */
public class PassthruServerDetails {
	...
	PassthruServerDetails(
		String name, String domain, InetAddress addr, boolean online) {
		m_name = name;
		m_domain = domain;
		m_address = addr;
		m_online = online;
	...
	public String toString() {
		...
		str.append(",");
		str.append(getAuthenticationDateTime() != 0L ? new Date(
			getAuthenticationDateTime()).toString() : "0");
		str.append("]");
