...
/**
 * Datagram Session Handler Class <p>Implementation of a session handler that
 * uses a Java datagram socket to listen for incoming requests.
 * 
 * @author gkspencer
 */
public abstract class DatagramSessionHandler
	 ...
	 * @param port int
	 */
	protected DatagramSessionHandler(
	 ...
	 * @return boolean Return true to reuse the DatagramPacket, else false to
	...
	protected void sendDatagram(DatagramPacket pkt)
		...
		// Default implementation sends the datagram immediately via the
		// datagram socket
	...
	public void run() {
		...
		try {
			...
			Thread.currentThread().setName(
				getProtocolName() + "_" + getHandlerName());
			...
			while (hasShutdown() == false) {
				...
				if (reusePkt == false) {
					...
					//
					// Reset to use our buffer as the datagram packet may have
					// been reused to send a response.
				...
				try {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							"[" + getProtocolName() +
							"] Error processing datagram, " + ex.toString());
				}
			}
		}
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[" + getProtocolName() + "] " + getHandlerName() +
