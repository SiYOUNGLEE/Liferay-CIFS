...
/**
 * Network Server Base Class <p>Base class for server implementations for
 * different protocols.
 * 
 * @author gkspencer
 */
public abstract class NetworkServer {
	...
	public NetworkServer(String proto, ServerConfiguration config) {
		...
		m_securityConfig = (SecurityConfigSection) m_config.getConfigSection(
			SecurityConfigSection.SectionName);
		m_globalConfig = (GlobalConfigSection) m_config.getConfigSection(
	...
	public final SharedDeviceList getShareList(String host, SrvSession sess) {
		if (m_securityConfig != null)
			return m_securityConfig.getShareMapper().getShareList(
				host, sess, false);
		return null;
	}
	 ...
	 * @return SharedDeviceList
	 */
	public final SharedDeviceList getFullShareList(
		String host, SrvSession sess) {
	 ...
	 * @exception Exception
	 */
	public final SharedDevice findShare(
		...
		if (m_securityConfig != null)
			return m_securityConfig.getShareMapper().findShare(
				host, name, typ, sess, create);
		return null;
	}
	...
	protected final void fireSessionClosedEvent(SrvSession sess) {
		...
		for (int i = 0; i < m_sessListeners.size(); i++) {
			...
			try {
				...
				// TODO: debugPrintln("Session listener error [closed]: " +
				// ex.toString());
			}
		}
	}
	...
	protected final void fireSessionLoggedOnEvent(SrvSession sess) {
		...
		for (int i = 0; i < m_sessListeners.size(); i++) {
			...
			try {
				...
				// TODO: debugPrintln("Session listener error [logon]: " +
				// ex.toString());
			}
		}
	}
	...
	protected final void fireSessionOpenEvent(SrvSession sess) {
		...
		for (int i = 0; i < m_sessListeners.size(); i++) {
			...
			try {
				...
				// TODO: debugPrintln("Session listener error [open]: " +
				// ex.toString());
			}
		}
	}
