...
public class JLANServerService implements WrapperListener, Runnable {
	...
	public Integer start(String[] args) {
		...
		// indicates a mis-configuration in the hosts file
		...
		try {
			...
			if (localAddr.equals("127.0.0.1")) {
				out.println(
					"%% Local IP address resolves to 127.0.0.1, this " +
						"may be caused by a mis-configured hosts file");
		...
		try {
			...
			if (m_config.hasConfigSection(CIFSConfigSection.SectionName)) {
				...
				CIFSConfigSection cifsConfig =
					(CIFSConfigSection) m_config.getConfigSection(
			...
			if (m_config.hasConfigSection(NFSConfigSection.SectionName)) {
				...
				NFSConfigSection nfsConfig =
					(NFSConfigSection) m_config.getConfigSection(
	...
	public int stop(int exitCode) {
		...
		DebugConfigSection dbgConfig =
			(DebugConfigSection) m_config.getConfigSection(
		...
		if (m_config.numberOfServers() > 0) {
			...
			for (int i = 0; i < m_config.numberOfServers(); i++) {
				...
				if (Debug.EnableInfo && dbgConfig != null &&
					dbgConfig.hasDebug())
					Debug.println(
						"Shutting server " + server.getProtocolName() + " ...");
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createSMBServer(
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createNetBIOSServer(
		ServerConfiguration config)
		throws Exception {
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createFTPServer(
		ServerConfiguration config)
		throws Exception {
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createNFSServer(
		ServerConfiguration config)
		throws Exception {
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createNFSMountServer(
		ServerConfiguration config)
		throws Exception {
	 ...
	 * @return NetworkServer
	 */
	protected final static NetworkServer createNFSPortMapper(
		ServerConfiguration config)
		throws Exception {
		...
		return createServer(
			"org.alfresco.jlan.oncprc.portmap.PortMapperServer", config);
	}
	 ...
	 * @exception Exception
	 */
	protected final static NetworkServer createServer(
		...
		if (m_config.numberOfServers() > 0) {
			...
			for (int i = 0; i < m_config.numberOfServers(); i++) {
				...
				if (Debug.EnableInfo && dbgConfig != null &&
					dbgConfig.hasDebug())
					Debug.println(
						"Starting server " + server.getProtocolName() + " ...");
