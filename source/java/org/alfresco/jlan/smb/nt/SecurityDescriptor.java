...
public class SecurityDescriptor {
	 ...
	 */
	public SecurityDescriptor(
		String name, int type, SID owner, SID group, ACL dacl, ACL sacl) {
		m_name = name;
		m_type = type;
	...
	public final int loadDescriptor(byte[] buf, int off)
		...
		// Make sure the security descriptor is self-raltive, if not then abort
		// the load

		if (isSelfRelative() == false)
			throw new LoadException(
				"Security descriptor not self-relative, cannot load");
	...
	public final int saveDescriptor(byte[] buf, int off)
		...
		if (hasOwner()) {
			...
			DataPacker.putIntelInt(pos - off, buf, off + 4); // offset to owner
																// SID
		...
		if (hasGroup()) {
			...
			DataPacker.putIntelInt(pos - off, buf, off + 8); // offset to group
																// SID
