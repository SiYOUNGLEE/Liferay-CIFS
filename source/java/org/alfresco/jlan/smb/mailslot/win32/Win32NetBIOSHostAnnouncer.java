 ...
 * existence and capabilities. <p>The Win32 NetBIOS host announcer sends out the
 * announcements using datagrams sent via the Win32 Netbios() Netapi32 call.
 * 
 * @author gkspencer
 */
public class Win32NetBIOSHostAnnouncer extends HostAnnouncer {
	 ...
	 */
	public Win32NetBIOSHostAnnouncer(
	...
	protected void sendAnnouncement(
		...
		NetBIOSName destNbName =
			new NetBIOSName(getDomain(), NetBIOSName.MasterBrowser, false);
		byte[] destName = destNbName.getNetBIOSName();

		// Send the host announce datagram via the Win32 Netbios() API call

		Win32NetBIOS.SendDatagram(
			getLana(), getNameNumber(), destName, buf, 0, len);
	}
