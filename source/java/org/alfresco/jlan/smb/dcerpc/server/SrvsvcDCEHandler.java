...
public class SrvsvcDCEHandler implements DCEHandler {
	...
	public void processRequest(
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_DCERPC))
			sess.debugPrintln("DCE/RPC SrvSvc request=" +
	 ...
	 * @return boolean
	 */
	protected final boolean netShareEnum(
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_DCERPC))
			sess.debugPrintln("NetShareEnum srvName=" + srvName + ", shrInfo=" +
				shrInfo.toString());

		// Get the share list from the server

		SharedDeviceList shareList =
			sess.getServer().getShareMapper().getShareList(srvName, sess, false);
		...
		while (enm.hasMoreElements()) {
			...
			ShareInfo info =
				new ShareInfo(
					shrInfo.getInformationLevel(), share.getName(), shrTyp,
					share.getComment());
			infoList.add(info);
	 ...
	 * @return boolean
	 */
	protected final boolean netShareGetInfo(
		SMBSrvSession sess, DCEBuffer inBuf, DCEBuffer outBuf) {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_DCERPC))
			sess.debugPrintln("netShareGetInfo srvname=" + srvName +
				", share=" + shrName + ", infoLevel=" + infoLevel);
		...
		try {
			...
			share =
				sess.getServer().findShare(
					srvName, shrName, ShareType.UNKNOWN, sess, false);
		}
		catch (Exception ex) {
		}
		...
		ShareInfo shrInfo =
			new ShareInfo(
				infoLevel, share.getName(), shrTyp, share.getComment());
	 ...
	 * @return boolean
	 */
	protected final boolean netServerGetInfo(
		SMBSrvSession sess, DCEBuffer inBuf, DCEBuffer outBuf) {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_DCERPC))
			sess.debugPrintln("netServerGetInfo srvname=" + srvName +
				", infoLevel=" + infoLevel);
