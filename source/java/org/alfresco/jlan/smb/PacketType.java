...
/**
 * SMB packet type class <p>Contains definitions of the available SMB/CIFS
 * request types, plus SMB transaction/transaction2 request types, remote admin
 * protocol (RAP) requests and NT transaction request types. <p>There are also
 * static methods for converting an SMB command type, transaction type or NT
 * transaction type to a name.
 * 
 * @author gkspencer
 */
public class PacketType {
	...
	private static String[] _cmdNames1 =
		{
			"CreateDirectory", "DeleteDirectory", "OpenFile", "CreateFile",
			"CloseFile", "FlushFile", "DeleteFile", "RenameFile",
			"GetFileAttributes", "SetFileAttributes", "ReadFile", "WriteFile",
			"LockFile", "UnLockFile", "CreateTemporary", "CreateNew",
			"CheckDirectory", "ProcessExit", "SeekFile", "LockAndRead",
			"WriteAndUnlock", null, null, null, null, null, "ReadRaw",
			"ReadMpx", "ReadMpxSecondary", "WriteRaw", "WriteMpx", null,
			"WriteComplete", null, "SetInformation2", "QueryInformation2",
			"LockingAndX", "Transaction", "TransactionSecond", "IOCtl",
			"IOCtlSecondary", "Copy", "Move", "Echo", "WriteAndClose",
			"OpenAndX", "ReadAndX", "WriteAndX", null,
			"CloseAndTreeDisconnect", "Transaction2", "Transaction2Secondary",
			"FindClose2", "FindNotifyClose"
		};

	private static String[] _cmdNames2 =
		{
			"TreeConnect", "TreeDisconnect", "Negotiate", "SessionSetupAndX",
			"LogoffAndX", "TreeConnectAndX"
		};

	private static String[] _cmdNames3 = {
		"DiskInformation", "Search", "Find", "FindUnique"
	};

	private static String[] _cmdNames4 = {
		"NTTransact", "NTTransactSecondary", "NTCreateAndX", null, "NTCancel"
	};

	private static String[] _cmdNames5 = {
		"OpenPrintFile", "WritePrintFile", "ClosePrintFile", "GetPrintQueue"
	};

	private static String[] _cmdNames6 =
		{
			"SendMessage", "SendBroadcast", "SendForward", "CancelForward",
			"GetMachineName", "SendMultiStart", "SendMultiEnd", "SendMultiText"
		};

	// Transaction2 operation code names

	private static String[] _transNames =
		{
			"Trans2Open", "Trans2FindFirst", "Trans2FindNext",
			"Trans2QueryFileSys", "Trans2QueryPath", "Trans2SetPath",
			"Trans2QueryFile", "Trans2SetFile", "Trans2CreateDirectory",
			"Trans2SessionSetup"
		};

	// NT transaction operation code names

	private static String[] _ntTranNames =
		{
			"", // zero not used
