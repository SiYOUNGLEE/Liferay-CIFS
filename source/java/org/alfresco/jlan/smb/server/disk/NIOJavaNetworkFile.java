...
public class NIOJavaNetworkFile extends NetworkFile {
	...
	public void lockFile(NIOFileLock lock)
		...
		// If the file is open for write access get an exclusive lock, for read
		// access get a shared lock

		boolean sharedLock =
			getGrantedAccess() == NetworkFile.READONLY ? true : false;
		...
		try {
			...
			if (lock.isWholeFile()) {
				...
				nioLock =
					m_channel.tryLock(
						lock.getOffset(), lock.getLength(), sharedLock);
		}
		...
		Debug.println("** Add lock " + lock + ", cnt=" +
			getLockList().numberOfLocks());
	}
	...
	public void unlockFile(NIOFileLock lock)
		...
		if (fLock != null && fLock.getNIOLock() != null) {
			...
			Debug.println("** Remove lock " + lock + ", cnt=" +
				getLockList().numberOfLocks());
		}
	}
