...
/**
 * Protocol handler abstract base class. <p>The protocol handler class is the
 * base of all SMB protocol/dialect handler classes.
 * 
 * @author gkspencer
 */
abstract class ProtocolHandler {
	...
	protected final boolean hasChainedCommand(SMBSrvPacket pkt) {
		...
		if (cmd == PacketType.SessionSetupAndX ||
			cmd == PacketType.TreeConnectAndX || cmd == PacketType.OpenAndX ||
			cmd == PacketType.WriteAndX || cmd == PacketType.ReadAndX ||
			cmd == PacketType.LogoffAndX || cmd == PacketType.LockingAndX ||
	 ...
	 * @exception IOException
	 */
	protected final SrvDiskInfo getDiskInformation(
		DiskInterface disk, DiskDeviceContext ctx)
		throws IOException {
		...
		// check if the driver implements the
	 ...
	 * @return VolumeInfo
	 */
	protected final VolumeInfo getVolumeInformation(
		DiskInterface disk, DiskDeviceContext ctx) {
		...
		// If we did not get valid volume information from the device context
		// check if the driver implements the
		...
		// If we still have not got valid volume information then create empty
		// volume information
