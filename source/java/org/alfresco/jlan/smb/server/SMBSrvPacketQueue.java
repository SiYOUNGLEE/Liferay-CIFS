...
/**
 * SMB Packet Queue Class <p>Packet queue used for asynchronous I/O queueing.
 * 
 * @author gkspencer
 */
public class SMBSrvPacketQueue {
	...
	public class QueuedSMBPacket {
		 ...
		 * Class constructor
		 * 
		 * @param pkt SMBSrvPacket 2param offset int
		 * @param len int
		 * @param writeRaw boolean
		 */
		public QueuedSMBPacket(
			SMBSrvPacket pkt, int offset, int len, boolean writeRaw) {
			m_pkt = pkt;
			m_offset = offset;
			m_len = len;
			m_writeRaw = writeRaw;
	...
	/**
	 * Return the request at the head of the queue without removing from the
	 * queue
	 * 
	 * @return QueuedSMBPacket
	 */
