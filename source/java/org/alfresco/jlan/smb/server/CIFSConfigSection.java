...
public class CIFSConfigSection extends ConfigSection {
	...
	public static final int DefSessionTimeout = 15 * 60 * 1000; // 15 minutes,
																// milliseconds
	...
	/**
	 * Return the Win32 NetBIOS allowed client name, defaults to '*' to allow
	...
	public final int setAuthenticator(
		String authClass, ConfigElement params, int accessMode,
		boolean allowGuest)
		...
		try {
			...
			throw new InvalidConfigurationException("Authenticator class " +
				authClass + " not found");
		}
		catch (Exception ex) {
			throw new InvalidConfigurationException(
				"Authenticator class error");
		}
	...
	/**
	 * Set the SMB dialects that the server may use when negotiating a session
	 * with a client.
	 * 
	...
	public final int setHostAnnouncer(boolean b)
		...
		if (m_announce != b) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBAnnceEnable, new Boolean(b));
			m_announce = b;
		}
	...
	public final int setHostAnnounceInterval(int ival)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBAnnceInterval, new Integer(ival));
	...
	public final int setHostAnnouncerPort(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBAnncePort, new Integer(port));
		m_announcePort = port;
	...
	public final int setNameServerPort(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.NetBIOSNamePort, new Integer(port));
		m_namePort = port;
	...
	public final int setDatagramPort(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.NetBIOSDatagramPort, new Integer(port));
	...
	public final int setNetBIOSDebug(boolean ena)
		...
		if (m_nbDebug != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.NetBIOSDebugEnable, new Boolean(ena));
			m_nbDebug = ena;
		}
	...
	public final int setHostAnnounceDebug(boolean ena)
		...
		if (m_announceDebug != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBAnnceDebug, new Boolean(ena));
			m_announceDebug = ena;
		}
	...
	public final int setMacintoshExtensions(boolean ena)
		...
		if (m_macExtensions != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBMacExtEnable, new Boolean(ena));
			m_macExtensions = ena;
		}
	...
	public final int setServerType(int typ)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBServerType, new Integer(typ));
		m_srvType = typ;
	...
	public final int setSessionDebugFlags(int flags)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBSessionDebug, new Integer(flags));
		m_sessDebug = flags;
	...
	public final int setSessionPort(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.NetBIOSSessionPort, new Integer(port));
		m_sessPort = port;
	...
	public final int setNetBIOSSMB(boolean ena)
		...
		if (m_netBIOSEnable != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBNetBIOSEnable, new Boolean(ena));
			m_netBIOSEnable = ena;
		}
	...
	public final int setTcpipSMB(boolean ena)
		...
		if (m_tcpSMBEnable != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBTCPEnable, new Boolean(ena));
			m_tcpSMBEnable = ena;
		}
	...
	public final int setWin32NetBIOS(boolean ena)
		...
		if (m_win32NBEnable != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBWin32NetBIOS, new Boolean(ena));
			m_win32NBEnable = ena;
		}
	...
	public final int setWin32HostAnnouncer(boolean b)
		...
		if (m_win32NBAnnounce != b) {
			...
			sts = fireConfigurationChange(
				ConfigId.SMBWin32NBAnnounce, new Boolean(b));
			m_win32NBAnnounce = b;
		}
	...
	public final int setWin32LANA(int ival)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBWin32NBLana, new Integer(ival));
		m_win32NBLANA = ival;
	...
	public final int setWin32HostAnnounceInterval(int ival)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBWin32NBAnnounce, new Integer(ival));
	...
	/**
	 * Set the Win32 NetBIOS interface to use either Winsock NetBIOS or the
	 * Netbios() API calls
	 * 
	...
	public final int setWin32WinsockNetBIOS(boolean useWinsock)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBWin32NBWinsock, new Boolean(useWinsock));
	...
	public final int setTcpipSMBPort(int port)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBTCPPort, new Integer(port));
		m_tcpSMBPort = port;
	...
	public final int setDisableNIOCode(boolean disableNIO)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBDisableNIO, new Boolean(disableNIO));
	...
	public final int setSocketTimeout(int tmo)
		...
		int sts = fireConfigurationChange(
			ConfigId.SMBSocketTimeout, new Integer(tmo));
	...
	/**
	 * Set the native code disabled flag, to prevent calls to the Win32NetBIOS
	 * DLL
	 * 
