...
/**
 * CIFs Packet Pool Class <p>Allocates buffers from the main byte buffer pool
 * and wraps them in a CIFS specific packet.
 * 
 * @author gkspencer
 */
public class CIFSPacketPool {
	...
	// Allow over sized packet allocations, maximum over sized packet size to
	// allow
	...
	public final SMBSrvPacket allocatePacket(int reqSiz)
		...
		else if (allowsOverSizedAllocations() &&
			reqSiz <= getMaximumOverSizedAllocation()) {

			// DEBUG

			if (Debug.EnableDbg && hasDebug())
				Debug.println("[SMB] Allocating an over-sized packet, reqSiz=" +
					reqSiz);
		...
		if (buf == null) {
			...
			if (Debug.EnableDbg && hasDebug())
				Debug.println("[SMB] CIFS Packet allocate failed, reqSiz=" +
					reqSiz);
	...
	/**
	 * Allocate a CIFS packet with the specified buffer size, copy the header
	 ...
	 * @exception NoPooledMemoryException
	 */
	public final SMBSrvPacket allocatePacket(
		int reqSiz, SMBSrvPacket reqPkt, int copyLen)
		throws NoPooledMemoryException {
		...
		System.arraycopy(
			reqPkt.getBuffer(), 4, respPkt.getBuffer(), 4, copyLen == -1
				? SMBSrvPacket.HeaderLength : copyLen);
		...
		if (Debug.EnableDbg && hasDebug())
			Debug.println("[SMB]  Associated packet reqSiz=" + reqSiz +
				" with pktSiz=" + reqPkt.getBuffer().length);
	...
	public final void releasePacket(SMBSrvPacket smbPkt) {
		...
		// Check if the packet is an over sized packet, just let the garbage
		...
		if (smbPkt.getBuffer().length <= m_maxPoolBufSize) {
			...
			if (Debug.EnableDbg && hasDebug() &&
				smbPkt.hasAssociatedPacket() == false)
				Debug.println("[SMB] CIFS Packet released bufSiz=" +
					smbPkt.getBuffer().length);
		}
		else if (Debug.EnableDbg && hasDebug())
			Debug.println("[SMB] Over sized packet left for garbage collector");

		// Check if the packet has an associated packet which also needs
		// releasing
		...
		if (smbPkt.hasAssociatedPacket()) {
			...
			if (assocBuf.length <= m_maxPoolBufSize) {
				...
				m_bufferPool.releaseBuffer(
					smbPkt.getAssociatedPacket().getBuffer());
				...
				if (Debug.EnableDbg && hasDebug())
					Debug.println("[SMB] CIFS Packet released bufSiz=" +
						smbPkt.getBuffer().length +
						" and assoc packet, bufSiz=" +
						smbPkt.getAssociatedPacket().getBuffer().length);
			}
			else if (Debug.EnableDbg && hasDebug())
				Debug.println("[SMB] Over sized associated packet left for " +
						"garbage collector");
