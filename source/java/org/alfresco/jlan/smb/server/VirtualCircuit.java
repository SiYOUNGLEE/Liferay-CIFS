...
/**
 * Virtual Circuit Class <p>Represents an authenticated circuit on an SMB/CIFS
 * session. There may be multiple virtual circuits opened on a single
...
public class VirtualCircuit {
	...
	//
	// Allocated by the server and sent by the client to identify the virtual
	// circuit
	...
	public int addConnection(SharedDevice shrDev)
		...
		if (m_connections == null)
			m_connections =
				new Hashtable<Integer, TreeConnection>(DefaultConnections);
	...
	public final void closeCircuit(SrvSession sess) {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_STATE))
			sess.debugPrintln("Cleanup vc=" + getVCNumber() + ", UID=" +
				getUID() + ", searches=" + getSearchCount() + ", treeConns=" +
				getConnectionCount());
				...
				while (enm.hasMoreElements()) {
					...
					if (tree.openFileCount() > 0) {
						...
						for (int i = 0; i < tree.getFileTableLength(); i++) {
							...
							if (curFile != null &&
								devIface instanceof DiskInterface) {

								// Access the disk share interface

								DiskInterface diskIface =
									(DiskInterface) devIface;

								try {

									// Remove the file from the tree connection
									// list
