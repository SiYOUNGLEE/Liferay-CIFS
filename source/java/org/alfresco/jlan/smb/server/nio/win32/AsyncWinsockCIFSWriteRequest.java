...
/**
 * Asynchronous Winsock NIO CIFS Write Request Class <p>Holds the details of a
 * Winsock NetBIOS JNI socket based CIFS session request for processing by a
 * thread pool.
 * 
 * @author gkspencer
 */
public class AsyncWinsockCIFSWriteRequest implements ThreadRequest {
	 ...
	 */
	public AsyncWinsockCIFSWriteRequest(
		SMBSrvSession sess, int sockEvent,
	...
	public void runRequest() {
		...
		if (m_sess.isShutdown() == false &&
			...
			try {
				...
				// Get the packet handler and check if there are queued write
				// requests

				AsynchronousWritesHandler writeHandler =
					(AsynchronousWritesHandler) m_sess.getPacketHandler();

				if (writeHandler.getQueuedWriteCount() > 0) {

					Debug.println(
						"%%% Processing queued writes, queued=" +
						writeHandler.getQueuedWriteCount() + " %%%");
