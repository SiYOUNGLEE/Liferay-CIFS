...
/**
 * NIO Connections Handler Class <p>Initializes the configured CIFS session
 * handlers and listens for incoming requests using a single thread.
 * 
 * @author gkspencer
 */
public class NIOCifsConnectionsHandler
	...
	protected class IdleSessionReaper implements Runnable {
		...
		public void run() {
			...
			while (m_shutdown == false) {
				...
				Enumeration<CIFSRequestHandler> enumHandlers =
				...
				while (enumHandlers.hasMoreElements()) {
					...
					if (curHandler != null) {
						...
						if (idleCnt > 0 && Debug.EnableInfo && hasDebug())
							Debug.println("[SMB] Idle session check, removed " +
	...
	public final void initializeHandler(SMBServer srv, CIFSConfigSection config)
		...
		if (config.hasTcpipSMB()) {
			...
			ChannelSessionHandler sessHandler =
				new TcpipSMBChannelSessionHandler(
					srv, config.getSMBBindAddress(), config.getTcpipSMBPort());
			...
			try {
				...
				throw new InvalidConfigurationException(
					"Error initializing TCP-IP SMB session handler, " +
						ex.getMessage());
			}
		}
		...
		if (config.hasNetBIOSSMB()) {
			...
			ChannelSessionHandler sessHandler =
				new NetBIOSSMBChannelSessionHandler(
					srv, config.getSMBBindAddress(), config.getSessionPort());
			sessHandler.setDebug(hasDebug());
			...
			try {
				...
				throw new InvalidConfigurationException(
					"Error initializing NetBIOS SMB session handler, " +
						ex.getMessage());
			}
		}
		...
		CIFSRequestHandler reqHandler = new CIFSRequestHandler(
			m_server.getThreadPool(), SessionSocketsPerHandler,
			m_clientSocketTimeout, hasDebug());
	...
	public void run() {
		...
		try {
			...
			for (int idx = 0; idx < m_handlerList.numberOfHandlers(); idx++) {
				...
				// Get the current server socket channel and register with the
				// selector for socket accept events

				ChannelSessionHandler curHandler =
					(ChannelSessionHandler) m_handlerList.getHandlerAt(idx);

				ServerSocketChannel sockChannel = curHandler.getSocketChannel();
				sockChannel.configureBlocking(false);
				sockChannel.register(
					m_selector, SelectionKey.OP_ACCEPT, curHandler);
		...
		while (m_shutdown != true) {
			...
			Iterator<SelectionKey> keysIter =
				m_selector.selectedKeys().iterator();
			...
			while (keysIter.hasNext()) {
				...
				if (selKey.isAcceptable()) {
					...
					try {
						...
						ChannelSessionHandler channelHandler =
							(ChannelSessionHandler) selKey.attachment();
						PacketHandler pktHandler =
							channelHandler.createPacketHandler(sockChannel);
		...
		if (m_selector != null) {
			...
			try {
				...
				if (Debug.EnableError && hasDebug())
					Debug.println(
						"[SMB] Error closing socket selector, " +
						ex.getMessage());
			}
		}
	...
	private final void queueSessionToHandler(SMBSrvSession sess) {
		...
		synchronized (m_requestHandlers) {
			...
			if (reqHandler == null || 
				reqHandler.hasFreeSessionSlot() == false) {

				// Create a new session request handler and add to the head of
				// the list

				reqHandler = new CIFSRequestHandler(
					m_server.getThreadPool(), SessionSocketsPerHandler,
					m_clientSocketTimeout, hasDebug());
				reqHandler.setThreadDebug(m_threadDebug);
				reqHandler.setListener(this);
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"[SMB] Added new CIFS request handler, " +
						reqHandler);
			}
		}
