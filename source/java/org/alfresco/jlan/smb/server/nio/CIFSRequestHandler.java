...
public class CIFSRequestHandler extends RequestHandler implements Runnable {
	 ...
	 * @param debug boolean
	 */
	public CIFSRequestHandler(
		ThreadRequestPool threadPool, int maxSess, int sockTmo, boolean debug) {
	...
	public void run() {
		...
		while (m_shutdown == false) {
			...
			if (m_selector.keys().size() == 0) {
				...
				try {
					...
					if (Debug.EnableError && hasDebug() &&
						...
						Debug.println(
							"[SMB] Request handler error waiting for events");
						Debug.println(ex);
					}
				}
				catch (IOException ex) {
			...
			if (sessCnt > 0) {
				...
				// Debug.println( "[SMB] Request handler " + m_thread.getName()
				// + " session events, sessCnt=" + sessCnt + "/" +
				...
				while (keysIter.hasNext()) {
					...
					// Get the current selection key and check if has an
					// incoming request
					...
					if (selKey.isValid() == false) {
						...
						Debug.println(
							"CIFSRequestHandler: Cancelling selection key - " +
							selKey);
						selKey.cancel();
						...
						if (Debug.EnableInfo && hasDebug())
							Debug.println(
								"[SMB] NIO Selection key not valid, sess=" +
								selKey.attachment());
					}
						...
						// Switch off read events for this channel until the
						// current processing is complete

						selKey.interestOps(selKey.interestOps() &
							~SelectionKey.OP_READ);

						// Get the associated session and queue a request to the
						// thread pool to read and process the CIFS request

						SMBSrvSession sess =
							(SMBSrvSession) selKey.attachment();
						...
						// Check if there are enough thread requests to be
						// queued
						...
						if (reqList.size() >= 5) {
							...
							// if ( Debug.EnableInfo && hasDebug())
							// Debug.println( "[SMB] Queueing " + reqList.size()
							// + " thread requests");
				...
				if (reqList.size() > 0) {
					...
					// if ( Debug.EnableInfo && hasDebug()) // && reqList.size()
					// > 1)
					// Debug.println( "[SMB] Queueing " + reqList.size() +
					// " thread requests (last)");
			...
			// Check if there are any new sessions that need to be registered
			// with the selector, or sessions to be removed
			...
			if (m_sessQueue.numberOfSessions() > 0) {
				...
				while (m_sessQueue.numberOfSessions() > 0) {
					...
					if (sess != null) {
						...
						// check the session state, if the session is in a setup
						// state it is a new session
						...
						if (sess.getState() <= SMBSrvSessionState.SMBNEGOTIATE) {
							...
							if (Debug.EnableError && hasDebug())
								Debug.println(
									"[SMB] Register session with request " +
									"handler, handler=" +
									m_thread.getName() +
									", sess=" +
									sess.getUniqueId());

							// Get the socket channel from the sessions packet
							// handler

							if (sess.getPacketHandler() instanceof 
								ChannelPacketHandler) {

								// Get the channel packet handler and register
								// the socket channel with the selector

								ChannelPacketHandler chanPktHandler =
									(ChannelPacketHandler) sess
									.getPacketHandler();
								SocketChannel sessChannel =
									chanPktHandler.getSocketChannel();

								try {

									// Register the session channel with the
									// selector

									sessChannel.configureBlocking(false);
									sessChannel.register(
										m_selector, SelectionKey.OP_READ, sess);
								}
									...
									if (Debug.EnableError && hasDebug())
										Debug.println(
											"[SMB] Failed to register session" +
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[SMB] Closed CIFS request handler, " +
				m_thread.getName());
	}
				...
				if (sess != null && sess.getLastIOTime() < checkTime) {
					...
					if (Debug.EnableInfo && hasDebug())
						Debug.println(
							...
							sess.getUniqueId() + ", addr=" +
							sess.getRemoteAddress().getHostAddress());
