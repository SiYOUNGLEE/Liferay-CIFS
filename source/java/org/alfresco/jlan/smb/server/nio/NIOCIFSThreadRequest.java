...
/**
 * NIO CIFS Thread Request Class <p>Holds the details of an NIO channel based
 * CIFS session request for processing by a thread pool.
 * 
 * @author gkspencer
 */
		...
		if (m_sess.isShutdown() == false) {
			...
			try {
				...
				if (smbPkt == null) {
					...
					if (Debug.EnableInfo &&
						m_sess.hasDebug(SMBSrvSession.DBG_SOCKET))
						Debug.println(
							"Received null packet, closing session sess=" +
							m_sess.getUniqueId() +
							", addr=" +
					...
					m_selectionKey.interestOps(m_selectionKey.interestOps() |
				...
				if (Debug.EnableInfo &&
					m_sess.hasDebug(SMBSrvSession.DBG_SOCKET))
					Debug.println(
						"Error during packet receive, closing session sess=" +
						m_sess.getUniqueId() +
						", addr=" +
						m_sess.getRemoteAddress().getHostAddress() +
						" ex=" +
