...
/**
 * Asynchronous Winsock NetBIOS Session Handler Class <p>
 * 
 * @author gkspencer
 */
public class AsyncWinsockNetBIOSSessionHandler extends SessionHandlerBase {
	 ...
	 */
	public AsyncWinsockNetBIOSSessionHandler(
		int lana, NetBIOSName nbName, NetworkServer server) {
	...
	public void initializeSessionHandler(NetworkServer server)
		...
		if (lanas.length > 0) {
			...
			if (m_lana == -1)
				...
				// If the LANA is not available then exit with an exception for
				// now

				if (lanaOnline == false)
					throw new IOException("LANA " + getLANA() +
						" is not online");
			}
		}
		else {
	...
	public PacketHandler createPacketHandler(NetBIOSSocket clientSock)
		...
		return new WinsockNetBIOSPacketHandler(
			m_lana, clientSock, getSMBServer().getPacketPool(), true);
	}
