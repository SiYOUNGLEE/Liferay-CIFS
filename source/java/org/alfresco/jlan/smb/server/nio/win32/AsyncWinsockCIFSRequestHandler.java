...
/**
 * CIFS Request Handler Class <p>Handles the receiving of CIFS requests for a
 * number of CIFS sessions.
 * 
 * @author gkspencer
 */
public class AsyncWinsockCIFSRequestHandler extends RequestHandler
	implements Runnable {
	...
	// NetBIOS name that the clients are connecting to, required to make a
	// loopback connection
	...
	// Socket event arrays
	//
	// The first element in the active array is used to wakeup the main request
	// handler thread.
	 ...
	 * @param sockTmo int
	 */
	public AsyncWinsockCIFSRequestHandler(
		NetBIOSName srvName, int srvLANA, ThreadRequestPool threadPool,
		int maxSess, int sockTmo) {
	...
	public final boolean hasFreeSessionSlot() {
		return (getCurrentSessionCount() + m_sessQueue.numberOfSessions()) <
	...
	public void run() {
		...
		try {
			...
			// Create the event used to wakeup the request handler thread to
			// add/remove sessions
			...
			if (Debug.EnableError && hasDebug()) {
				Debug.println(
					"[SMB] Failed to initialize wakeup event " +
					m_thread.getName());
				Debug.println(ex);
			}
		...
		while (m_shutdown == false) {
			...
			if (m_activeEventsLen == 1 && m_eventTable.size() == 0) {
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"[SMB] Request handler " +
						m_thread.getName() + " waiting for session ...");
			}
			...
			try {
				...
				eventIdx =
					Win32NetBIOS.WinsockWaitForMultipleEvents(
						m_activeEventsLen, m_win32ActiveEvents, false,
						Winsock2.WSA_INFINITE, false);
			}
			...
			if (eventIdx == 0) {
				...
				// if ( Debug.EnableInfo && hasDebug())
				// Debug.println( "[SMB] Wakeup event, active=" +
				// m_activeEventsLen);
				...
				if (m_requeueLen > 0) {
					...
					// if ( Debug.EnableDbg && hasDebug())
					// Debug.println("[SMB] Requeue events to active list, requeueLen="
					// + m_requeueLen);
					...
					synchronized (m_requeueEvents) {
						...
						for (int i = 0; i < m_requeueLen; i++) {
							...
							WinsockNetBIOSPacketHandler winsockPktHandler =
								(WinsockNetBIOSPacketHandler) sess
								.getPacketHandler();
							NetBIOSSocket nbSocket =
								winsockPktHandler.getSocket();
							...
							try {
								...
								int evtEnum =
									Win32NetBIOS.WinsockEnumNetworkEvents(
										nbSocket.getSocket(), 0);
								if (evtEnum != 0)
									Debug.println(
										"[SMB] Requeue event, evtEnum=0x" +
										Integer.toHexString(evtEnum));

								// Reset the events to be reported for the
								// socket

								Win32NetBIOS.WinsockEventSelect(
									nbSocket.getSocket(), m_requeueEvents[i],
									Winsock2.FD_READ | Winsock2.FD_WRITE |
										Winsock2.FD_CLOSE);
							}
							catch (Exception ex) {
								Debug.println(
									"[SMB] Error re-enabling FD_READ events " +
									"sess=" +
									sess.getUniqueId());
								Debug.println(ex);
							}
				...
				while (m_sessQueue.numberOfSessions() > 0) {
					...
					SMBSrvSession sess =
						(SMBSrvSession) m_sessQueue.removeSessionNoWait();
					...
					if (sess != null) {
						...
						if (Debug.EnableError && hasDebug())
							Debug.println(
								"[SMB] Register session with request handler," +
								" handler=" +
								m_thread.getName() +
								", sess=" +
								sess.getUniqueId());

						// Get the NetBIOS socket from the sessions packet
						// handler

						if (sess.getPacketHandler() instanceof 
										WinsockNetBIOSPacketHandler) {

							// Get the channel packet handler and create a
							// socket event for the new session
							...
							try {
								...
								Win32NetBIOS.setSocketReceiveBufferSize(
									nbSocket.getSocket(), 128000);
								Win32NetBIOS.setSocketSendBufferSize(
									nbSocket.getSocket(), 256000);

								// Enable the required socket events for the
								// session socket

								nbSocket.configureBlocking(false);
								Win32NetBIOS.WinsockEventSelect(
									nbSocket.getSocket(), sockEvent,
									Winsock2.FD_READ | Winsock2.FD_WRITE |
										Winsock2.FD_CLOSE);
								...
								m_win32ActiveEvents[m_activeEventsLen++] =
									sockEvent;
							}
								...
								if (Debug.EnableError && hasDebug()) {
									Debug.println(
										"[SMB] Failed to initialize socket " +
										"event for new session");
									Debug.println(ex);
								}
							}
						}
		...
		if (m_eventTable.size() > 0) {
			...
			while (eventIds.hasMoreElements()) {
				...
				if (sess != null) {
					...
					if (Debug.EnableDbg && hasDebug())
						Debug.println("[SMB] Closing session " +
							sess.getUniqueId() + ", event=" + eventId);
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println("[SMB] Closed CIFS request handler, " +
				m_thread.getName());
	}
	...
	protected final int checkForIdleSessions() {
		...
		if (m_eventTable.size() > 0) {
			...
			long checkTime =
				System.currentTimeMillis() - (long) m_clientSocketTimeout;
	...
	protected void wakeupRequestHandler() {
		...
		if (Win32NetBIOS.Win32SetEvent(m_win32ActiveEvents[0]) == false) {
			...
			if (Debug.EnableDbg && hasDebug())
				Debug.println(
					"[SMB] Failed to wakeup request handler, " +
					m_thread.getName());
		}
	}
	...
	protected void requeueSocketEvent(int sockEvent, int sockPtr) {
		...
		/**
		 * // Re-enable read events for the socket try {
		 * Win32NetBIOS.WinsockEventSelect( sockPtr, sockEvent, Winsock2.FD_READ
		 * | Winsock2.FD_WRITE | Winsock2.FD_CLOSE); } catch (
		 * WinsockNetBIOSException ex) { // DEBUG if ( Debug.EnableDbg &&
		 * hasDebug()) {
		...
		if (Win32NetBIOS.Win32SetEvent(m_win32ActiveEvents[0]) == false) {
			...
			if (Debug.EnableDbg && hasDebug())
				Debug.println(
					"[SMB] Failed to wakeup request handler (requeue), " +
					m_thread.getName());
		}
	}
	...
	private void processSocketEvent(int eventId) {
		...
		WinsockNetBIOSPacketHandler winsockPktHandler =
			(WinsockNetBIOSPacketHandler) sess.getPacketHandler();
		...
		try {
			...
			triggeredEvent =
				Win32NetBIOS.WinsockEnumNetworkEvents(nbSocket.getSocket(), 0);
		}
		catch (WinsockNetBIOSException ex) {
			...
			if (Debug.EnableDbg && hasDebug()) {
				Debug.println(
					"[SMB] Socket error event, sess=" +
					sess.getUniqueId() + ", eventId=" + eventId);
				Debug.println(ex);
			}
		...
		// if ( Debug.EnableDbg && hasDebug())
		// Debug.println("[SMB] Triggered id=" + eventId + ", sess=" +
		// sess.getUniqueId() + ", event=0x" + Integer.toHexString(
		// triggeredEvent));
		...
		if ((triggeredEvent & Winsock2.FD_CLOSE) != 0) {
			...
			while (m_win32ActiveEvents[eventIdx] != eventId &&
				eventIdx < m_activeEventsLen)
				eventIdx++;

			if (eventIdx == m_activeEventsLen)
				return;

			// Remove the socket event from the active list, and shorten the
			// list

			m_win32ActiveEvents[eventIdx] =
				m_win32ActiveEvents[m_activeEventsLen--];
		...
		if ((triggeredEvent & Winsock2.FD_READ) != 0) {
			...
			/**
			 * try { Win32NetBIOS.WinsockEventSelect( nbSocket.getSocket(),
			 * eventId, Winsock2.FD_WRITE | Winsock2.FD_CLOSE); } catch (
			 * WinsockNetBIOSException ex) { // DEBUG if ( Debug.EnableDbg &&
			 * hasDebug()) {
			...
			if (sess.hasReadInProgress() == false) {
				...
				AsyncWinsockCIFSReadRequest threadReq =
					new AsyncWinsockCIFSReadRequest(
						sess, eventIdKey.intValue(), this);
		...
		if ((triggeredEvent & Winsock2.FD_WRITE) != 0) {
			...
			AsynchronousWritesHandler writesHandler =
				(AsynchronousWritesHandler) winsockPktHandler;
			if (writesHandler.getQueuedWriteCount() > 0) {
				...
				if (Debug.EnableDbg && hasDebug())
					Debug.println(
						"[SMB] Submit queued writes for processing, sess=" +
						sess.getUniqueId());

				// Queue the packet handler to the thread pool for processing

				AsyncWinsockCIFSWriteRequest threadReq =
					new AsyncWinsockCIFSWriteRequest(
						sess, eventIdKey.intValue(), this);
				m_threadPool.queueRequest(threadReq);
			}
		}
