...
/**
 * Asynchronous Winsock NIO Connections Handler Class <p>Initializes the
 * configured CIFS session handlers and listens for incoming requests using a
 * single thread.
 * 
 * @author gkspencer
 */
public class AsyncWinsockCifsConnectionsHandler
	implements CifsConnectionsHandler, RequestHandlerListener, Runnable {
	...
	// File server and workstation NetBIOS names to listen for incoming
	// connections on
	...
	//
	// Each handler processes the socket read events for a number of session
	// socket channels
	...
	/**
	 * Idle Session Reaper Thread Class <p>Check for sessions that have no
	 * recent I/O requests. The session timeout is configurable.
	 */
	protected class IdleSessionReaper implements Runnable {
		...
		public void run() {
			...
			while (m_shutdown == false) {
				...
				Enumeration<AsyncWinsockCIFSRequestHandler> enumHandlers =
					m_requestHandlers.elements();

				while (enumHandlers.hasMoreElements()) {

					// Get the current request handler and check for idle
					// session

					AsyncWinsockCIFSRequestHandler curHandler =
						enumHandlers.nextElement();
					if (curHandler != null) {
						...
						if (idleCnt > 0 && Debug.EnableInfo && hasDebug())
							Debug.println(
								"[SMB] Idle session check, removed " +
								idleCnt + " sessions for " +
								curHandler.getName());
					}
				}
			}
	...
	public final void initializeHandler(SMBServer srv, CIFSConfigSection config)
		...
		if (config.hasWin32NetBIOS()) {
			...
			// Create the local NetBIOS names to listen for incoming connections
			// on

			m_srvNbName = new NetBIOSName(
				srvName, NetBIOSName.FileServer, false);
			m_wksNbName = new NetBIOSName(
				srvName, NetBIOSName.WorkStation, false);

			// Create the session handlers to listen for incoming requests

			try {

				// Get the NetBIOS LANA to use, or -1 if the first available
				// should be used
				...
				AsyncWinsockNetBIOSSessionHandler sessHandler =
					new AsyncWinsockNetBIOSSessionHandler(
						lana, m_srvNbName, srv);
				...
				sessHandler = new AsyncWinsockNetBIOSSessionHandler(
					lana, m_wksNbName, srv);
				sessHandler.initializeSessionHandler(m_server);
				m_handlerList.addHandler(sessHandler);
			}
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"[SMB] Error initializing session handler, " +
						ex.getMessage());
		...
		if (m_handlerList.numberOfHandlers() == 0)
			throw new InvalidConfigurationException(
				"No CIFS session handlers enabled");
		...
		AsyncWinsockCIFSRequestHandler reqHandler =
			new AsyncWinsockCIFSRequestHandler(
				m_srvNbName, m_srvLANA, m_server.getThreadPool(),
				SessionSocketsPerHandler, m_clientSocketTimeout);
	...
	public final void stopHandler() {
		...
		if (m_thread != null) {
			...
			// Close the first session handler socket to wakeup the listener
			// thread
			...
			if (m_handlerList.numberOfHandlers() > 0) {
				...
				AsyncWinsockNetBIOSSessionHandler sessHandler =
					(AsyncWinsockNetBIOSSessionHandler) m_handlerList
					.getHandlerAt(0);
	...
	public void run() {
		...
		try {
			...
			for (int idx = 0; idx < m_handlerList.numberOfHandlers(); idx++) {
				...
				// Get the current Winsock NetBIOS server socket and register
				// with the selector for socket accept events

				AsyncWinsockNetBIOSSessionHandler curHandler =
					(AsyncWinsockNetBIOSSessionHandler) m_handlerList
					.getHandlerAt(idx);
				...
				srvSocket.register(
					m_nbSelector, NetBIOSSelectionKey.OP_ACCEPT, curHandler);

				// DEBUG

				if (Debug.EnableInfo && hasDebug())
					Debug.println("[SMB] Listening for connections on " +
						curHandler);
			}
		}
		catch (IOException ex) {
		...
		while (m_shutdown != true) {
			...
			Iterator<NetBIOSSelectionKey> keysIter =
				m_nbSelector.selectedKeys().iterator();

			while (keysIter.hasNext()) {

				// Get the current selection key and check if there is an
				// incoming connection
				...
				if (selKey.isAcceptable()) {
					...
					try {
						...
						// Get the listening server socket, accept the new
						// client connection

						AsyncWinsockNetBIOSSessionHandler channelHandler =
							(AsyncWinsockNetBIOSSessionHandler) selKey
							.attachment();
						NetBIOSSocket clientSock =
							channelHandler.getSocket().accept();
						...
						if (channelHandler.getNetBIOSName().getType() == 
							...
							PacketHandler pktHandler =
								channelHandler.createPacketHandler(clientSock);

							// Create the new session

							SMBSrvSession sess =
								SMBSrvSession.createSession(
									pktHandler, m_server, ++m_sessId);
		...
		while (m_requestHandlers.size() > 0) {
			...
			AsyncWinsockCIFSRequestHandler reqHandler =
				m_requestHandlers.remove(0);
			reqHandler.closeHandler();

			// DEBUG

			if (Debug.EnableInfo && hasDebug())
				Debug.println("[SMB] Closed request handler, " +
					reqHandler.getName());
		}
		...
		if (m_nbSelector != null) {
			...
			try {
				...
				if (Debug.EnableError && hasDebug())
					Debug.println("[SMB] Error closing socket selector, " +
						ex.getMessage());
			}
		}
	...
	private final void queueSessionToHandler(SMBSrvSession sess) {
		...
		AsyncWinsockCIFSRequestHandler reqHandler =
			m_requestHandlers.firstElement();

		if (reqHandler == null || reqHandler.hasFreeSessionSlot() == false) {

			// Create a new session request handler and add to the head of the
			// list
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println("[SMB] Added new CIFS request handler, " +
					reqHandler);
		}
	...
	public void requestHandlerEmpty(RequestHandler reqHandler) {
		...
		synchronized (m_handlerList) {
			...
			// Check if the request handler is the current head of the handler
			// list, if not then we can close
			// this request handler

			if (m_requestHandlers.get(0).getName().equals(
				reqHandler.getName()) == false) {
				...
				if (Debug.EnableInfo && hasDebug())
					Debug.println(
						"[SMB] Removed empty request handler, " +
						reqHandler.getName());
			}
		}
	}
