...
public class TcpipSMBSessionSocketHandler extends SocketSessionHandler {
	...
	private static final ThreadGroup TcpipSMBGroup =
		new ThreadGroup("TcpipSMBSessions");
	 ...
	 * @param debug boolean
	 */
	public TcpipSMBSessionSocketHandler(
	...
	protected void acceptConnection(Socket sock) {
		...
		try {
			...
			PacketHandler pktHandler =
				new TcpipSMBPacketHandler(sock, smbServer.getPacketPool());
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println("[SMB] TCP-SMB Failed to create session, " +
					ex.toString());
		}
	}
	...
	public final static void createSessionHandlers(
		...
		SocketSessionHandler sessHandler =
			new TcpipSMBSessionSocketHandler(
				server, cifsConfig.getTcpipSMBPort(),
