...
public class NotifyRequestList {
	...
	/**
	 * Return the global filter mask, generated by combining all of the pending
	 * notify request filters
	 * 
	 * @return int
	 */
	 ...
	 * @return NotifyRequest
	 */
	public final synchronized NotifyRequest findRequest(
		...
		for (int i = 0; i < m_requests.size(); i++) {
			...
			if (curReq.getMultiplexId() == mid && curReq.getTreeId() == tid &&
				curReq.getUserId() == uid && curReq.getProcessId() == pid) {
	...
	public final synchronized NotifyRequest findRequest(
		...
		for (int i = 0; i < m_requests.size(); i++) {
			...
			NotifyRequest curReq = m_requests.get(i);
			if (curReq.getDirectory() == dir && curReq.getFilter() == filter &&
	 ...
	 */
	public final synchronized void removeAllRequestsForSession(
		SMBSrvSession sess) {
