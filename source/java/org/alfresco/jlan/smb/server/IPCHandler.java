...
/**
 * <p> The IPCHandler class processes requests made on the IPC$ remote admin
 * pipe. The code is shared amongst different SMB protocol handlers.
 * 
 * @author gkspencer
 */
class IPCHandler {
	...
	public static void processIPCRequest(SMBSrvSession sess, SMBSrvPacket smbPkt)
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("IPC$ Request [" + smbPkt.getTreeId() +
				"] - cmd = " + smbPkt.getPacketTypeString());
	...
	protected static void procTransaction(
		VirtualCircuit vc, SrvTransactBuffer tbuf, SMBSrvSession sess,
		SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("IPC$ Transaction  pipe=" + tbuf.getName() +
				", subCmd=" +
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileOpen(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileRead(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileReadAndX(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileWrite(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileWriteAndX(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procIPCFileClose(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("IPC$ File close [" + smbPkt.getTreeId() +
				"] fid=" + fid);
	...
	protected static void procSetNamedPipeHandleState(
		SMBSrvSession sess, VirtualCircuit vc, SrvTransactBuffer tbuf,
		SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("  SetNmPHandState pipe=" + netFile.getName() +
				", fid=" + fid + ", state=0x" + Integer.toHexString(state));
	 ...
	 * @exception SMBSrvException
	 */
	protected static void procNTCreateAndX(
		SMBSrvSession sess, SMBSrvPacket smbPkt)
		throws IOException, SMBSrvException {
		...
		String fileName =
			DataPacker.getUnicodeString(smbPkt.getBuffer(), pos, nameLen);
		if (fileName == null) {
			sess.sendErrorResponseSMB(
				smbPkt, SMBStatus.NTInvalidParameter, SMBStatus.NTErr);
			return;
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("NT Create AndX [" + smbPkt.getTreeId() +
				"] name=" + fileName + ", flags=0x" +
				Integer.toHexString(flags) + ", attr=0x" +
		...
		if (DCEPipeHandler.getHandlerForType(pipeType) == null) {
			sess.sendErrorResponseSMB(
				smbPkt, SMBStatus.NTAccessDenied, SMBStatus.NTErr);
			return;
		}
		...
		try {
			...
			sess.sendErrorResponseSMB(
				smbPkt, SMBStatus.Win32InvalidHandle, SMBStatus.NTErr);
			return;
		}
	...
	protected static final void procTrans2QueryFile(
		...
		if (Debug.EnableInfo && sess.hasDebug(SMBSrvSession.DBG_IPC))
			sess.debugPrintln("IPC$ Query File - level=0x" +
		...
		try {
			...
			FileInfo fileInfo =
				new FileInfo(
					netFile.getName(), netFile.getFileSize(),
					netFile.getFileAttributes());
			...
			// Set the file allocation size, looks like it is used as the pipe
			// buffer size
