...
/**
 * SMB error text class. <p> The SMBErrorText is a static class that converts
 * SMB error class/error codes into their appropriate error message strings. The
 * class is used by the SMBException class when outputting an SMB exception as a
 * string. <p> SMB error classes and error codes are declared in the SMBStatus
 * class.
 * 
 * @author gkspencer
 */
public final class SMBErrorText {
	...
	public final static String ErrorString(int errclass, int errcode) {
		...
		if (errtext == null)
			errtext =
				"[Unknown error status/class: " + errclass + "," + errcode +
					"]";
	...
	private static String DOSErrorText(int errcode) {
		...
		case 1:
			errtext =
				"Invalid function. Server did not recognize/perform system call";
			break;
		case 2:
	...
	private final static String NetworkErrorText(int errcode) {
		...
		case SMBStatus.NETCannotPerformOp:
			errtext =
				"Operation cannot be performed on the print job in it's current state";
			break;
		...
		case SMBStatus.NETLogonSrvOldSoftware:
			errtext =
				"Logon server is running old software version, cannot validate logon";
			break;
