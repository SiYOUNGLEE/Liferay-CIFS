...
/**
 * Default RPC Authenticator Class <p>RPC authenticator implementation that
 * allows any client to access the RPC servers.
 * 
 * @author gkspencer
 */
public class DefaultRpcAuthenticator implements RpcAuthenticator {
	...
	private int[] _authTypes = {
		AuthType.Null, AuthType.Unix
	};
	...
	public Object authenticateRpcClient(int authType, RpcPacket rpc)
		...
		case AuthType.Unix:
			...
			sessKey = new Long(
				(((long) rpc.getClientAddress().hashCode()) << 32) +
				(gid << 16) + uid);
			break;
		}
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"RpcAuth: RPC from " + rpc.getClientDetails() +
	...
	public ClientInfo getRpcClientInformation(Object sessKey, RpcPacket rpc) {
		...
		case AuthType.Null:
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					...
					AuthType.getTypeAsString(authType) + ", addr=" +
		...
		case AuthType.Unix:
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"RpcAuth: Client info, type=" +
					AuthType.getTypeAsString(authType) + ", name=" +
					cInfo.getClientAddress() + ", uid=" + cInfo.getUid() +
					", gid=" + cInfo.getGid() + ", groups=" + grpLen);
			break;
		}
