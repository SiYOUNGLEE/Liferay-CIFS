...
/**
 * ONC/RPC Request Thread Pool Class <p>Processes RPC requests using a pool of
 * worker threads.
 * 
 * @author gkspencer
 */
public class RpcRequestThreadPool {
			...
			while (mi_shutdown == false) {
				...
				if (rpc != null) {
					...
					try {
						...
						if (response != null)
							response.getPacketHandler().sendRpcResponse(
								response);
					}
					catch (Throwable ex) {
						...
						if (rpc.getClientProtocol() == Rpc.TCP &&
							rpc.isAllocatedFromPool())
							rpc.getOwnerPacketPool().releasePacket(rpc);

						if (response != null &&
							response.getClientProtocol() == Rpc.TCP &&
							response.getBuffer() != rpc.getBuffer() &&
							response.isAllocatedFromPool())
							response.getOwnerPacketPool().releasePacket(
								response);
	 ...
	 * @param rpcServer RpcProcessor
	 */
	public RpcRequestThreadPool(
		String threadName, int poolSize, RpcProcessor rpcServer) {
