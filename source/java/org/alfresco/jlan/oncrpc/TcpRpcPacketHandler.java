...
/**
 * TCP RPC Packet Handler Class <p>Processes RPC requests received via TCP
 * session.
 * 
 * @author gkspencer
 */
public class TcpRpcPacketHandler extends SocketPacketHandler
	implements Runnable {
	 ...
	 * @throws IOException
	 */
	public TcpRpcPacketHandler(
		TcpRpcSessionHandler handler, int sessId, RpcProcessor server,
		Socket socket, int maxRpcSize)
		throws IOException {
	...
	protected final void sendRpc(RpcPacket rpc)
		...
		//
		// If the fragment header is written seperately to the main RPC response
		// packet trace tools
	...
	protected final int receiveRpc(RpcPacket rpc)
		...
		int rxLen = receiveRpc(
			rpc.getBuffer(), RpcPacket.FragHeaderLen,
			rpc.getBuffer().length - RpcPacket.FragHeaderLen);
		if (rxLen > 0) {

			// Set the received length

			rpc.setBuffer(
				RpcPacket.FragHeaderLen, rxLen + RpcPacket.FragHeaderLen);

			// Set the client details

			rpc.setClientDetails(
				getSocket().getInetAddress(), getSocket().getPort(), Rpc.TCP);
		}
		...
		while (lastFrag == false) {
			...
			if (fragLen > (buffer.length - rxOffset))
				throw new IOException(
					"Receive RPC buffer overflow, fragment len = " + fragLen);

			// Read the data part of the packet into the users buffer, this may
			// take
			// several reads
