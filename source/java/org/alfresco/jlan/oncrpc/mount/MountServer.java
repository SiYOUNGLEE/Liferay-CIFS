...
/**
 * Mount Server Class <p>Contains the NFS mount server.
 * 
 * @author gkspencer
 */
public class MountServer extends RpcNetworkServer implements RpcProcessor {
	...
	public void startServer() {
		...
		try {
			...
			m_udpHandler = new UdpRpcDatagramHandler(
				"Mountd", "Mnt", this, this, null, getPort(),
				MaxRequestSize);
			...
			m_tcpHandler = new TcpRpcSessionHandler(
				"Mountd", "Mnt", this, this, null, getPort(),
				MaxRequestSize);
			...
			mappings[0] = new PortMapping(
				Mount.ProgramId, Mount.VersionId1, Rpc.UDP,
				m_udpHandler.getPort());
			mappings[1] = new PortMapping(
				Mount.ProgramId, Mount.VersionId3, Rpc.UDP,
				m_udpHandler.getPort());
			mappings[2] = new PortMapping(
				Mount.ProgramId, Mount.VersionId1, Rpc.TCP,
				m_tcpHandler.getPort());
			mappings[3] = new PortMapping(
				Mount.ProgramId, Mount.VersionId3, Rpc.TCP,
				m_tcpHandler.getPort());
	...
	public RpcPacket processRpc(RpcPacket rpc)
		...
		if (rpc.getProgramId() != Mount.ProgramId) {
			...
			rpc.buildProgramMismatchResponse(
				Mount.VersionId1, Mount.VersionId3);
			return rpc;
		}
	 ...
	 * @return RpcPacket
	 */
	private final RpcPacket procMount(
		NFSSrvSession sess, RpcPacket rpc, int version) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[Mount] Mount request from " +
				rpc.getClientDetails() + " path=" + mountPath);
		...
		if (version == 1) {
			...
			rpc.packIntArrayWithLength(
				getNFSConfiguration().getRpcAuthenticator()
	 ...
	 * @return RpcPacket
	 */
	private final RpcPacket procDump(
		NFSSrvSession sess, RpcPacket rpc, int version) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[Mount] Dump request from " + rpc.getClientDetails());
	 ...
	 * @return RpcPacket
	 */
	private final RpcPacket procUnMount(
		NFSSrvSession sess, RpcPacket rpc, int version) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[Mount] UnMount request from " + rpc.getClientDetails() + 
				" path=" + mountPath);
	 ...
	 * @return RpcPacket
	 */
	private final RpcPacket procUnMountAll(
		NFSSrvSession sess, RpcPacket rpc, int version) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[Mount] UnMountAll request from " + rpc.getClientDetails());
	 ...
	 * @return RpcPacket
	 */
	private final RpcPacket procExport(
		NFSSrvSession sess, RpcPacket rpc, int version) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[Mount] Export request from " + rpc.getClientDetails());

		// Get the share list from the server

		SharedDeviceList shareList =
			sess.getServer().getShareMapper().getShareList(
				getConfiguration().getServerName(), sess, false);
		...
		if (sess.getServer().hasAccessControlManager()) {
			...
			AccessControlManager aclMgr =
				sess.getServer().getAccessControlManager();
	...
	private final NFSSrvSession createTemporarySession(RpcPacket rpc)
		...
		Object sessKey = rpcAuth.authenticateRpcClient(
			rpc.getCredentialsType(), rpc);
	 ...
	 * @return int
	 */
	protected final int mountPath(
		NFSSrvSession sess, String path, byte[] handle) {
		...
		if (path.startsWith(UNIX_SEPERATOR) && path.length() >= 2) {
			...
			try {
				share = getShareMapper().findShare(
			...
			if (share != null) {
				...
				if (getSecurityConfiguration().hasAccessControlManager()) {
					...
					AccessControlManager aclMgr =
						getSecurityConfiguration().getAccessControlManager();

					if (aclMgr.checkAccessControl(sess, share) == AccessControl
									.NoAccess) {
				...
				if (extraPath != null) {
					...
					if (extraPath.endsWith(DOS_SEPERATOR))
						extraPath = extraPath.substring(
							0, extraPath.length() - 2);
					...
					try {
						...
						TreeConnection conn =
							m_connections.findConnection(shareId);
						DiskInterface disk =
							(DiskInterface) conn.getSharedDevice()
							.getInterface();
						...
						NFSHandle.packDirectoryHandle(
							shareId, finfo.getFileId(), handle);
					}
					catch (Exception ex) {
					}
				}
