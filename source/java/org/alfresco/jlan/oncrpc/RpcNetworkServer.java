...
/**
 * RPC Network Server Abstract Class <p>Provides the base class for RPC servers
 * (such as mount and NFS).
 * 
 * @author gkspencer
 */
public abstract class RpcNetworkServer extends NetworkServer
	...
	protected final void registerRPCServer(PortMapping[] mappings)
		...
		TcpRpcClient rpcClient =
			new TcpRpcClient(localHost, PortMapper.DefaultPort, 512);
		...
		// Loop through the port mappings and register each port with the
		// portmapper service
		...
		for (int i = 0; i < mappings.length; i++) {
			...
			setPortRpc.buildRequestHeader(
				PortMapper.ProgramId, PortMapper.VersionId, PortMapper.ProcSet,
				0, null, 0, null);
			...
			// Debug.println("[" + getProtocolName() + "] Register server RPC "
			// + setPortRpc.toString());
			...
			// if ( Debug.EnableInfo && hasDebug())
			// Debug.println("[" + getProtocolName() + "] Register response " +
			// rxRpc.toString());
		}
	}
	...
	protected final void unregisterRPCServer(PortMapping[] mappings)
		...
		TcpRpcClient rpcClient = new TcpRpcClient(
			localHost, PortMapper.DefaultPort, 512);
		...
		// Loop through the port mappings and unregister each port with the
		// portmapper service
		...
		for (int i = 0; i < mappings.length; i++) {
			...
			setPortRpc.buildRequestHeader(
				PortMapper.ProgramId, PortMapper.VersionId,
				PortMapper.ProcUnSet, 0, null, 0, null);
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"[" + getProtocolName() +
					"] UnRegister server RPC " + setPortRpc.toString());
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"[" + getProtocolName() +
					"] UnRegister response " + rxRpc.toString());
		}
	}
