...
public class RpcPacket {
	...
	public final int getVerifierType() {
		return DataPacker.getInt(
			m_buffer, m_offset + getCredentialsLength() + 32);
	}
	...
	public final int getVerifierLength() {
		return DataPacker.getInt(
			m_buffer, m_offset + getCredentialsLength() + 36);
	}
	 ...
	 * @param protocol int
	 */
	public final void setClientDetails(
		InetAddress addr, int port, int protocol) {
	...
	public final void setVerifierType(int verftype) {
		DataPacker.putInt(
			verftype, m_buffer, m_offset + getCredentialsLength() + 32);
	}
	...
	public final void setVerifierLength(int verflen) {
		DataPacker.putInt(
			verflen, m_buffer, m_offset + getCredentialsLength() + 36);
	}
	 ...
	 * @param verf byte[]
	 */
	public final void buildRequestHeader(
		int progId, int verId, int procId, int credType, byte[] cred,
	...
	public final void buildResponseHeader() {
		...
		if (verfLen > 0)
			System.arraycopy(
				m_buffer, getVerifierOffset(), m_buffer, 
				m_offset + 20, verfLen);
	...
	public final void buildErrorResponse(int stsCode) {
		...
		if (isReply == false) {
			...
			if (verfLen > 0)
				System.arraycopy(
					m_buffer, getVerifierOffset(), m_buffer, m_offset + 20,
					verfLen);
		}
		else {
