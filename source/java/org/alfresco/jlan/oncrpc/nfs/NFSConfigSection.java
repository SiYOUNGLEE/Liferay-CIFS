...
public class NFSConfigSection extends ConfigSection {
	...
	public final int setNFSPortMapper(boolean ena)
		...
		if (m_nfsPortMapper != ena) {
			...
			sts =
				fireConfigurationChange(ConfigId.NFSPortMapEnable, new Boolean(
					ena));
			m_nfsPortMapper = ena;
		}
	...
	public final int setNFSDebug(int dbg)
		...
		int sts = fireConfigurationChange(
			ConfigId.NFSDebugFlags, new Integer(dbg));
		m_nfsDebug = dbg;
	...
	public final int setPortMapperPort(int port)
		...
		int sts =
			fireConfigurationChange(ConfigId.NFSPortMapPort, new Integer(port));
	...
	public final int setMountServerPort(int port)
		...
		int sts =
			fireConfigurationChange(ConfigId.NFSMountPort, new Integer(port));
	...
	public final int setNFSServerPort(int port)
		...
		int sts =
			fireConfigurationChange(ConfigId.NFSServerPort, new Integer(port));
	...
	public final int setNFSThreadPoolSize(int poolSize)
		...
		int sts =
			fireConfigurationChange(ConfigId.NFSThreads, new Integer(poolSize));
	...
	public final int setNFSPacketPoolSize(int poolSize)
		...
		int sts = fireConfigurationChange(
			ConfigId.NFSPacketPool, new Integer(poolSize));
	...
	public final int setPortMapperDebug(boolean dbg)
		...
		if (m_portMapDebug != dbg) {
			...
			sts = fireConfigurationChange(
				ConfigId.NFSPortMapDebug, new Boolean(dbg));
			m_portMapDebug = dbg;
		}
	...
	public final int setMountServerDebug(boolean dbg)
		...
		if (m_portMapDebug != dbg) {
			...
			sts = fireConfigurationChange(
				ConfigId.NFSMountDebug, new Boolean(dbg));
		...
		try {
			...
			if (authObj instanceof RpcAuthenticator) {
				...
				throw new InvalidConfigurationException(
					"RPC Authenticator is not derived from required " +
					"base class");
		}
		catch (ClassNotFoundException ex) {
			throw new InvalidConfigurationException(
				"RPC Authenticator class " + authClass + " not found");
		}
		catch (Exception ex) {
			throw new InvalidConfigurationException(
				"RPC Authenticator class error");
		}
	...
	public final int setNFSFileCacheIOTimer(long ioTimer)
		...
		int sts = fireConfigurationChange(
			ConfigId.NFSFileCacheIOTimer, new Long(ioTimer));
	...
	public final int setNFSFileCacheCloseTimer(long closeTimer)
		...
		int sts = fireConfigurationChange(
			ConfigId.NFSFileCacheCloseTimer, new Long(closeTimer));
	...
	public final int setNFSFileCacheDebug(boolean ena)
		...
		if (m_nfsFileCacheDebug != ena) {
			...
			sts = fireConfigurationChange(
				ConfigId.NFSFileCacheDebug, new Boolean(ena));
