...
/**
 * Search Cache Class <p>Holds the details of the active searches for the NFS
 * server
 * 
 * @author gkspencer
 */
				...
				synchronized (m_searches) {
					...
					for (int i = 0; i < m_searches.length; i++) {
						...
						if (m_searches[i] != null &&
							m_searches[i].getTimeout() < timeNow) {
							...
							if (Debug.EnableInfo && hasDebug())
								Debug.println(
									"NFSSearchExpiry: Closed search=" +
									entry.getSearch().getSearchString() +
									", id=" + i);
						}
					}
				}
