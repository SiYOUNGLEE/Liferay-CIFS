...
public class NFSSrvSession extends SrvSession {
	...
	//
	// Identifies this session uniquely within the authentication type being
	// used by the client
	...
	// Last time the session was accessed. Used to determine when to expire UDP
	// sessions.
	 ...
	 * @param type int
	 */
	public NFSSrvSession(
		NetworkServer srv, InetAddress addr, int port, int type) {
		...
		// Create a unique id for the session from the remote address, port and
		// type
	...
	protected final void cleanupSession() {
		...
		if (Debug.EnableInfo && hasDebug(NFSServer.DBG_SESSION))
			debugPrintln(
				"NFS Cleanup session, searches=" + getSearchCount() +
				", files=" +
				(m_fileCache != null ? m_fileCache.numberOfEntries() : 0) +
				", treeConns=" +
