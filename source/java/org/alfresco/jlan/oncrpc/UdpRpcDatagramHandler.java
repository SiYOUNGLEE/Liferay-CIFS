...
/**
 * UDP RPC Datagram Handler Class <p>Receives RPC requests via a datagram and
...
public class UdpRpcDatagramHandler extends DatagramSessionHandler {
	...
	public UdpRpcDatagramHandler(
		...
		// Set the RPC server implementation that will handle the actual
		// requests
	...
	protected boolean processDatagram(DatagramPacket pkt)
		...
		// The default implementation processes the RPC immediately then returns
		// to the main datagram handler
		// to wait for the next datagram to be received. In this case the
		// datagram packet can be re-used as
		...
		if (rpcPkt.getRpcVersion() != Rpc.RpcVersion) {
			...
			pkt.setData(
				rpcPkt.getBuffer(), rpcPkt.getOffset(),
				RpcPacket.ResponseMismatchLen);
		...
		// Indicate that the existing datagram packet can be re-used for the
		// next request
