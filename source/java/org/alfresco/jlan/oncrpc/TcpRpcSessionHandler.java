...
/**
 * TCP RPC Session Handler Class <p>Receives session requests via a TCP
 * socketRPC requests via a datagram and passes the request to the registered
 * RPC server.
 * 
 * @author gkspencer
 */
public class TcpRpcSessionHandler extends SocketSessionHandler {
	...
	protected void acceptConnection(Socket sock) {
		...
		try {
			...
			// Create a packet handler for the new session and add to the active
			// session list
			...
			if (Debug.EnableInfo && hasDebug())
				Debug.println(
					"[" + getProtocolName() +
					"] Created new session id = " + sessId + ", from = " +
	...
	protected final void closeSession(int sessId) {
		...
		PacketHandlerInterface pktHandler = m_sessions.remove(
			new Integer(sessId));
		if (pktHandler != null) {
	...
	protected TcpRpcPacketHandler createPacketHandler(int sessId, Socket sock)
		...
		return new TcpRpcPacketHandler(
			this, sessId, m_rpcProcessor, sock, getMaximumRpcSize());
	}
