...
/**
 * Multi-threaded TCP RPC Session Handler Class <p>Extend the basic TCP RPC
...
public class MultiThreadedTcpRpcSessionHandler extends TcpRpcSessionHandler {
	 ...
	 * @param maxSize int
	 */
	public MultiThreadedTcpRpcSessionHandler(
	...
	protected TcpRpcPacketHandler createPacketHandler(int sessId, Socket sock)
		...
		// Create a multi-threaded packet handler to use the session handlers
		// thread pool to
		// process the RPC requests

		return new MultiThreadedTcpRpcPacketHandler(
			this, sessId, getRpcProcessor(), sock, getMaximumRpcSize());
	}
	...
	public final void setPacketPool(int poolSize) {
		...
		if (m_packetPool == null) {
			...
			m_packetPool = new RpcPacketPool(
				DefaultSmallPacketSize, poolSize, getMaximumRpcSize(),
				poolSize);
		}
	}
