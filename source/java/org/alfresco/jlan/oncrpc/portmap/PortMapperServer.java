...
public class PortMapperServer extends NetworkServer implements RpcProcessor {
	...
	public PortMapperServer(ServerConfiguration config) {
		...
		m_nfsConfig =
			(NFSConfigSection) config.getConfigSection(
	...
	public void startServer() {
		...
		try {
			...
			m_udpHandler = new UdpRpcDatagramHandler(
				"PortMap", "Port", this, this, null, getPort(), MaxRequestSize);
			...
			PortMapping portMap = new PortMapping(
				PortMapper.ProgramId, PortMapper.VersionId, Rpc.UDP, getPort());
			addPortMapping(portMap);

			portMap = new PortMapping(
				PortMapper.ProgramId, PortMapper.VersionId, Rpc.TCP, getPort());
			addPortMapping(portMap);
		}
		catch (Exception ex) {
			Debug.println(ex);
	...
	public RpcPacket processRpc(RpcPacket rpc)
		...
		if (rpc.getProgramId() != PortMapper.ProgramId) {
			...
			rpc.buildProgramMismatchResponse(
				PortMapper.VersionId, PortMapper.VersionId);
			return rpc;
		}
	...
	private final RpcPacket procSet(RpcPacket rpc) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				...
				Rpc.getServiceName(progId) + ", version=" + verId +
				", protocol=" + (proto == Rpc.TCP ? "TCP" : "UDP") + ", port=" +
				port);
	...
	private final RpcPacket procDump(RpcPacket rpc) {
		...
		if (Debug.EnableInfo && hasDebug())
			Debug.println(
				"[PortMap] Dump ports request from " + rpc.getClientDetails());
	...
	private final boolean addPortMapping(PortMapping portMap) {
		...
		key = new Integer(PortMapping.generateHashCode(
			portMap.getProgramId(), 0, portMap.getProtocol()));
	 ...
	 * @return PortMapping
	 */
	private final PortMapping findPortMapping(
		int progId, int verId, int proto) {

		// Create a key for the RPC service

		Integer key = new Integer(
			PortMapping.generateHashCode(progId, verId, proto));
