...
/**
 * Multi-Threaded UDP RPC Datagram Handler Class <p>Extend the basic UDP RPC
 * handler class to process RPC requests using a thread pool.
 * 
 * @author gkspencer
 */
public class MultiThreadedUdpRpcDatagramHandler extends UdpRpcDatagramHandler
			...
			while (mi_shutdown == false) {
				...
				if (rpc != null) {
					...
					try {
						...
						mi_txPkt.setData(
							rpc.getBuffer(), rpc.getOffset(), rpc.getLength());
	 ...
	 * @param maxSize int
	 */
	public MultiThreadedUdpRpcDatagramHandler(
		String name, String protocol, RpcProcessor rpcServer,
	...
	public void initializeSessionHandler(NetworkServer server)
		...
		if (m_packetPool == null)
			m_packetPool =
				new RpcPacketPool(
					DefaultSmallPacketSize, DefaultPacketPoolSize,
					getMaximumDatagramSize(), DefaultPacketPoolSize);

		// Create the RPC request handling thread pool, if not already created

		if (m_threadPool == null)
			m_threadPool =
				new RpcRequestThreadPool(getHandlerName(), getRpcProcessor());
	...
	protected boolean processDatagram(DatagramPacket pkt)
		...
		// Make sure that the received data is using the same buffer that we
		// allocated in the
		// allocateBuffer() method, if not the buffer did not come from the
		// packet pool.

		if (pkt.getData() != m_rxPkt.getBuffer())
			throw new IOException(
				"Received datagram is not in expected buffer");
		...
		// Indicate that the datagram buffer cannot be re-used, the main
		// datagram receiving thread must
	 ...
	 * @param largePool int
	 */
	public final void setPacketPool(
		int smallSize, int smallPool, int largeSize, int largePool) {
		...
		if (m_packetPool == null) {
			...
			m_packetPool = new RpcPacketPool(
				smallSize, smallPool, largeSize, largePool);
		}
	}
	...
	public final void setPacketPool(int poolSize) {
		...
		if (m_packetPool == null) {
			...
			m_packetPool = new RpcPacketPool(
				DefaultSmallPacketSize, poolSize, getMaximumDatagramSize(),
				poolSize);
		}
	}
	...
	public final void setThreadPool(int numThreads) {
		...
		if (m_threadPool == null) {
			...
			m_threadPool = new RpcRequestThreadPool(
				getHandlerName(), numThreads, getRpcProcessor());
		}
	}
