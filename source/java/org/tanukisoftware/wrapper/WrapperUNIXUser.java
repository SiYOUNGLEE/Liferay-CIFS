...
public class WrapperUNIXUser extends WrapperUser {
	...
	WrapperUNIXUser(
		int uid, int gid, byte[] user, byte[] realName, byte[] home,
		byte[] shell) {
		super(user);
		...
		// The real name field appears to contain several fields, we only want
		// the first.
	...
	/**
	 * Returns the WrapperUNIXGroup which corresponds to the GID. Null will be
	 * returned if groups were not requested with the user.
	 * 
