 ...
 * receiving events from the Wrapper. <p> Some of these permissions can result
 * in performance degredations if used impropperly. <p> The following are
 * examples of how to specify the permission within a policy file. <pre> grant
 * codeBase "file:../lib/-" { // Grant various permissions to a specific
 * service. permission
 * org.tanukisoftware.wrapper.security.WrapperEventPermission "service";
 * permission org.tanukisoftware.wrapper.security.WrapperEventPermission
 * "service, core"; permission
 * org.tanukisoftware.wrapper.security.WrapperEventPermission "*"; }; </pre> <p>
 * Possible eventTypes include the following: <table border='1' cellpadding='2'
 * cellspacing='0'> <tr> <th>Permission Event Type Name</th> <th>What the
 * Permission Allows</th> <th>Risks of Allowing this Permission</th> </tr> <tr>
 * <td>service</td> <td>Register to obtain events whenever the Wrapper service
 * receives any service events.</td> <td>Malicious code could receive this event
 * and never return and thus cause performance and timeout problems with the
 * Wrapper. Normal use of these events are quite safe however.</td> </tr> <tr>
 * <td>control</td> <td>Register to obtain events whenever the Wrapper receives
 * any system control signals.</td> <td>Malicious code could trap and consome
 * control events, thus preventing an application from being shut down
 * cleanly.</td> </tr> <tr> <td>core</td> <td>Register to obtain events on the
 * core workings of the Wrapper.</td> <td>Malicious code or even well meaning
 * code can greatly affect the performance of the Wrapper simply by handling
 * these methods slowly. Some of these events are fired from within the core
 * timing code of the Wrapper. They are useful for testing and performance
 * checks, but in general they should not be used by most applications. </td>
 * </tr> </table>
 * 
...
public class WrapperEventPermission extends Permission {
	...
	/**
	 * Return the canonical string representation of the eventTypes. Always
	 * returns present eventTypes in the following order: start, stop, pause,
	 * continue, interrogate. userCode.
	 * 
	...
	/**
	 * Checks if this WrapperEventPermission object "implies" the specified
	 * permission. <P> More specifically, this method returns true if:<p> <ul>
	 * <li><i>p2</i> is an instanceof FilePermission,<p> <li><i>p2</i>'s
	 * eventTypes are a proper subset of this object's eventTypes, and<p>
	 * <li><i>p2</i>'s service name is implied by this object's service name.
	 * For example, "MyApp*" implies "MyApp". </ul>
	 * 
	...
	public boolean implies(Permission p2) {
		...
		return ((m_eventTypeMask & wsp.m_eventTypeMask) == 
			wsp.m_eventTypeMask) && impliesIgnoreEventTypeMask(wsp);
	}
...
final class WECollection extends PermissionCollection {
	...
	/**
	 * Returns an enumeration of all the WrapperEventPermission objects in the
	 * container.
	 * 
	 * @return An enumeration of all the WrapperEventPermission objects.
	 */
