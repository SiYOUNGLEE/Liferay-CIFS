...
/**
 * WrapperServicePermissions are used to grant the right to start, stop, pause,
 * continue, interrogate, or send custom codes to other services running on a
 * Windows system. <p> These permissions are inherently quite dangerous so great
 * care should be taken when granting them. When doing so, try to only grant
 * permission to those services which really need to be controlled. <p> The
 * following are examples of how to specify the permission within a policy file.
 * <pre> grant codeBase "file:../lib/-" { // Grant various permissions to a
 * specific service. permission
 * org.tanukisoftware.wrapper.security.WrapperServicePermission "myservice",
 * "interrogate"; permission
 * org.tanukisoftware.wrapper.security.WrapperServicePermission "myservice",
 * "interrogate,start,stop"; permission
 * org.tanukisoftware.wrapper.security.WrapperServicePermission "myservice",
 * "userCode"; permission
 * org.tanukisoftware.wrapper.security.WrapperServicePermission "myservice",
 ...
 * "my*", "*"; // Let the calling code do anything to any service on the system
 * permission org.tanukisoftware.wrapper.security.WrapperServicePermission "*",
 * "*"; permission org.tanukisoftware.wrapper.security.WrapperServicePermission
 * "*"; }; </pre> <p> Possible actions include the following: <table border='1'
 * cellpadding='2' cellspacing='0'> <tr> <th>Permission Action Name</th>
 * <th>What the Permission Allows</th> <th>Risks of Allowing this
 * Permission</th> </tr> <tr> <td>start</td> <td>Start a service which is
 * installed but has not been started.</td> <td>Malicious code could potentially
 * start any service that is not currently running. This includes services which
 * were previously stopped or that are configured to be started manually. Many
 * Windows systems have several services stopped by default because of the
 * security hazards that they pose. Starting such services could open the system
 * up to attacks related to that service.</td> </tr> <tr> <td>stop</td> <td>Stop
 * a service which is currently running.</td> <td>Malicious code could
 * potentially stop running service. This could result in a denial of service
 * attack if the service is a web or database server. Or it result in more
 * dangerous attacks if the service is a firewall or virus scanner. </td> </tr>
 * <tr> <td>pause</td> <td>Pause a service which is currently running.</td>
 * <td>Malicious code could potentially pause running service. This could result
 * in a denial of service attack if the service is a web or database server. Or
 * it result in more dangerous attacks if the service is a firewall or virus
 * scanner. </td> </tr> <tr> <td>continue</td> <td>Continue a service which was
 * previously paused.</td> <td>Malicious code could resume services which had
 * been paused for a good reason.</td> </tr> <tr> <td>interrogate</td>
 * <td>Interrogate a service as to its current state.</td> <td>Malicious code
 * learn a lot about a system and its weakness by probing which services are
 * currently running.</td> </tr> <tr> <td>userCode</td> <td>Send any custom user
 * code to a service.</td> <td>The danger of this action depends on whether or
 * not the service understands custom user codes, and what it does with them.
 * This could potentially be a very dangerous permission to grant.</td> </tr>
 * </table>
 * 
...
public class WrapperServicePermission extends Permission {
	...
	private static int MASK_ALL =
		MASK_START | MASK_STOP | MASK_PAUSE | MASK_CONTINUE | MASK_INTERROGATE |
			MASK_USER_CODE;
	...
	/**
	 * Creates a new WrapperServicePermission for the specified service. This
	 * version of the constructor grants all actions.
	 * 
	 * @param serviceName The name of the service whose access is being
	 *            controlled.
	 */
	...
	/**
	 * Checks two Permission objects for equality. <p> Do not use the equals
	 * method for making access control decisions; use the implies method.
	 * 
	 ...
	 * present actions in the following order: start, stop, pause, continue,
	 * interrogate. userCode.
	 * 
	...
	/**
	 * Checks if this WrapperServicePermission object "implies" the specified
	 * permission. <P> More specifically, this method returns true if:<p> <ul>
	 * <li><i>p2</i> is an instanceof FilePermission,<p> <li><i>p2</i>'s actions
	 * are a proper subset of this object's actions, and<p> <li><i>p2</i>'s
	 * service name is implied by this object's service name. For example,
	 * "MyApp*" implies "MyApp". </ul>
	 * 
	...
	/**
	 * Tests whether this permissions implies another without taking the action
	 * mask into account.
	 */
	boolean impliesIgnoreActionMask(WrapperServicePermission p2) {
		...
		if (p2.getName().endsWith("*")) {
			if (getName().startsWith(
				p2.getName().substring(0, p2.getName().length() - 1))) {
				return true;
			}
		}
		return false;
...
final class WSCollection extends PermissionCollection {
	 ...
	 *                FilePermission
	 * @exception SecurityException - if this FilePermissionCollection object has
	...
	/**
	 * Returns an enumeration of all the WrapperServicePermission objects in the
	 * container.
	 * 
	 * @return An enumeration of all the WrapperServicePermission objects.
	 */
