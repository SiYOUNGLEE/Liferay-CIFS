...
/**
 * If an application instantiates an instance of this class, the JVM will listen
 * on the specified port for connections. When a connection is detected, the
 * first byte of input will be read from the socket and then the connection will
 * be immediately closed. An action will then be performed based on the byte
 * read from the stream. <p> The easiest way to invoke an action manually is to
 * telnet to the specified port and then type the single command key.
 * <code>telnet localhost 9999</code>, for example. <p> Valid commands include:
 * <ul> <li><b>S</b> : Shutdown cleanly.</li> <li><b>H</b> : Immediate forced
 * shutdown.</li> <li><b>R</b> : Restart</li> <li><b>D</b> : Perform a Thread
 * Dump</li> <li><b>U</b> : Unexpected shutdown. (Simulate a crash for
 * testing)</li> <li><b>V</b> : Cause an access violation. (For testing)</li>
 * <li><b>G</b> : Make the JVM appear to be hung. (For testing)</li> </ul>
 * Additional user defined actions can be defined by calling the
 * {@link #registerAction(byte command, Runnable action )} method. The Wrapper
 * project reserves the right to define any upper case commands in the future.
 * To avoid future conflicts, please use lower case for user defined commands.
 * <p> This application will work even in most deadlock situations because the
 * thread is in issolation from the rest of the application. If the JVM is
 * truely hung, this class will fail to accept connections but the Wrapper
 * itself will detect the hang and restart the JVM externally. <p> The following
 * code can be used in your application to start up the WrapperActionServer with
 * all default actions enabled: <pre> int port = 9999; WrapperActionServer
 * server = new WrapperActionServer( port ); server.enableShutdownAction( true
 * ); server.enableHaltExpectedAction( true ); server.enableRestartAction( true
 * ); server.enableThreadDumpAction( true ); server.enableHaltUnexpectedAction(
 * true ); server.enableAccessViolationAction( true );
 * server.enableAppearHungAction( true ); server.start(); </pre> Then remember
 * to stop the server when your application shuts down: <pre> server.stop();
 * </pre>
 * 
...
public class WrapperActionServer implements Runnable {
	...
	/**
	 * Creates and starts WrapperActionServer instance bound to the specified
	 * port and address.
	 * 
	...
	/**
	 * Creates and starts WrapperActionServer instance bound to the specified
	 * port. The socket will bind to all addresses and should be concidered a
	 * security risk.
	 * 
	 * @param port Port on which to listen for connections.
	 */
	...
	public void run() {
		...
		try {
				...
				try {
					...
					if (command >= 0) {
						...
						synchronized (m_actions) {
							action =
								(Runnable) m_actions.get(new Integer(command));
						}
					...
					// Check for throwable type this way rather than with
					// seperate catches
					// to work around a problem where InterruptedException can
					// be thrown
			...
			synchronized (this) {
				...
				// Wake up the stop method if it is waiting for the runner to
				// stop.
	 ...
	 *             specified port or there are any other problems opening a
	 *             socket.
	 */
	public void start()
		throws IOException {
	 ...
	 * dumped to the console if uncaught by the action. To avoid this, wrap the
	 * code in a <code>try { ... } catch (Throwable t) { ... }</code> block.
	 * 
	 * @param command Command to be registered. Will override any exiting action
	 *            already registered with the same command.
	...
	/**
	 * Enable or disable the expected halt command. Disabled by default. This
	 * will shutdown the JVM, but will do so immediately without going through
	...
	/**
	 * Enable or disable the unexpected halt command. Disabled by default. If
	 * this command is executed, the Wrapper will think the JVM crashed and
	 * restart it.
	 * 
	 * @param enable True to enable to action, false to disable it.
	 */
		...
		if (enable) {
			registerAction(COMMAND_HALT_UNEXPECTED, new Runnable() {
				...
				public void run() {
					...
					// Execute runtime.halt(0) using reflection so this class
					// will
					...
					try {
						haltMethod =
							Runtime.class.getMethod("halt", new Class[] {
								Integer.TYPE
							});
					}
					...
					if (haltMethod != null) {
						...
						try {
							...
							m_out.println("Unable to call runitme.halt: " +
								e.getMessage());
						}
	...
	/**
	 * Enable or disable the access violation command. Disabled by default. This
	 * command is useful for testing how an application handles the worst case
	 * situation where the JVM suddenly crashed. When this happens, the the JVM
	 * will simply die and there will be absolutely no chance for any shutdown
	 * or cleanup work to be done by the JVM.
	 * 
	 * @param enable True to enable to action, false to disable it.
	 */
	...
	/**
	 * Enable or disable the appear hung command. Disabled by default. This
	 * command is useful for testing how an application handles the situation
	 * where the JVM stops responding to the Wrapper's ping requests. This can
	 * happen if the JVM hangs or some piece of code deadlocks. When this
	 * happens, the Wrapper will give up after the ping timeout has expired and
	 * kill the JVM process. The JVM will not have a chance to clean up and
	 * shudown gracefully.
	 * 
	 * @param enable True to enable to action, false to disable it.
	 */
